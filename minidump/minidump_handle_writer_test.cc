...
void GetHandleDataStream(
  ...
  ASSERT_EQ(
            directory[kDirectoryIndex].StreamType,kMinidumpStreamTypeHandleData);
...
TEST(MinidumpHandleDataWriter, OneHandle) {
  ...
  EXPECT_EQ(
            base::UTF16ToUTF8(MinidumpStringAtRVAAsString(
                string_file.string(), handle_descriptor->TypeNameRva)),handle_snapshot.type_name);
...
TEST(MinidumpHandleDataWriter, RepeatedTypeName) {
  ...
  EXPECT_EQ(
            base::UTF16ToUTF8(MinidumpStringAtRVAAsString(
                string_file.string(), handle_descriptor2->TypeNameRva)),handle_snapshot2.type_name);
