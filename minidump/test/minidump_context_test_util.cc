...
void InitializeMinidumpContextPPC64(MinidumpContextPPC64* context,
  ...
  uint64_t value = seed;
  for (size_t i = 0; i < arraysize(context->regs); ++i) {
  ...
  for (size_t i = 0; i < arraysize(context->fpregs); ++i) {
    context->fpregs[i] = static_cast<double>(i);
  }
  context->fpscr = value++;

  for (size_t i = 0; i < arraysize(context->vregs.save_vr); ++i) {
...
void ExpectMinidumpContextPPC64(uint32_t expect_seed,
  ...
  for (size_t i = 0; i < arraysize(expected.regs); ++i) {
  ...
  for (size_t i = 0; i < arraysize(expected.fpregs); ++i) {
    EXPECT_EQ(observed->fpregs[i], expected.fpregs[i]);
  }
  EXPECT_EQ(observed->fpscr, expected.fpscr);

  for (size_t i = 0; i < arraysize(expected.vregs.save_vr); ++ i) {
