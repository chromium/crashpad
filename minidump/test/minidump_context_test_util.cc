  ...
  for (size_t st_mm_index = 0;
    ...
    EXPECT_EQ(
              BytesToHexString(observed->st_mm[st_mm_index].st,
                               arraysize(observed->st_mm[st_mm_index].st)),BytesToHexString(expected->st_mm[st_mm_index].st,
  ...
  for (size_t xmm_index = 0;
    ...
    EXPECT_EQ(
              BytesToHexString(observed->xmm[xmm_index],
                               arraysize(observed->xmm[xmm_index])),BytesToHexString(expected->xmm[xmm_index],
  ...
  for (size_t index = 0; index < arraysize(expected.fsave.st); ++index) {
    EXPECT_EQ(
              BytesToHexString(observed->fsave.st[index],
                               arraysize(observed->fsave.st[index])),BytesToHexString(expected.fsave.st[index],
...
void ExpectMinidumpContextAMD64(
  ...
  for (size_t index = 0; index < arraysize(expected.vector_register); ++index) {
    if (snapshot) {
      ...
      EXPECT_EQ(
                observed->vector_register[index].lo,expected.vector_register[index].lo) << "index " << index;
      EXPECT_EQ(
                observed->vector_register[index].hi,expected.vector_register[index].hi) << "index " << index;
    }
  }
  ...
  if (snapshot) {
    ...
    EXPECT_EQ(
              observed->last_exception_from_rip,expected.last_exception_from_rip);
  }
}
