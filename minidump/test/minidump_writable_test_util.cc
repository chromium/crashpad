...
template <>
const CodeViewRecordElf*
MinidumpWritableAtLocationDescriptor<CodeViewRecordElf>(
    const std::string& file_contents,
    const MINIDUMP_LOCATION_DESCRIPTOR& location) {
  const CodeViewRecordElf* cv = reinterpret_cast<const CodeViewRecordElf*>(
      MinidumpWritableAtLocationDescriptorInternal(
          file_contents,
          location,
          offsetof(CodeViewRecordElf, build_id),
          true));
  ...
  if (cv->signature != CodeViewRecordElf::kSignature) {
    return nullptr;
  }
