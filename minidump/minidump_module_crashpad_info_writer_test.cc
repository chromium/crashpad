...
TEST(MinidumpModuleCrashpadInfoWriter, EmptyList) {
  ...
  ASSERT_EQ(
            string_file.string().size(),sizeof(MinidumpModuleCrashpadInfoList));
...
TEST(MinidumpModuleCrashpadInfoWriter, FullModule) {
  ...
  EXPECT_EQ(
            module_list->modules[0].minidump_module_list_index,kMinidumpModuleListIndex);
  const MinidumpModuleCrashpadInfo* module =
      MinidumpWritableAtLocationDescriptor<MinidumpModuleCrashpadInfo>(
          string_file.string(), module_list->modules[0].location);
  ASSERT_TRUE(module);
  ...
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(string_file.string(),
                                            list_annotations->children[0]),kEntry);
  ...
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations->entries[0].key),kKey);
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations->entries[0].value),kValue);
}
...
TEST(MinidumpModuleCrashpadInfoWriter, ThreeModules) {
  ...
  EXPECT_EQ(
            module_list->modules[0].minidump_module_list_index,kMinidumpModuleListIndex0);
  ...
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations_0->entries[0].key),kKey0);
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations_0->entries[0].value),kValue0);

  EXPECT_EQ(
            module_list->modules[1].minidump_module_list_index,kMinidumpModuleListIndex1);
  ...
  EXPECT_EQ(
            module_list->modules[2].minidump_module_list_index,kMinidumpModuleListIndex2);
  ...
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations_2->entries[0].key),kKey2A);
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations_2->entries[0].value),kValue2A);
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations_2->entries[1].key),kKey2B);
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations_2->entries[1].value),kValue2B);
}
...
TEST(MinidumpModuleCrashpadInfoWriter, InitializeFromSnapshot) {
  ...
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations_0->entries[0].key),kKey0B);
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations_0->entries[0].value),kValue0B);
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations_0->entries[1].key),kKey0A);
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
  ...
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations_2->entries[0].key),kKey2);
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(
                string_file.string(), simple_annotations_2->entries[0].value),kValue2);
  ...
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(string_file.string(),
                                            list_annotations_3->children[0]),kEntry3A);
  EXPECT_EQ(
            MinidumpUTF8StringAtRVAAsString(string_file.string(),
                                            list_annotations_3->children[1]),kEntry3B);
