...
void MinidumpAnnotationWriter::InitializeFromAnnotation(
...
void MinidumpAnnotationListWriter::InitializeFromList(
  ...
  for (const auto& annotation : list) {
    ...
    writer->InitializeFromAnnotation(annotation);
...
bool MinidumpAnnotationListWriter::WriteObject(
  ...
  for (size_t i = 0; i < objects_.size(); ++i) {
    size_t index = i + 1;
    iov[index].iov_base = objects_[i]->minidump_annotation();
    iov[index].iov_len = sizeof(MinidumpAnnotation);
  }
