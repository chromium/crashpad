...
std::vector<TestRange> ParseCoalesceSpec(const char* spec) {
  ...
  enum { None, Space, Dot } state = None;
...
TEST(CoalesceSpecHelper, Parse) {
  const auto empty = ParseCoalesceSpec("");
  ASSERT_EQ(empty.size(), 0u);

  const auto a = ParseCoalesceSpec("...");
  ASSERT_EQ(a.size(), 1u);
  EXPECT_EQ(a[0].base, 0);
  EXPECT_EQ(a[0].size, 3);

  const auto b = ParseCoalesceSpec("  ...");
  ASSERT_EQ(b.size(), 1u);
  EXPECT_EQ(b[0].base, 2);
  EXPECT_EQ(b[0].size, 3);

  const auto c = ParseCoalesceSpec("  ...  ");
  ASSERT_EQ(c.size(), 1u);
  EXPECT_EQ(c[0].base, 2);
  EXPECT_EQ(c[0].size, 3);

  const auto d = ParseCoalesceSpec("  ...  ....");
  ASSERT_EQ(d.size(), 2u);
  EXPECT_EQ(d[0].base, 2);
  EXPECT_EQ(d[0].size, 3);
  EXPECT_EQ(d[1].base, 7);
  EXPECT_EQ(d[1].size, 4);

  const auto e = ParseCoalesceSpec("  ...  ...... ... ");
  ASSERT_EQ(e.size(), 3u);
  EXPECT_EQ(e[0].base, 2);
  EXPECT_EQ(e[0].size, 3);
  EXPECT_EQ(e[1].base, 7);
  EXPECT_EQ(e[1].size, 6);
  EXPECT_EQ(e[2].base, 14);
  EXPECT_EQ(e[2].size, 3);
}
...
void CoalesceTest(const char* r1_spec, const char* r2_spec, const char* expected_spec) {
  ...
  for (const auto& range : expected) {
    ...
    mmd.Memory.DataSize = range.size;
