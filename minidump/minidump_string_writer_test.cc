...
TEST(MinidumpStringWriter, MinidumpUTF16StringWriter) {
...

  {
    ...
    EXPECT_EQ(
              MinidumpStringAtRVAAsString(string_file.string(), 0),base::string16());
  }
  ...
  for (size_t index = 0; index < arraysize(kTestData); ++index) {
    ...
    ASSERT_EQ(
              string_file.string().size(),sizeof(MINIDUMP_STRING) + expected_utf16_bytes);
    ...
    EXPECT_EQ(
              MinidumpStringAtRVAAsString(string_file.string(), 0),expect_string);
  }
}
...
TEST(MinidumpStringWriter, ConvertInvalidUTF8ToUTF16) {
  ...
  for (size_t index = 0; index < arraysize(kTestData); ++index) {
    ...
    ALLOW_UNUSED_LOCAL(tmp);
    EXPECT_EQ(
              minidump_string->Length,string_file.string().size() - sizeof(MINIDUMP_STRING) -
                  sizeof(tmp.Buffer[0]));
...
TEST(MinidumpStringWriter, MinidumpUTF8StringWriter) {
...

  {
    ...
    EXPECT_TRUE(minidump_string);
    EXPECT_EQ(
              MinidumpUTF8StringAtRVAAsString(string_file.string(), 0),std::string());
  }
  ...
  for (size_t index = 0; index < arraysize(kTestData); ++index) {
    ...
    ASSERT_EQ(
              string_file.string().size(),sizeof(MinidumpUTF8String) + expected_utf8_bytes_with_nul);

    const MinidumpUTF8String* minidump_string =
        MinidumpUTF8StringAtRVA(string_file.string(), 0);
    EXPECT_TRUE(minidump_string);
    EXPECT_EQ(
              MinidumpUTF8StringAtRVAAsString(string_file.string(), 0),test_string);
  }
}
  ...
  for (size_t index = 0; index < strings.size(); ++index) {
    EXPECT_EQ(
              Traits::ObservationAtRVA(string_file.string(),
                                       list->children[index]),Traits::ExpectationForUTF8(strings[index]));
  }
}
