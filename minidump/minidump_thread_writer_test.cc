...
void ExpectThread(const MINIDUMP_THREAD* expected,
  ...
  EXPECT_EQ(
            observed->Stack.StartOfMemoryRange,expected->Stack.StartOfMemoryRange);
...
TEST(MinidumpThreadWriter, ThreeThreads_x86_MemoryList) {
  ...
  ASSERT_EQ(  // 12 for alignment
            string_file.string().size(),sizeof(MINIDUMP_HEADER) + 2 * sizeof(MINIDUMP_DIRECTORY) +
