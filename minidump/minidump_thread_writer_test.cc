...
class MinidumpThreadWriterSnapshotTest : public testing::Test {
 public:
  ...
  void InitializeFromSnapshot(bool thread_id_collision, bool has_dump_thread) {
    uint64_t thread_ids[kThreadCount] = {};
 ...
 private:
  ...
  const MINIDUMP_MEMORY_LIST* memory_list_;
  StringFile string_file_;
  MINIDUMP_THREAD expect_threads_[kThreadCount];
  uint32_t context_seeds_[kThreadCount];
  uint8_t memory_values_[kThreadCount];
  MINIDUMP_MEMORY_DESCRIPTOR tebs_[kThreadCount];

  DISALLOW_COPY_AND_ASSIGN(MinidumpThreadWriterSnapshotTest);
};
...
template <typename Traits>
constexpr size_t MinidumpThreadWriterSnapshotTest<Traits>::kThreadCount;
