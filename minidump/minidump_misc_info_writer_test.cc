...
void ExpectMiscInfoEqual<MINIDUMP_MISC_INFO_2>(
  ...
  EXPECT_EQ(
            observed->ProcessorCurrentIdleState,expected->ProcessorCurrentIdleState);
}
...
void ExpectMiscInfoEqual<MINIDUMP_MISC_INFO_5>(
  ...
  EXPECT_EQ(
            observed->XStateData.EnabledFeatures,expected->XStateData.EnabledFeatures);
  for (size_t feature_index = 0;
    ...
    EXPECT_EQ(
              observed->XStateData.Features[feature_index].Offset,expected->XStateData.Features[feature_index].Offset);
    EXPECT_EQ(
              observed->XStateData.Features[feature_index].Size,expected->XStateData.Features[feature_index].Size);
  }
