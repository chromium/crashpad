...
// ExpectModuleWithElfCv() is like ExpectModule( but expects the module to
// have a Elf CodeView Record.
void ExpectModuleWithElfCv(const MINIDUMP_MODULE* expected,
                           const MINIDUMP_MODULE* observed,
                           const std::string& file_contents,
                           const std::string& expected_module_name,
  ...
  const CodeViewRecordElf* codeview_elf_record =
      MinidumpWritableAtLocationDescriptor<CodeViewRecordElf>(
          file_contents, observed->CvRecord);
  ASSERT_TRUE(codeview_elf_record);
  EXPECT_EQ(memcmp(expected_build_id.data(),
                   &codeview_elf_record->build_id,
...
TEST(MinidumpModuleWriter, OneModule_CodeViewElf) {
  // MinidumpModuleWriter.OneModule tested with a Elf CodeView
  MinidumpFileWriter minidump_file_writer;
  auto module_list_writer = std::make_unique<MinidumpModuleListWriter>();
  ...
  auto codeview_elf_writer =
      std::make_unique<MinidumpModuleCodeViewRecordElfWriter>();
  codeview_elf_writer->SetBuildID(build_id_data);
  module_writer->SetCodeViewRecord(std::move(codeview_elf_writer));
  ...
  ASSERT_NO_FATAL_FAILURE(ExpectModuleWithElfCv(&expected,
                                                &module_list->Modules[0],
                                                string_file.string(),
                                                kModuleName,
                                                build_id_data));
