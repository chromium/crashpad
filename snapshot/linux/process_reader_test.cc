...
class TestThreadPool {
 public:
  ...
  ~TestThreadPool() {
    ...
    for (const ThreadInfo* thread_info : thread_infos_) {
      EXPECT_EQ(pthread_join(thread_info->pthread, nullptr), 0)
  ...
  void StartThreads(size_t thread_count, size_t stack_size = 0) {
    for (size_t thread_index = 0; thread_index < thread_count; ++thread_index) {
      ...
      if (stack_size > 0) {
        void* stack_ptr;
        ASSERT_EQ(
            (errno = posix_memalign(&stack_ptr, getpagesize(), stack_size)), 0)
            << ErrnoMessage("posix_memalign");
        thread_info->stack.reset(stack_ptr);

        thread_info->stack.reset(new char[stack_size]);
        ASSERT_EQ(
            pthread_attr_setstack(&attr, thread_info->stack.get(), stack_size),
            0)
            << ErrnoMessage("pthread_attr_setstack");
        thread_info->expectation.max_stack_size = stack_size;
      }

      ASSERT_EQ(pthread_attr_setschedpolicy(&attr, SCHED_OTHER), 0)
          << ErrnoMessage("pthread_attr_setschedpolicy");
      thread_info->expectation.sched_policy = SCHED_OTHER;

      sched_param param;
      param.sched_priority = 0;
      ASSERT_EQ(pthread_attr_setschedparam(&attr, &param), 0)
          << ErrnoMessage("pthread_attr_setschedparam");
      thread_info->expectation.static_priority = 0;

      thread_info->expectation.nice_value = thread_index % 20;

      ASSERT_EQ(
          pthread_create(&thread_info->pthread, &attr, ThreadMain, thread_info),
          0)
  ...
  pid_t GetThreadExpectation(size_t thread_index,
    ...
    *expectation = thread_info->expectation;
 ...
 private:
  ...
  static void* ThreadMain(void* argument) {
    ...
    CHECK_EQ(setpriority(PRIO_PROCESS, 0, thread_info->expectation.nice_value),
             0)
        << ErrnoMessage("setpriority");
    thread_info->expectation.stack_address =
        reinterpret_cast<LinuxVMAddress>(&thread_info);
    thread_info->tid = gettid();
...
class ChildThreadTest : public Multiprocess {
 ...
 private:
  void MultiprocessParent() override {
    ...
    ExpectThreads(thread_map, threads);
  }
...
//  ASSERT_NO_FATAL_FAILURE(thread_pool.StartThreads(kThreadCount));
//
//  const std::vector<ProcessReader::Thread>& threads =
//  process_reader.Threads();
//
//  ASSERT_GE(threads.size(), kThreadCount);
//  EXPECT_EQ(threads[0].tid, getpid());
