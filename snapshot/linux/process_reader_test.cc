...
int ExpectFindModule(dl_phdr_info* info, size_t size, void* data) {
  SCOPED_TRACE(base::StringPrintf("module %s at 0x%" PRIx64 " phdrs 0x%" PRIx64,
                                  info->dlpi_name,
                                  VMAddress{info->dlpi_addr},
                                  FromPointerCast<VMAddress>(info->dlpi_phdr)));

  auto modules =
      reinterpret_cast<const std::vector<ProcessReader::Module>*>(data);

  auto phdr_addr = FromPointerCast<VMAddress>(info->dlpi_phdr);
