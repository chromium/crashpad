...
void SetTLS(LinuxVMAddress* address) {
  LinuxVMAddress tls;
...
class TestThreadPool {
  ...
  void StartThreads(size_t thread_count, size_t stack_size = 0) {
    for (size_t thread_index = 0; thread_index < thread_count; ++thread_index) {
      ...
      if (stack_size > 0) {
        ...
        ASSERT_EQ(
            pthread_attr_setstack(&attr, thread->stack.get(), stack_size),
            0)
      ...
      ASSERT_EQ(
          pthread_create(&thread->pthread, &attr, ThreadMain, thread),
          0)
 ...
 private:
  ...
  static void* ThreadMain(void* argument) {
    ...
    CHECK_EQ(setpriority(PRIO_PROCESS, 0, thread->expectation.nice_value),
             0)
        << ErrnoMessage("setpriority");

    SetTLS(&thread->expectation.tls);
