...
class TestThreadPool {
 public:
  ...
  TestThreadPool() : thread_infos_(), thread_stacks_() {}
  ...
  void StartThreads(size_t thread_count, size_t stack_size = 0) {
    for (size_t thread_index = 0; thread_index < thread_count; ++thread_index) {
      ...
      thread_info->expectation.priority = thread_index;
      ...
      if (stack_size > 0) {
        thread_stacks_.push_back(std::unique_ptr<char[]>(new char[stack_size]));
        ASSERT_EQ(pthread_attr_setstack(
                      &attr, thread_stacks_.back().get(), stack_size),
                  0)
 ...
 private:
  ...
  static void* ThreadMain(void* argument) {
    ...
    CHECK_EQ(setpriority(PRIO_PROCESS, 0, thread_info->expectation.priority), 0)
  ...
  std::vector<std::unique_ptr<char[]>> thread_stacks_;
...
void ExpectThreads(const ThreadMap& thread_map,
  ...
  for (auto& thread : threads) {
    ...
    EXPECT_EQ(thread.priority, iterator->second.priority);
...
TEST(ProcessReader, ChildThreadsWithUserStacks) {
  ChildThreadTest test(getpagesize() * 4);
  test.Run();
}
