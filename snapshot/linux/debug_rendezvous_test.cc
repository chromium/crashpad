...
void TestAgainstTarget(pid_t pid, bool is_64_bit) {
  ...
  // Android's loader never sets the dynamic array for the executable and
  // doesn't set the load bias until Android 4.3 (API 18).
  ...
  for (const DebugRendezvous::LinkEntry& module : debug.Modules()) {
    ...
    // (API 17).
    if (android_runtime_api < 17 && (module.name == "/system/bin/linker" ||
    ...
    EXPECT_PRED2(
        [](const MemoryMap::Mapping* mapping, const std::string& module_name) {
          return module_name.empty() ==
                 (mapping->device == 0 && mapping->inode == 0 &&
                  mapping->name == "[vdso]");
        },
        module_mapping,
        module.name);
    ...
    // Android's loader doesn't set its load bias until Android 6.0 (API 23).
    if (android_runtime_api < 23 && (module.name == "/system/bin/linker" ||
                                     module.name == "/system/bin/linker64")) {
      EXPECT_EQ(module.load_bias, 0);
    } else {
