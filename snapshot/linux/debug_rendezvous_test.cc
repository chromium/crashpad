...
void TestAgainstTarget(pid_t pid, bool is_64_bit) {
  ...
  ASSERT_TRUE(debug.Initialize(range, debug_address, mappings.Size()));
  ...
  for (const DebugRendezvous::LinkEntry& module : debug.Modules()) {
    ...
    // Android's loader doesn't set its own load bias between Android 4.4.4
    // (API 20) and Android 6.0 (API 23).
