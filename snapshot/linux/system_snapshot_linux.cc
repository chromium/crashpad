...
bool ReadCPUsOnline(uint8_t* first_cpu, uint8_t* cpu_count) {
  std::string contents;
  ...
  if (!base::IsValueInRangeForNumericType<uint8_t>(count)) {
    LOG(WARNING) << "count exceeds storage";
  }
  *cpu_count = base::saturated_cast<uint8_t>(count);
  *first_cpu = first;
  return true;
}
