...
#include "util/linux/reinterpret_bytes.h"
...
bool ThreadSnapshotLinux::Initialize(
  ...
  if (process_reader->Is64Bit()) {
    ...
    InitializeCPUContextX86_64(thread.thread_context,
                               thread.float_context,
                               context_.x86_64)
  } else {
    context_.architecture = kCPUArchitectureX86;
    context_.x86 = &context_union.x86;
    InitializeCPUContextX86(thread.thread_context,
                            thread.float_context,
                            context_.x86)
  }
#else
#error Port.
  ...
  struct LinuxPriority {
    ...
    // priority, so place that in the most significant byte position.
    uint8_t static_priority = 0;

    // Put this in the most significant byte position to ensure to prevent
    // negative priorities.
    uint8_t unused = 0;
  ...
  static_assert(sizeof(LinuxPriority) <= sizeof(int));
