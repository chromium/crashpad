...
bool DebugRendezvous::Initialize(const ProcessMemoryRange& memory,
  ...
  if (!(memory.Is64Bit()
            ? InitializeSpecific<Traits64>(memory, address, max_entries)
            : InitializeSpecific<Traits32>(memory, address, max_entries))) {
    return false;
  }
...
bool DebugRendezvous::InitializeSpecific(const ProcessMemoryRange& memory,
  ...
  while (link_entry_address) {
    if (visited.size() >= max_entries) {
      LOG(ERROR) << "max entries";
      return false;
    }
