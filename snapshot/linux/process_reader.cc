...
bool ProcessReader::Thread::InitializeStack(ProcessReader* reader) {
  LinuxVMAddress stack_pointer;
#if defined(ARCH_CPU_X86_FAMILY)
  stack_pointer = reader->Is64Bit() ? thread_context.t64.rsp
                             : thread_context.t32.esp;
#elif defined(ARCH_CPU_ARM_FAMILY)
  stack_pointer = reader->Is64Bit() ? thread_context.t64.sp
                             : thread_context.t32.sp;
#else
#error Port.
#endif
  ...
  if (!mapping->readable) {
    ...
    if (reader->Is64Bit()) {
      ...
      LinuxVMAddress red_zone_base = stack_region_start - std::min(kRedZoneSize, stack_region_start);
...
void ProcessReader::InitializeThreads() {
  ...
  Thread main_thread;
  main_thread.tid = ProcessID();
  if (!main_thread.InitializePtrace() ||
      !main_thread.InitializeStack(this)) {
    LOG(ERROR) << "Couldn't initialize main thread.";
    return;
  }
