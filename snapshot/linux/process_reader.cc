...
void ProcessReader::Thread::InitializeStack(ProcessReader* reader) {
  ...
  stack_pointer =
      reader->Is64Bit() ? thread_info.thread_context.t64.rsp : thread_info.thread_context.t32.esp;
#elif defined(ARCH_CPU_ARM_FAMILY)
  stack_pointer =
      reader->Is64Bit() ? thread_info.thread_context.t64.sp : thread_info.thread_context.t32.sp;
#else
#error Port.
#endif
  ...
  if (tid != reader->ProcessID() &&
    ...
    stack_region_size = thread_info.thread_specific_data_address - stack_region_address;
  }
}
...
bool ProcessReader::Initialize(PtraceConnection* connection, pid_t pid) {
  INITIALIZATION_STATE_SET_INITIALIZING(initialized_);
  connection_ = connection;

  if (!process_info_.InitializeWithPtrace(connection_, pid)) {
    return false;
  }
