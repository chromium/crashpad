...
void ProcessReader::InitializeThreads() {
  ...
  char path[32];
  snprintf(path, arraysize(path), "/proc/%d/task", ProcessID());
  ...
  if (!main_thread.InitializePtrace() || !main_thread.InitializeStack(this)) {
  ...
  while ((dir_entry = readdir(scoped_dir.get()))) {
    ...
    if (tid == ProcessID()) {
      ...
      Thread thread;
      thread.tid = tid;
      if (thread.InitializePtrace() && thread.InitializeStack(this)) {
