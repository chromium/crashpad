...
class ProcessReader {
 public:
  ...
  struct Thread {
    ...
    bool InitializeStack(ProcessReader* reader);
  ...
  //! \brief Return a vector of threads that are in the task process. The
  //!     element at index `0` corresponds to the main thread.
  const std::vector<Thread>& Threads();

 private:
  void InitializeThreads();
  void PtraceInitializeThreads();
  bool InitializeThreadStack(Thread* thread);
