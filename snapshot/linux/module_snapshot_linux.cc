...
bool ModuleSnapshotLinux::GetCrashpadOptions(CrashpadInfoClientOptions* options) {
  ...
  options->crashpad_handler_behavior = crashpad_info_->CrashpadHandlerBehavior();
  options->system_crash_reporter_forwarding = crashpad_info_->SystemCrashReporterForwarding();
  options->gather_indirectly_referenced_memory = crashpad_info_->GatherIndirectlyReferencedMemory();
  options->indirectly_referenced_memory_cap = crashpad_info_->IndirectlyReferencedMemoryCap();
  return true;
}
...
std::map<std::string, std::string> ModuleSnapshotLinux::AnnotationsSimpleMap()
  ...
  if (crashpad_info_ && crashpad_info_->SimpleAnnotations()) {
    AnnotationReader reader(elf_reader_->Memory());
...
std::vector<AnnotationSnapshot> ModuleSnapshotLinux::AnnotationObjects() const {
  ...
  if (crashpad_info_ && crashpad_info_->AnnotationsList()) {
    AnnotationReader reader(elf_reader_->Memory());
    reader.AnnotationList(crashpad_info_->AnnotationsList(), &annotations);
  }
  return annotations;
}
