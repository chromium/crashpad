...
using NativeCPUContext = struct {
  ucontext_t ucontext;
  char extra[1024];
};

struct SignalCrunchContext {
  uint32_t mvdx[16][2];
  uint32_t mvax[4][3];
  uint32_t dspsc[2];
};

struct SignalIWMMXTContext {
  uint32_t save[38];
};
...
void InitializeContext(NativeCPUContext* context) {
  ...
  static_assert(
      sizeof(internal::CoprocessorContextHead) * 5 +
              sizeof(SignalCrunchContext) + sizeof(SignalIWMMXTContext) * 2 +
              sizeof(user_vfp) + sizeof(user_vfp_exc) <=
  ...
  header->size =
      sizeof(internal::CoprocessorContextHead) + sizeof(SignalCrunchContext);
  LOG(INFO) << "set crunch size " << header->size;
  context_space += sizeof(*header);
  auto crunch = reinterpret_cast<SignalCrunchContext*>(context_space);
  ...
  header->size =
      sizeof(internal::CoprocessorContextHead) + sizeof(SignalIWMMXTContext);
  LOG(INFO) << "set iwmmxt size " << header->size;
  context_space += sizeof(*header);
  auto iwmmxt = reinterpret_cast<SignalIWMMXTContext*>(context_space);
  memset(iwmmxt, 'i', sizeof(*iwmmxt));
  context_space += sizeof(*iwmmxt);

  // According to the ABI, a dummy entry may be used to place unused space in
  // the signal context. In practice, the kernel may use this to store an unused
  ...
  header->size =
      sizeof(internal::CoprocessorContextHead) + sizeof(SignalIWMMXTContext);
  context_space += sizeof(*header);
  auto dummy = reinterpret_cast<SignalIWMMXTContext*>(context_space);
  memset(dummy, 'd', sizeof(*dummy));
  context_space += sizeof(*dummy);

  header = reinterpret_cast<internal::CoprocessorContextHead*>(context_space);
  header->magic = VFP_MAGIC;
  header->size = sizeof(internal::CoprocessorContextHead) + sizeof(user_vfp) +
                 sizeof(user_vfp_exc);
  context_space += sizeof(*header);
  auto vfp = reinterpret_cast<user_vfp*>(context_space);
  for (size_t reg = 0; reg < arraysize(vfp->fpregs); ++reg) {
    vfp->fpregs[reg] = reg;
  }
  vfp->fpscr = 42;
  context_space += sizeof(*vfp);

  auto vfp_exc = reinterpret_cast<user_vfp_exc*>(context_space);
  memset(vfp_exc, 'v', sizeof(*vfp_exc));
  context_space += sizeof(*vfp_exc);
