...
void HandleException(int signo, siginfo_t* siginfo, void* context) {
  ProcessReader process_reader;
  ASSERT_TRUE(process_reader.Initialize(getpid()));
  ...
  ASSERT_TRUE(exception.Initialize(&process_reader,
                                   ...
                                   FromPointerCast<LinuxVMAddress>(context),
                                   gettid()));
  EXPECT_EQ(exception.Exception(), static_cast<uint32_t>(SIGALRM));
}

TEST(ExceptionSnapshotLinux, SelfHandler) {
  Signals::InstallHandler(SIGALRM,
                          HandleException,
                          0,
                          nullptr);
  EXPECT_EQ(kill(getpid(), SIGALRM), 0);
}
