...
void InitializeContext(NativeCPUContext* context) {
  ...
  reinterpret_cast<uint16_t*>(&context->ucontext.__fpregs_mem.status)[1] =
      X86_FXSR_MAGIC;
  memset(context->fxsave.xmm[7], 43, sizeof(context->fxsave.xmm[7]));
}
...
void ExpectContext(const CPUContext& actual, const NativeCPUContext& expected) {
  ...
  for (size_t index = 0; index < sizeof(actual.x86->fxsave.xmm[7]); ++index) {
    SCOPED_TRACE(base::StringPrintf("Index = %d\n", index));
    EXPECT_EQ(actual.x86->fxsave.xmm[7][index], expected.fxsave.xmm[7][index]);
  }
}
...
static constexpr uint32_t g_signo = SIGALRM;
...
void HandleTimer(int signo, siginfo_t* siginfo, void* context) {
  ...
  ASSERT_TRUE(exception.Initialize(&process_reader,
                                   ...
                                   FromPointerCast<LinuxVMAddress>(context),
                                   gettid()));
  EXPECT_EQ(exception.Exception(), static_cast<uint32_t>(g_signo));
  EXPECT_EQ(exception.Codes().size(), 3u);
