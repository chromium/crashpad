...
bool ExceptionSnapshotLinux::ReadContext<ContextTraits32>(
  ...
  do {
    ...
    switch (head.magic) {
      case VFP_MAGIC:
        if (head.size != sizeof(dest_context->vfp_regs) + sizeof(head)) {
          LOG(ERROR) << "unexpected vfp context size";
          return false;
        }
      ...
      case CRUNCH_MAGIC:
        if (head.size != sizeof(SignalCrunchContext) + sizeof(head)) {
          LOG(ERROR) << "unexpected crunch context size";
          return false;
        }
      ...
      case IWMMXT_MAGIC:
        if (head.size != sizeof(SignalIWMMXTContext) + sizeof(head)) {
          LOG(ERROR) << "unexpected iwmmxt context size";
          return false;
        }
      ...
      case DUMMY_MAGIC:
        ...
        if (head.size != sizeof(SignalIWMMXTContext) + sizeof(head)) {
          LOG(WARNING) << "unexpected dummy context size";
        }
    ...
    switch (head.magic) {
      case FPSIMD_MAGIC:
        if (head.size != sizeof(SignalFPSIMDContext) + sizeof(head)) {
          LOG(ERROR) << "unexpected fpsimd context size";
          return false;
        }
      ...
      case ESR_MAGIC:
        if (head.size != sizeof(SignalESRContext) + sizeof(head)) {
          LOG(ERROR) << "unexpected esr context size";
          return false;
        }
