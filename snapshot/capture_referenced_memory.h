...
#ifndef CRASHPAD_SNAPSHOT_CAPTURE_REFERENCED_MEMORY_H_
#define CRASHPAD_SNAPSHOT_CAPTURE_REFERENCED_MEMORY_H_
...
class MemoryCaptureDelegate {
 public:
  virtual ~MemoryCaptureDelegate() {}
...
void CaptureMemoryPointedToByContext(const CPUContext& context,
                                     MemoryCaptureDelegate* delegate);
...
//! \param[in] process_reader A MemoryCaptureProcessReader to read from the
//!     target process, and that handles adding new ranges.
void CaptureMemoryPointedToByMemoryRange(const MemorySnapshot& memory,
                                         MemoryCaptureDelegate* delegate);

}  // namespace internal
}  // namespace crashpad

#endif  // CRASHPAD_SNAPSHOT_CAPTURE_REFERENCED_MEMORY_H_
