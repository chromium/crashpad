...
StringAnnotation<32> g_test_annotation_one{"#TEST# one"};
StringAnnotation<32> g_test_annotation_two{"#TEST# two"};
StringAnnotation<32> g_test_annotation_three{"#TEST# same-name"};
StringAnnotation<32> g_test_annotation_four{"#TEST# same-name"};
...
class TestMachOImageAnnotationsReader final
 ...
 private:
  ...
  void MachMultiprocessParent() override {
    ...
    for (const auto& annotation : all_annotations) {
      ...
      std::string value(reinterpret_cast<const char*>(annotation.value.data()),
                        annotation.value.size() - 1);

      if (annotation.name == "#TEST# one") {
        EXPECT_EQ(value, "moocow\0");
  ...
  void MachMultiprocessChild() override {
    ...
    AnnotationsList::Register();  // This is “leaked” to crashpad_info.
    g_test_annotation_one.Set("moocow");
    g_test_annotation_two.Set("this will be cleared");
    g_test_annotation_three.Set("same-name 3");
    g_test_annotation_four.Set("same-name 4");
    g_test_annotation_two.Clear();
