...
TEST(ProcessTypes, DyldImagesSelf) {
  ...
  if (macos_version_number >= 101500) {
    EXPECT_GE(self_image_infos->version, 16u);
  } else if (macos_version_number >= 101200) {
    EXPECT_GE(self_image_infos->version, 15u);
  } else if (macos_version_number >= 100900) {
    EXPECT_GE(self_image_infos->version, 13u);
  } else if (macos_version_number >= 100700) {
    EXPECT_GE(self_image_infos->version, 8u);
  } else if (macos_version_number >= 100600) {
  ...
  // (at run time).
#if __MAC_OS_X_VERSION_MAX_ALLOWED >= __MAC_10_7
  if (macos_version_number >= 100700) {
  ...
  for (size_t index = 0; index < base::size(kVersionsAndSizes); ++index) {
    ...
    if (version == 15) {
      if (macos_version_number / 100 == 1012) {
        ...
        EXPECT_EQ(process_types::internal::dyld_all_image_infos<
                      ...
                      ExpectedSizeForVersion(version),
                  304u);
      } else if (macos_version_number >= 101300 &&
                 macos_version_number < 101500) {
        EXPECT_EQ(process_types::internal::dyld_all_image_infos<
                      process_types::internal::Traits32>::
                      ExpectedSizeForVersion(version),
                  176u);
  ...
  if (proctype_image_infos.version >= 15 && macos_version_number >= 101300) {
