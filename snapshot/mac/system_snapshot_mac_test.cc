...
TEST_F(SystemSnapshotMacTest, GetOperatingSystem) {
  EXPECT_EQ(
            system_snapshot().GetOperatingSystem(),SystemSnapshot::kOperatingSystemMacOSX);
}
...
TEST_F(SystemSnapshotMacTest, TimeZone) {
  ...
  for (size_t index = 0; index < arraysize(kTestTimeZones); ++index) {
    ...
    EXPECT_EQ(
              dst_status != SystemSnapshot::kDoesNotObserveDaylightSavingTime,test_time_zone.observes_dst);
    EXPECT_EQ(
              standard_offset_seconds,test_time_zone.standard_offset_hours * 60 * 60);
    EXPECT_EQ(
              daylight_offset_seconds,test_time_zone.daylight_offset_hours * 60 * 60);
