...
void ExpectSegmentCommand(const SegmentCommand* expect_segment,
  ...
  EXPECT_EQ(
            segment_name,MachOImageSegmentReader::SegmentNameString(expect_segment->segname));
  ...
  if (actual_segment->SegmentSlides()) {
    EXPECT_EQ(
              actual_segment->vmaddr() + actual_image->Slide(),actual_segment->Address());
    ...
    EXPECT_EQ(
              actual_segment->Size(),actual_segment->vmsize() + actual_image->Slide());
  }
  ...
  // structure.
  ASSERT_EQ(
            expect_segment->cmdsize,sizeof(*expect_segment) + expect_segment->nsects * sizeof(Section));
  ...
  for (size_t index = 0; index < actual_segment->nsects(); ++index) {
    ...
    if (actual_segment->SegmentSlides()) {
      EXPECT_EQ(
                actual_section->addr + actual_image->Slide(),actual_section_address);
  ...
  EXPECT_EQ(
            actual_segment->GetSectionByName("NoSuchSection", nullptr),nullptr);
}
...
void ExpectSegmentCommands(const MachHeader* expect_image,
  ...
  if (section) {
    ...
    EXPECT_EQ(
              actual_image->GetSectionByName(
                  section->segname, section->sectname, nullptr),section);
  }
  EXPECT_FALSE(
...
void ExpectMachImage(const MachHeader* expect_image,
  ...
  EXPECT_EQ(
            actual_image->Slide(),expect_image_address - actual_text_segment->vmaddr());
