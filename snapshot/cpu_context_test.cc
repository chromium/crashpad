...
TEST(CPUContextX86, FxsaveToFsave) {
  ...
  for (size_t index = 0; index < arraysize(fsave.st); ++index) {
    EXPECT_EQ(
              BytesToHexString(fsave.st[index], arraysize(fsave.st[index])),BytesToHexString(fxsave.st_mm[index].st,
  ...
  for (size_t index = 0; index < arraysize(fxsave.st_mm); ++index) {
    EXPECT_EQ(
              BytesToHexString(fxsave.st_mm[index].st,
                               arraysize(fxsave.st_mm[index].st)),BytesToHexString(fsave.st[index], arraysize(fsave.st[index])))
        << "index " << index;
    EXPECT_EQ(
              BytesToHexString(fxsave.st_mm[index].st_reserved,
                               arraysize(fxsave.st_mm[index].st_reserved)),std::string(arraysize(fxsave.st_mm[index].st_reserved) * 2, '0'))
        << "index " << index;
  }
  size_t unused_len = sizeof(fxsave) - offsetof(decltype(fxsave), xmm);
  EXPECT_EQ(
            BytesToHexString(fxsave.xmm, unused_len),std::string(unused_len * 2, '0'));
...
TEST(CPUContextX86, FxsaveToFsaveTagWord) {
  ...
  EXPECT_EQ(
            CPUContextX86::FxsaveToFsaveTagWord(fsw, fxsave_tag, st_mm),0xff22);
  ...
  EXPECT_EQ(
            CPUContextX86::FxsaveToFsaveTagWord(fsw, fxsave_tag, st_mm),0xa9ff);
  ...
  EXPECT_EQ(
            CPUContextX86::FxsaveToFsaveTagWord(fsw, fxsave_tag, st_mm),0xeebb);
  ...
  EXPECT_EQ(
            CPUContextX86::FxsaveToFsaveTagWord(fsw, fxsave_tag, st_mm),0xfe90);
  ...
  EXPECT_EQ(
            CPUContextX86::FxsaveToFsaveTagWord(fsw, fxsave_tag, st_mm),0xffff);
}
