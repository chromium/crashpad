...
bool ThreadSnapshotWin::Initialize(
  ...
  MemoryCaptureDelegateWin memory_capture_delegate(
      process_reader, thread_, &pointed_to_memory_);
  CaptureMemoryPointedToByContext(context_, &memory_capture_delegate);
  if (gather_indirectly_referenced_memory)
    CaptureMemoryPointedToByMemoryRange(stack_, &memory_capture_delegate);
