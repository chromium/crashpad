...
bool ThreadSnapshotWin::Initialize(
  ...
  if (process_reader->Is64Bit()) {
    ...
    InitializeX64Context(process_reader_thread.context.native(),
                         context_.x86_64);
    context_.x86_64->xstate.context_size = static_cast<uint32_t>(
        thread_.context.data.size() - thread_.context.offset);
  } else {
    context_.architecture = kCPUArchitectureX86;
    context_.x86 = &context_union_.x86;
  ...
  InitializeARM64Context(process_reader_thread.context.native(),
                         context_.arm64);
#else
