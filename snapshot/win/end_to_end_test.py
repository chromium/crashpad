...
def RunTests(cdb_path,
  ...
  out = CdbRun(cdb_path, sigfpe_path, '.ecxr')
  out.Check('code c00002b5', 'got sigfpe signal')
  out.Check('CrashySignalMain', '  stack in expected location')

  out = CdbRun(cdb_path, sigabrt_path, '.ecxr')
  out.Check('code 00000016', 'got sigabrt signal')
  out.Check('::HandleSIG', '  stack in expected location')
...
def main(args):
  try:
    ...
    sigfpe_path = GetDumpFromSignal(args[0], pipe_name, 'SIGFPE')
    if not sigfpe_path:
      return 1

    sigabrt_path = GetDumpFromSignal(args[0], pipe_name, 'SIGABRT')
    if not sigabrt_path:
      return 1
