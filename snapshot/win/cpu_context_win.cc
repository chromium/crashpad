...
#undef ASSERT_WOW64_EQUIVALENT
  ...
  if (HasContextPart(context, WOW64_CONTEXT_EXTENDED_REGISTERS)) {
    ...
    // The unused high bits in the |context| version of ControlWord, StatusWord,
    // TagWord, ErrorSelector, and DataSelector are sometimes set. They’re
    // meaningless, so mask them out with a static_cast.
