...
void InitializeX64Context(const CONTEXT* context, CPUContextX86_64* out) {
  ...
  if (HasContextPart(context, CONTEXT_XSTATE)) {
    GetXStateFeaturesMask(const_cast<CONTEXT*>(context),
                          &(out->xstate.enabled_features));
    if (out->xstate.enabled_features & XSTATE_MASK_CET_U) {
      DWORD cet_u_size = 0;
      XSAVE_CET_U_FORMAT* cet_u =
          reinterpret_cast<XSAVE_CET_U_FORMAT*>(LocateXStateFeature(
              const_cast<CONTEXT*>(context), XSTATE_CET_U, &cet_u_size));
      out->cet_u.offset =
          static_cast<uint32_t>(reinterpret_cast<const char*>(cet_u) -
                                reinterpret_cast<const char*>(context));
      out->cet_u.size = cet_u_size;
      out->cet_u.cetmsr = cet_u->Ia32CetUMsr;
      out->cet_u.ssp = cet_u->Ia32Pl3SspMsr;
    }
  }
}
