 ...
 public:
  ...
  class NoteReader {
   public:
    ...
    Result NextNote(std::string* name, uint32_t* type, std::string* desc);
   ...
   private:
    enum class InternalResult { kError, kSuccess, kRetry };
    ...
    template <typename T>
    InternalResult ReadNote(std::string* name,
                            uint32_t* type,
                            std::string* desc);
