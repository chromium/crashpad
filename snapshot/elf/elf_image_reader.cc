...
ElfImageReader::NoteReader::Result ElfImageReader::NoteReader::NextNote(
  ...
  do {
    while (current_segment_address_ == segment_end_address_) {
      VMSize segment_size;
...
ElfImageReader::NoteReader::Result ElfImageReader::NoteReader::ReadNote(
    ...
    uint64_t* type,
    std::string* desc) {
  DCHECK_LT(current_segment_address_, segment_end_address_);
...
#define PAD(x) ((x) + align - 1 & ~(align - 1))
  ...
  LOG(INFO) << "Checking note at address 0x" << std::hex
            << current_segment_address_;
  // Notes typically have 4-byte alignment. However .note.android.ident uses
  // 2-byte alignment. If this note was aligned at less than 4-bytes, expect
  // that the next note will be aligned at 4-bytes add extra padding, if
  // necessary.
  VMAddress end_of_note =
      std::min(PAD(current_segment_address_ + note_size), segment_end_address_);

  if (max_note_size_ >= 0 && note_size > static_cast<size_t>(max_note_size_)) {
    current_segment_address_ = end_of_note;
...
ElfImageReader::NoteReader ElfImageReader::NotesWithType(
