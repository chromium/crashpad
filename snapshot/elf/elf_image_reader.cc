  ...
  bool VerifyLoadSegments() const override {
    ...
    for (const auto& header : table_) {
      if (header.p_type == PT_LOAD) {
        CheckedVMAddressRange load_range(is_64_bit, header.p_vaddr,
                                         header.p_memsz);
...
#define VERIFY_HEADER(header)                                  \
  ...
  if (!range.ContainsRange(CheckedVMAddressRange(memory.Is64Bit(), phdr_address,
                                                 program_headers_->Size()))) {
