...
bool IsValidTriState(uint8_t value) {
  switch (value) {
  ...
  LOG(ERROR) << "invalid tristate " << value;
  return false;
}
...
class CrashpadInfoReader::InfoContainerSpecific : public InfoContainer {
 public:
  ...
  bool Read(const ProcessMemoryRange* memory, VMAddress address) override {
    ...
    if (!IsValidTriState(
            static_cast<uint8_t>(info.crashpad_handler_behavior)) ||
        !IsValidTriState(
            static_cast<uint8_t>(info.system_crash_reporter_forwarding)) ||
        !IsValidTriState(
            static_cast<uint8_t>(info.gather_indirectly_referenced_memory))) {
      return false;
    }
...
#endif
  static_assert(sizeof(InfoContainerSpecific<NATIVE_TRAITS>::info) ==
                    sizeof(CrashpadInfo),
