...
class ProcessSnapshotIOSIntermediateDumpTest : public testing::Test {
 protected:
  ...
  void WriteProcessInfo(IOSIntermediateDumpWriter* writer) {
    ...
    timeval timev;
    EXPECT_TRUE(writer->AddProperty(Key::kStartTime, &timev));

    time_value_t time_v_t;
    {
      IOSIntermediateDumpWriter::ScopedMap taskInfo(writer,
                                                    Key::kTaskBasicInfo);
      EXPECT_TRUE(writer->AddProperty(Key::kUserTime, &time_v_t));
      EXPECT_TRUE(writer->AddProperty(Key::kSystemTime, &time_v_t));
    }
    {
      IOSIntermediateDumpWriter::ScopedMap taskThreadTimesMap(
          writer, Key::kTaskThreadTimes);

      writer->AddProperty(Key::kUserTime, &time_v_t);
      writer->AddProperty(Key::kSystemTime, &time_v_t);
    }
  ...
  void WriteSystemInfo(IOSIntermediateDumpWriter* writer) {
    ...
    EXPECT_TRUE(writer->AddProperty(Key::kCpuVendor, cpu_vendor.c_str()));
  ...
  void WriteThreads(IOSIntermediateDumpWriter* writer) {
    ...
    for (uint64_t thread_id = 1; thread_id < 10; thread_id++) {
      ...
      writer->AddProperty(Key::kDebugState, &debug_state);
#elif defined(ARCH_CPU_ARM64)
      ...
      const vm_address_t stack_region_address = 0;
      EXPECT_TRUE(
