...
bool ModuleSnapshotIOSIntermediateDump::Initialize(
  ...
  // These keys is often missing.
  GetDataValueFromMapIfExists(image_data, Key::kTimestamp, &timestamp_);
  GetDataValueFromMapIfExists(
      image_data, Key::kDylibCurrentVersion, &dylib_version_);

  const auto uuid_dump = GetDataFromMap(image_data, IntermediateDumpKey::kUUID);
  if (uuid_dump) {
    unsigned char const* bytes = uuid_dump->bytes();
    if (!bytes || uuid_dump->length() != 16) {
  ...
  if (annotation_list) {
    for (auto& annotation : *annotation_list) {
      ...
      if (type_dump && value_dump && type_dump->GetValue<uint16_t>(&type)) {
        const uint8_t* value = value_dump->bytes();
        uint64_t length = value_dump->length();
  ...
  const auto crash_info_dump =
      image_data->GetAsMap(IntermediateDumpKey::kAnnotationsCrashInfo);
  if (crash_info_dump) {
    const auto message1_dump = crash_info_dump->GetAsData(
    ...
    const auto message2_dump = crash_info_dump->GetAsData(
