...
void MemorySnapshotIOS::Initialize(vm_address_t address, vm_size_t size) {
  ...
  // TODO(justincohen): Note that this design is temporary, as MemorySnapshotIOS
  // will likely be able to point directly to the deserialized data dump rather
  // than copying data around.
  buffer_ = std::unique_ptr<uint8_t[]>(new uint8_t[size_]);
  memcpy(buffer_.get(), (void*)address_, size_);
