...
bool ThreadSnapshotIOSIntermediateDump::Initialize(
  ...
  const auto nsexception_frames =
      thread_data->GetAsData(Key::kThreadUncaughtNSExceptionFrames);
  const auto thread_data_dump = thread_data->GetAsData(Key::kStackRegionData);
  if (nsexception_frames && thread_data_dump) {
  ...
  if (thread_data_dump) {
    ...
    auto bytes = thread_data_dump->bytes();
    const vm_address_t stack_region_data =
        reinterpret_cast<vm_address_t>(bytes.data());
    vm_size_t stack_region_size = bytes.size();
    stack_.Initialize(
        stack_region_address, stack_region_data, stack_region_size);

    GetDataValueFromMap(thread_data, Key::kThreadState, &thread_state);
    ...
    auto bytes = nsexception_frames->bytes();
...
#error Port to your CPU architecture
  ...
  if (thread_context_memory_regions) {
    for (auto& region : *thread_context_memory_regions) {
      ...
      const auto region_data =
          GetDataFromMap(region.get(), Key::kThreadContextMemoryRegionData);
      if (!region_data)
        continue;
      if (GetDataValueFromMap(
              region.get(), Key::kThreadContextMemoryRegionAddress, &address)) {
        auto bytes = region_data->bytes();
        vm_size_t data_size = bytes.size();
        if (data_size == 0)
          continue;

        const vm_address_t data = reinterpret_cast<vm_address_t>(bytes.data());
