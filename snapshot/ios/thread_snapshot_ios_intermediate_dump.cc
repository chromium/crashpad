...
bool ThreadSnapshotIOSIntermediateDump::Initialize(
  ...
  if (thread_data->HasKey(
    ...
    const auto frame_data =
        GetDataFromMap(thread_data, Key::kThreadUncaughtNSExceptionFrames);
    if (frame_data) {
      const uint64_t* frames =
          reinterpret_cast<const uint64_t*>(frame_data->bytes());
    ...
    auto thread_data_dump = GetDataFromMap(thread_data, Key::kStackRegionData);
    if (thread_data_dump) {
      const vm_address_t stack_region_data =
          reinterpret_cast<vm_address_t>(thread_data_dump->bytes());
      vm_size_t stack_region_size = thread_data_dump->length();
      stack_.Initialize(
...
#error Port to your CPU architecture
  ...
  for (auto& region : *thread_context_memory_regions) {
    ...
    if (GetDataValueFromMap(
      ...
      vm_size_t data_size = region_data->length();
      if (data_size == 0)
        continue;

      const vm_address_t data =
          reinterpret_cast<vm_address_t>(region_data->bytes());
