...
bool ProcessSnapshotIOSIntermediateDump::Initialize(
  ...
  const auto root_map = reader_.RootMap();
  if (root_map->empty())
    return false;
  ...
  const auto basic_info = process_info->GetAsMap(Key::kTaskBasicInfo);
  if (basic_info) {
  ...
  const auto thread_times = process_info->GetAsMap(Key::kTaskThreadTimes);
  if (thread_times) {
  ...
  auto system_info = GetMapFromMap(root_map, Key::kSystemInfo);
  if (!system_info)
    return false;
  system_.Initialize(system_info);

  // Threads
  const auto thread_list = GetListFromMap(root_map, Key::kThreads);
  if (thread_list) {
  ...
  const auto module_list = GetListFromMap(root_map, Key::kModules);
  if (module_list) {
  ...
  // Exceptions
  if (root_map->HasKey(Key::kSignalException)) {
    exception_.reset(new internal::ExceptionSnapshotIOSIntermediateDump());
    if (!exception_->InitializeFromSignal(
            GetMapFromMap(root_map, Key::kSignalException))) {
      return false;
    }
  } else if (root_map->HasKey(Key::kMachException)) {
    exception_.reset(new internal::ExceptionSnapshotIOSIntermediateDump());
    if (!exception_->InitializeFromMachException(
            GetMapFromMap(root_map, Key::kMachException),
            GetListFromMap(root_map, Key::kThreads))) {
      return false;
    }
  } else if (root_map->HasKey(Key::kNSException)) {
    exception_.reset(new internal::ExceptionSnapshotIOSIntermediateDump());
    if (!exception_->InitializeFromNSException(
            GetMapFromMap(root_map, Key::kNSException),
            GetListFromMap(root_map, Key::kThreads))) {
      return false;
    }
  }
