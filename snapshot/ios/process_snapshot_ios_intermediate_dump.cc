...
bool ProcessSnapshotIOSIntermediateDump::Initialize(
  ...
  // Exceptions
  auto signal_exception = GetMapFromMap(root_map, Key::kSignalException);
  auto mach_exception = GetMapFromMap(root_map, Key::kMachException);
  auto ns_exception = GetMapFromMap(root_map, Key::kNSException);
  if (signal_exception) {
