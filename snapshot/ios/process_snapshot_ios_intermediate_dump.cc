...
bool ProcessSnapshotIOSIntermediateDump::Initialize(
  ...
  // Exceptions
  auto signal_exception = root_map->GetAsMap(Key::kSignalException);
  auto mach_exception = root_map->GetAsMap(Key::kMachException);
  auto ns_exception = root_map->GetAsMap(Key::kNSException);
  if (signal_exception) {
