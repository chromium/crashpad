...
#error Port to your CPU architecture
  ...
  GetDataValueFromMap(exception_data, Key::kThreadID, &thread_id_);
...
bool ExceptionSnapshotIOSIntermediateDump::InitializeFromMachException(
  ...
  auto code_dump = GetDataFromMap(exception_data, Key::kCode);
  if (code_dump) {
    const mach_exception_data_type_t* code =
        reinterpret_cast<const mach_exception_data_type_t*>(code_dump->bytes());
    if (!code || code_dump->length() !=
...
bool ExceptionSnapshotIOSIntermediateDump::InitializeFromNSException(
  ...
  for (const auto& other_thread : *thread_list) {
    ...
    if (GetDataValueFromMap(
            other_thread.get(), Key::kThreadID, &other_thread_id)) {
      if (thread_id_ == other_thread_id) {
        if (other_thread->HasKey(Key::kThreadUncaughtNSExceptionFrames)) {
          LoadContextFromUncaughtNSExceptionFrames(other_thread.get());
        } else {
          LoadContextFromThread(exception_data, other_thread.get());
        }
        break;
...
void ExceptionSnapshotIOSIntermediateDump::LoadContextFromThread(
  ...
  if (GetDataValueFromMap(exception_data, Key::kStateCount, &state_count) &&
    ...
    auto state_dump = GetDataFromMap(exception_data, Key::kState);
    if (state_dump) {
      size_t actual_length = state_dump->length();
      if (expected_length == actual_length) {
        ConstThreadState state =
            reinterpret_cast<ConstThreadState>(state_dump->bytes());
...
#error Port to your CPU architecture
  ...
  auto frames_dump =
      GetDataFromMap(other_thread, Key::kThreadUncaughtNSExceptionFrames);
  if (frames_dump) {
    const uint64_t* frames =
        reinterpret_cast<const uint64_t*>(frames_dump->bytes());
    size_t num_frames = frames_dump->length() / sizeof(uint64_t);
    if (num_frames <= 2) {
      return;
    }
