...
bool ExceptionSnapshotIOSIntermediatedump::InitializeFromSignal(
  ...
  exception_data[IntermediateDumpKey::kThreadState]
      .AsData()
      .GetValue<thread_state_type>(&thread_state);
  float_state_type float_state;
  exception_data[IntermediateDumpKey::kFloatState]
      .AsData()
      .GetValue<float_state_type>(&float_state);
...
#error Port to your CPU architecture
  ...
  exception_data[IntermediateDumpKey::kThreadID].AsData().GetValue<uint64_t>(
      &thread_id_);
  exception_data[IntermediateDumpKey::kSignalNumber]
      .AsData()
      .GetValue<uint32_t>(&exception_);
  exception_data[IntermediateDumpKey::kSignalCode].AsData().GetValue<uint32_t>(
      &exception_info_);
  exception_data[IntermediateDumpKey::kSignalAddress]
      .AsData()
      .GetValue<uintptr_t>(&exception_address_);
...
bool ExceptionSnapshotIOSIntermediatedump::InitializeFromMachException(
  ...
  exception_data[IntermediateDumpKey::kException]
      .AsData()
      .GetValue<exception_type_t>(&exception);
  ...
  exception_data[IntermediateDumpKey::kCodeCount]
      .AsData()
      .GetValue<mach_msg_type_number_t>(&code_count);
  ...
  for (auto& other_thread : thread_list) {
    uint64_t other_thread_id;
    (*other_thread)[IntermediateDumpKey::kThreadID].AsData().GetValue<uint64_t>(
        &other_thread_id);
    if (thread_id_ == other_thread_id) {
      LoadContextFromThread(exception_data, *other_thread);
    }
  }
...
bool ExceptionSnapshotIOSIntermediatedump::InitializeFromNSException(
  ...
  for (auto& other_thread : thread_list) {
    ...
    (*other_thread)[IntermediateDumpKey::kThreadID].AsData().GetValue<uint64_t>(
        &other_thread_id);
    if (thread_id_ == other_thread_id) {
      if (other_thread->HasKey(
              IntermediateDumpKey::kThreadUncaughtNSExceptionFrames)) {
        LoadContextFromUncaughtNSExceptionFrames(*other_thread);
      } else {
        LoadContextFromThread(exception_data, *other_thread);
      }
      break;
...
void ExceptionSnapshotIOSIntermediatedump::LoadContextFromThread(
  ...
  other_thread[IntermediateDumpKey::kThreadState]
      .AsData()
      .GetValue<thread_state_type>(&thread_state);
  other_thread[IntermediateDumpKey::kFloatState]
      .AsData()
      .GetValue<float_state_type>(&float_state);
  other_thread[IntermediateDumpKey::kDebugState]
      .AsData()
      .GetValue<debug_state_type>(&debug_state);

  thread_state_flavor_t flavor = THREAD_STATE_NONE;
  exception_data[IntermediateDumpKey::kFlavor]
      .AsData()
      .GetValue<thread_state_flavor_t>(&flavor);
  ...
  if (expected_length == actual_length) {
    ConstThreadState state = reinterpret_cast<ConstThreadState>(
        exception_data[IntermediateDumpKey::kState].bytes());
    mach_msg_type_number_t state_count = 0;
    exception_data[IntermediateDumpKey::kStateCount]
        .AsData()
        .GetValue<mach_msg_type_number_t>(&state_count);
...
#error Port to your CPU architecture
    ...
    // Yuck, this is all wrong.  Need to validate this better.
#if defined(ARCH_CPU_X86_64)
    context_.architecture = kCPUArchitectureX86_64;
    context_.x86_64 = &context_x86_64_;
#elif defined(ARCH_CPU_ARM64)
...
#error Port to your CPU architecture
  ...
  uint64_t* frames =
      (uint64_t*)
          other_thread[IntermediateDumpKey::kThreadUncaughtNSExceptionFrames]
              .bytes();
  size_t num_frames =
      other_thread[IntermediateDumpKey::kThreadUncaughtNSExceptionFrames]
          .AsData()
...
#if defined(ARCH_CPU_X86_64)
  // TODO(justincohen): There's no associated thread stack for x86_64 yet.
  context_x86_64_ = {};
