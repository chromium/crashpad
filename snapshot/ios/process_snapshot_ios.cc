...
bool ProcessSnapshotIOS::Initialize(
  ...
  uint8_t version;
  root_map["version"].AsData().GetData<uint8_t>(&version);
  if (version != 1)
    return false;

  const internal::IOSMinidumpMap& process_info =
      root_map["ProcessInfo"].AsMap();
  ...
  system_.Initialize(root_map["SystemInfo"].AsMap());

  // Threads
  const auto& thread_list = root_map["Threads"].AsList();
  ...
  const auto& module_list = root_map["Modules"].AsList();
  ...
  // Exceptions
  if (root_map.HasKey("SignalException")) {
    exception_.reset(new internal::ExceptionSnapshotIOS());
    exception_->InitializeFromSignal(root_map["SignalException"].AsMap());
  } else if (root_map.HasKey("MachException")) {
    exception_.reset(new internal::ExceptionSnapshotIOS());
    exception_->InitializeFromMachException(root_map["MachException"].AsMap(),
                                            root_map["Threads"].AsList());
  }
