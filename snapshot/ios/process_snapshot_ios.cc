...
bool ProcessSnapshotIOS::Initialize(const base::FilePath& dump_path) {
  INITIALIZATION_STATE_SET_INITIALIZING(initialized_);

  fd_ = open(dump_path.value().c_str(), O_RDONLY);
  struct stat filestat;
  fstat(fd_, &filestat);
  if (filestat.st_size == 0) {
    return false;
  }
  mapping_.ResetMmap(NULL, filestat.st_size, PROT_READ, MAP_SHARED, fd_, 0);
