...
bool ProcessSnapshotIOS::Initialize(const IOSSystemData* system_data) {
  INITIALIZATION_STATE_SET_INITIALIZING(initialized_);

  int mib[] = {CTL_KERN, KERN_PROC, KERN_PROC_PID, getpid()};
  size_t len = sizeof(kern_proc_info_);
  sysctl(mib, sizeof(mib), &kern_proc_info_, &len, nullptr, 0);
  ...
  system_.Initialize(&snapshot_time_, system_data);
