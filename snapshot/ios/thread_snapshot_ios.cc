...
void InitializeCPUContext(CPUContextType* type_context,
  ...
  type_context->dr0 = debug_state->__dr0;
  type_context->dr1 = debug_state->__dr1;
  type_context->dr2 = debug_state->__dr2;
  type_context->dr3 = debug_state->__dr3;
  type_context->dr4 = debug_state->__dr4;
  type_context->dr5 = debug_state->__dr5;
  type_context->dr6 = debug_state->__dr6;
  type_context->dr7 = debug_state->__dr7;
#endif
}
...
bool ThreadSnapshotIOS::Initialize(thread_t thread) {
  ...
  // TODO(justincohen): Move the get- and info- calls to fill the following
  // seven variables to the dump-on-read section.
  ThreadStateType thread_state;
  FloatStateType float_state;
  DebugStateType debug_state;
  ...
  kern_return_t kr =
      thread_get_state(thread,
                       ...
                       &thread_state_count);
  if (kr != KERN_SUCCESS) {
    MACH_LOG(ERROR, kr) << "thread_state_flavor";
  }
  ...
  kr = thread_get_state(thread,
                        ...
                        &float_state_count);
  if (kr != KERN_SUCCESS) {
    MACH_LOG(ERROR, kr) << "float_state_flavor";
  }
  ...
  kr = thread_get_state(thread,
                        ...
                        &debug_state_count);
  if (kr != KERN_SUCCESS) {
    MACH_LOG(ERROR, kr) << "debug_state_flavor";
  }
  ...
  kr = thread_info(thread,
                   ...
                   &count);
  if (kr != KERN_SUCCESS) {
    MACH_LOG(ERROR, kr) << "thread_basic_info";
  }
  ...
  kr = thread_policy_get(thread,
                         ...
                         &get_default);
  if (kr != KERN_SUCCESS) {
    MACH_LOG(ERROR, kr) << "thread_precedence_policy";
  }
  ...
  InitializeCPUContext(
      &type_context_, &thread_state, &float_state, &debug_state);
#if defined(ARCH_CPU_ARM64)
