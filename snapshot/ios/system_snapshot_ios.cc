...
void SystemSnapshotIOS::Initialize(const IOSSystemDataCollector& system_data) {
  ...
  kern_return_t kr = host_statistics(
      mach_host_self(), HOST_VM_INFO, (host_info_t)&vm_stat, &host_size);
  if (kr != KERN_SUCCESS) {
...
uint32_t SystemSnapshotIOS::CPURevision() const {
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
  // sysctlbyname machdep.cpu.* returns -1 on iOS/ARM64, but consider recording
  // this for X86_64 only.
  return 0;
}
...
void SystemSnapshotIOS::CPUFrequency(uint64_t* current_hz,
  ...
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
  // sysctlbyname hw.cpufrequency returns -1 on iOS/ARM64, but consider
  // recording this for X86_64 only.
  *current_hz = 0;
  *max_hz = 0;
}
