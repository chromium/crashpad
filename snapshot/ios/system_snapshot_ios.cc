...
void SystemSnapshotIOS::Initialize(const IOSSystemDataCollector& system_data) {
  ...
  os_version_major_ = system_data.MajorVersion();
  os_version_minor_ = system_data.MinorVersion();
  os_version_bugfix_ = system_data.PatchVersion();
  os_version_build_ = system_data.Build();
  machine_ = system_data.Machine();
  cpu_count_ = system_data.ProcessorCount();
  cpu_vendor_ = system_data.CPUVendor();
  if (system_data.HasDaylightSavingTime()) {
    if (system_data.IsDaylightSavingTime()) {
...
uint32_t SystemSnapshotIOS::CPURevision() const {
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
  // iOS/ARM64 does not expose this, but this could be recorded for X86_64.
  return 0l;
}
