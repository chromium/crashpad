...
bool ThreadSnapshotIOSIntermediatedump::Initialize(
  ...
  thread_data[IntermediateDumpKey::kSuspendCount].AsData().GetValue<int>(
      &suspend_count_);
  thread_data[IntermediateDumpKey::kPriority].AsData().GetValue<int>(
      &priority_);
  thread_data[IntermediateDumpKey::kThreadID].AsData().GetValue<uint64_t>(
      &thread_id_);
  thread_data[IntermediateDumpKey::kThreadDataAddress]
      .AsData()
      .GetValue<uint64_t>(&thread_specific_data_address_);
  ...
  if (thread_data.HasKey(
    ...
    auto& frame_data =
        thread_data[IntermediateDumpKey::kThreadUncaughtNSExceptionFrames]
            .AsData();
    uint64_t* frames = (uint64_t*)frame_data.bytes();
    ...
    thread_data[IntermediateDumpKey::kStackRegionAddress]
        .AsData()
        .GetValue<vm_address_t>(&stack_region_address);
