...
bool ModuleSnapshotIOSIntermediatedump::Initialize(
  ...
  image_data[IntermediateDumpKey::kAddress].AsData().GetValue<uint64_t>(
      &address_);
  image_data[IntermediateDumpKey::kSize].AsData().GetValue<uint64_t>(&size_);
  image_data[IntermediateDumpKey::kTimestamp].AsData().GetValue<time_t>(
      &timestamp_);
  image_data[IntermediateDumpKey::kDylibCurrentVersion]
      .AsData()
      .GetValue<uint32_t>(&dylib_version_);
  image_data[IntermediateDumpKey::kSourceVersion].AsData().GetValue<uint64_t>(
      &source_version_);
  image_data[IntermediateDumpKey::kFileType].AsData().GetValue<uint32_t>(
      &filetype_);
  unsigned char const* bytes = image_data[IntermediateDumpKey::kUUID].bytes();
  if (!bytes || image_data[IntermediateDumpKey::kUUID].length() != 16) {
  ...
  const auto& annotation_list =
      image_data[IntermediateDumpKey::kAnnotationObjects].AsList();
  for (auto& annotation : annotation_list) {
    ...
    if (name.empty() || name.length() > 64) {  // Annotation::kNameMaxLength
    ...
    annotation_map[IntermediateDumpKey::kAnnotationType]
        .AsData()
        .GetValue<uint16_t>(&type);

    const uint8_t* value =
        annotation_map[IntermediateDumpKey::kAnnotationValue].bytes();
    uint64_t length =
        annotation_map[IntermediateDumpKey::kAnnotationValue].AsData().length();

    if (!value || length > 20480) {  // Annotation::kValueMaxSize
  ...
  const auto& annotations_simple_map =
      image_data[IntermediateDumpKey::kAnnotationsSimpleMap].AsList();
  for (auto& annotation : annotations_simple_map) {
    const IOSIntermediatedumpMap& annotation_map = (*annotation).AsMap();

    std::string name, value;
    annotation_map[IntermediateDumpKey::kAnnotationName].GetString(&name);
    annotation_map[IntermediateDumpKey::kAnnotationValue].GetString(&value);
    annotations_simple_map_.insert(make_pair(name, value));
  }

  const auto& crashInfo =
      image_data[IntermediateDumpKey::kAnnotationsCrashInfo];
  std::string message1;
  crashInfo[IntermediateDumpKey::kAnnotationsCrashInfoMessage1].GetString(
      &message1);
  if (!message1.empty())
    annotations_vector_.push_back(message1);
  std::string message2;
  crashInfo[IntermediateDumpKey::kAnnotationsCrashInfoMessage2].GetString(
      &message2);
  if (!message2.empty())
    annotations_vector_.push_back(message2);

  std::string dyld_error_string;
  image_data[IntermediateDumpKey::kAnnotationsDyldErrorString].GetString(
      &dyld_error_string);
