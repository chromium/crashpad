...
bool ExceptionSnapshotIOS::InitializeFromSignal(
  ...
  x86_thread_state64_t* thread_state =
      exception_data["thread_state"].AsData().GetPointer<x86_thread_state64_t>();
  x86_float_state64_t* float_state =
      exception_data["float_state"].AsData().GetPointer<x86_float_state64_t>();
  ...
  arm_thread_state64_t* thread_state =
      exception_data["thread_state"].AsData().GetPointer<arm_thread_state64_t>();
  arm_neon_state64_t* float_state =
      exception_data["float_state"].AsData().GetPointer<arm_neon_state64_t>();
...
#error Port to your CPU architecture
  ...
  thread_identifier_info* identifier_info =
      exception_data["thread_identifier_info"]
          .AsData()
          .GetPointer<thread_identifier_info>();
  thread_id_ = identifier_info->thread_id;

  siginfo_t* siginfo =
      exception_data["siginfo"].AsData().GetPointer<siginfo_t>();
...
bool ExceptionSnapshotIOS::InitializeFromMachException(
  ...
  /*
   "MachException" : {
     ...
     "state" : 8,
     "state_count" : 4,
     "thread_identifier_info" : 24,
   },
   */

  exception_type_t* exception =
      exception_data["exception"].AsData().GetPointer<exception_type_t>();
  mach_msg_type_number_t* code_count =
      exception_data["code_count"]
          .AsData()
          .GetPointer<mach_msg_type_number_t>();
  ...
  thread_identifier_info* thread_info =
      exception_data["thread_identifier_info"]
          .AsData()
          .GetPointer<thread_identifier_info>();
  thread_id_ = thread_info->thread_id;
  ...
  for (auto& value : thread_list) {
    thread_identifier_info* thread_info =
        (*value)["thread_identifier_info"]
            .AsData()
            .GetPointer<thread_identifier_info>();
    if (thread_id_ == thread_info->thread_id) {
      thread_state =
          (*value)["thread_state"].AsData().GetPointer<thread_state_type>();
      float_state =
          (*value)["float_state"].AsData().GetPointer<float_state_type>();
      debug_state =
          (*value)["debug_state"].AsData().GetPointer<debug_state_type>();
      break;
    }
  }

  thread_state_flavor_t* flavor =
      exception_data["flavor"].AsData().GetPointer<thread_state_flavor_t>();
  ConstThreadState* state =
      exception_data["state"].AsData().GetPointer<ConstThreadState>();
  mach_msg_type_number_t* state_count =
      exception_data["state_count"]
          .AsData()
          .GetPointer<mach_msg_type_number_t>();
