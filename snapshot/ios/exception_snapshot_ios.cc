...
bool ExceptionSnapshotIOS::InitializeFromSignal(
    const IOSMinidumpMap& exception_data) {
...
bool ExceptionSnapshotIOS::InitializeFromMachException(
    const IOSMinidumpMap& exception_data,
    const IOSMinidumpList& thread_list) {
  INITIALIZATION_STATE_SET_INITIALIZING(initialized_);
  ...
  size_t actual_length = exception_data["state"].AsData().length();
  if (expected_length != actual_length)
    return false;
  ConstThreadState state = reinterpret_cast<ConstThreadState>(
      exception_data["state"].AsData().data());
