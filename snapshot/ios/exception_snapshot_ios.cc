...
void ExceptionSnapshotIOS::InitializeFromMachException(
  ...
  // recovered from the same list of thread.  To avoid duplicating all the logic
  // in ThreadSnapshotIOS, just initialize a new ThreadSnapshotIOS here.
  hack_thread_snapshot_ = std::make_unique<internal::ThreadSnapshotIOS>();
  if (hack_thread_snapshot_->Initialize(exception_thread)) {
    thread_id_ = hack_thread_snapshot_->ThreadID();
    ...
    if (code_1_is_exception_address) {
      ...
      exception_address_ =
          hack_thread_snapshot_->Context()->InstructionPointer();
    }
  }
...
const CPUContext* ExceptionSnapshotIOS::Context() const {
  ...
  if (hack_thread_snapshot_) {
    return hack_thread_snapshot_->Context();
  }
  return &context_;
}
