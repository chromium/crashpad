...
mach_vm_address_t ProcessReaderIOS::DyldAllImageInfo(
  ...
  if (kr != KERN_SUCCESS) {
    MACH_LOG(WARNING, kr) << "task_for_pid";
    return 0;
  }
  kr = task_info(
...
void ProcessReaderIOS::InitializeThreads() {
  // TODO.
}
...
void ProcessReaderIOS::InitializeModules() {
  ...
  for (uint32_t i = 0; i < image_count; ++i) {
    image = image_array + i;
    printf("\nmodule %s", image->imageFilePath);
    Module module;
    module.name = image->imageFilePath;
    module.timestamp = image->imageFileModDate;
    modules_.push_back(module);

    //
    //    const struct mach_header* header = image->imageLoadAddress;  /* base
    //    address image is mapped into */ const char* path =
    //    image->imageFilePath;    /* path dyld used to load the image */
    //    uintptr_t fileMode = image->imageFileModDate;  /* time_t of image file
    //    */
    //
    //    uint32_t  magic = header->magic;    /* mach magic number identifier */
    //      cpu_type_t  cputype = header->cputype;  /* cpu specifier */
    //      cpu_subtype_t  cpusubtype = header->cpusubtype;  /* machine
    //      specifier */ uint32_t  filetype = header->filetype;  /* type of file
    //      */ uint32_t  ncmds = header->ncmds;    /* number of load commands */
    //      uint32_t  sizeofcmds = header->sizeofcmds;  /* the size of all the
    //      load commands */ uint32_t  flags = header->flags;    /* flags */
  }
