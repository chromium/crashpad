...
bool SerializedProcessSnapshotIOS::Initialize(const base::FilePath& dump_path) {
  ...
  kern_proc_info_ =
      process_info["kern_proc_info"].AsData().GetPointer<kinfo_proc>();
  task_basic_info_64* task_basic_info =
      process_info["task_basic_info"].AsData().GetPointer<task_basic_info_64>();
  if (task_basic_info) {
  ...
  task_thread_times_info_data_t* task_thread_times =
      in_process_dump["task_thread_times"]
          .AsData()
          .GetPointer<task_thread_times_info_data_t>();
  if (task_thread_times) {
    thread_times_user_time_ = task_thread_times->user_time;
    thread_times_system_time_ = task_thread_times->system_time;
  }
  snapshot_time_ = process_info["snapshot_time"].AsData().GetPointer<timeval>();
