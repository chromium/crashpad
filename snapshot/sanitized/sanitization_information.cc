...
bool ReadAllowedMemoryRanges(
  ...
  if (!memory.Read(list_address, sizeof(list), &list)) {
    LOG(ERROR) << "Failed to read memory range allowed_memory_ranges.";
    return false;
  }
  ...
  if (!memory.Read(list.entries, sizeof(ranges[0]) * list.size,
                   ranges.data())) {
    LOG(ERROR) << "Failed to read memory range allowed_memory_ranges entries.";
    return false;
  }
  ...
  for (size_t i = 0; i < list.size; i++) {
    if (ranges[i].base > vm_max || ranges[i].length > vm_max - ranges[i].base) {
      LOG(ERROR) << "Invalid memory range allowed_memory_ranges entry base="
