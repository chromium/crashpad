...
bool ReadMemoryRangeWhitelist(
  ...
  if (!memory.Read(whitelist_address, sizeof(list), &list)) {
    DLOG(ERROR) << "Failed to read memory range whitelist.";
    return false;
  }
  ...
  if (list.size > kMaxListSize) {
    DLOG(ERROR) << "Whitelist exceeded maximum, size=" << list.size;
    return false;
  }

  SanitizationMemoryRangeWhitelist::Range ranges[list.size];
  if (!memory.Read(list.entries, sizeof(ranges), &ranges)) {
    DLOG(ERROR) << "Failed to read memory range whitelist entries.";
    return false;
  }
  ...
  for (size_t i = 0; i < list.size; i++) {
    if (ranges[i].length >
        std::numeric_limits<VMAddress>::max() - ranges[i].base) {
      DLOG(ERROR) << "Invalid memory range whitelist entry base="
