...
class ProcessSnapshotSanitized final : public ProcessSnapshot {
 public:
  ...
  //! \param[in] annotations_allowlist A list of annotations names to allow to
  //!     be returned by AnnotationsSimpleMap() or from this object's module
  //!     snapshots. If `nullptr`, all annotations will be returned.
  //! \param[in] memory_range_allowlist A list of memory ranges to allow to be
  ...
  bool Initialize(
      ...
      std::unique_ptr<const std::vector<std::string>> annotations_allowlist,
      std::unique_ptr<const std::vector<std::pair<VMAddress, VMAddress>>>
          memory_range_allowlist,
 ...
 private:
  // Only used when annotations_allowlist_ != nullptr.
  ...
  std::unique_ptr<const std::vector<std::string>> annotations_allowlist_;
