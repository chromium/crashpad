...
// https://bugs.fuchsia.dev/p/fuchsia/issues/detail?id=41212. A test of stack behavior that was
// intentionally overflowing the stack, and so when Crashpad received the exception the SP did not
// point into the actual stack. This caused Crashpad to erronously capture the "stack" from the next
// mapping in the address space (which could be very large, cause OOM, etc.).
