...
CRASHPAD_CHILD_TEST_MAIN(AddressSpaceChildTestMain) {
  ...
  for (const auto& t : kTestMappingPermAndSizes) {
    ...
    zx_status_t status = zx_vmo_create(t.pages * PAGE_SIZE, 0, &vmo);
...
bool HasSingleMatchingMapping(
  ...
  for (const auto* region : memory_map) {
    ...
    if (mmi.BaseAddress == address && mmi.RegionSize == size) {
      if (matching) {
        LOG(ERROR) << "multiple mappings matching address and size";
        return false;
      }
      matching = region;
    }
  ...
  return matching->AsMinidumpMemoryInfo().Protect == perm;
}
