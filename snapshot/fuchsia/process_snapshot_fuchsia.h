...
class ProcessSnapshotFuchsia : public ProcessSnapshot {
 public:
  ...
  //! Initialize().
  //!
  //! \param[in] thread_handle Handle to the thread which sustained the
  //!     exception. This handle is the same thread as that identified by \a
  //!     thread_id.
  ...
  //!     unchanged.
  bool InitializeException(zx_handle_t thread_handle, zx_koid_t thread_id);
