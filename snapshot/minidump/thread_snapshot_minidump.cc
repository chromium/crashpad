...
bool ThreadSnapshotMinidump::InitializeContext(
  ...
  if (context_.architecture == CPUArchitecture::kCPUArchitectureX86) {
    ...
    context_.x86->cs = src->cs;
    context_.x86->ds = src->ds;
    context_.x86->es = src->es;
    context_.x86->fs = src->fs;
    context_.x86->gs = src->gs;
    context_.x86->ss = src->ss;
    ...
    context_.x86_64 = reinterpret_cast<CPUContextX86_64*>(context_memory_.data());
    ...
    context_.arm->have_vfp_regs = !!(src->context_flags & kMinidumpContextARMVFP);
    ...
    context_.mipsel->mdhi = src->mdhi;
    context_.mipsel->mdlo = src->mdlo;
    ...
    context_.mipsel->cp0_epc = src->epc;
    context_.mipsel->cp0_badvaddr = src->badvaddr;
    ...
    std::memcpy(&context_.mipsel->fpregs, &src->fpregs, sizeof(src->fpregs));
  } else if (context_.architecture == CPUArchitecture::kCPUArchitectureMIPS64EL) {
    ...
    std::memcpy(&context_.mips64->fpregs, &src->fpregs, sizeof(src->fpregs));
  }
