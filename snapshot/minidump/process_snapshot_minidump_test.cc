...
TEST(ProcessSnapshotMinidump, Modules) {
  ...
  CodeViewRecordElf cvg;
  cvg.signature = CodeViewRecordElf::kSignature;

  auto elfcv_loc = static_cast<RVA>(string_file.SeekGet());

  EXPECT_TRUE(string_file.Write(&cvg, offsetof(CodeViewRecordElf, build_id)));
  EXPECT_TRUE(string_file.Write("atestbuildidbecausewhynot", 25));

  auto elfcv_size = static_cast<size_t>(string_file.SeekGet() - elfcv_loc);
  ...
  for (uint32_t minidump_module_index = 0;
    ...
    if (minidump_module_index % 2) {
      ...
      minidump_module.CvRecord.Rva = elfcv_loc;
      minidump_module.CvRecord.DataSize = elfcv_size;
    }
...
TEST(ProcessSnapshotMinidump, System) {
  ...
  base::c16memcpy(
      minidump_misc_info.BuildString, build_string.data(), build_string.size());
