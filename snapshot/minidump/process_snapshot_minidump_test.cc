...
TEST(ProcessSnapshotMinidump, CustomMinidumpStreams) {
  ...
  EXPECT_EQ(custom_streams[0]->memory()->Size(), sizeof(kStreamUnreservedData));

  ReadToVector delegate;
  EXPECT_TRUE(custom_streams[0]->memory()->Read(&delegate));
  EXPECT_STREQ((char*)&delegate.result.front(), kStreamUnreservedData);
}
