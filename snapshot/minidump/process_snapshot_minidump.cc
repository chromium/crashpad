...
bool ProcessSnapshotMinidump::InitializeCrashpadInfo() {
  ...
  if (stream_it->second->DataSize == crashpad_info_size - 8) {
    crashpad_info_size -= 8;
  }

  if (stream_it->second->DataSize < crashpad_info_size) {
  ...
  if (!file_reader_->ReadExactly(&crashpad_info_, crashpad_info_size)) {
    return false;
  }
