...
bool ProcessSnapshotMinidump::InitializeCustomMinidumpStreams() {
  for (size_t i = 0; i < stream_directory_.size(); i++) {
    ...
    const uint32_t stream_type = stream_directory_[i].StreamType;
    if (stream_type <=
    ...
    std::vector<uint8_t> data(stream_directory_[i].Location.DataSize);
    if (!file_reader_->SeekSet(stream_directory_[i].Location.Rva) ||
