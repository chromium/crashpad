...
class ScopedSigactionRestore {
 public:
  ...
  bool ResetAndInstallHandler(int signo, void (*handler)(int, siginfo_t*, void*)) {
    Reset();
 ...
 private:
  void PrintErrmsg(int err) {
    ...
    if (strerror_r(err, errmsg, sizeof(errmsg)) != 0) {
      snprintf(
          errmsg, sizeof(errmsg), "%s:%d: Couldn't set errmsg for %d: %d", __FILE__, __LINE__, err, errno);
      return;
    }
...
bool IsKitKat() {
  ...
  if (strcmp(prop_buf, "19") == 0 ||
      strcmp(prop_buf, "20") == 0) {
    return true;
  }
  return false;
}
...
class ScopedSetTID {
 public:
  explicit ScopedSetTID(pid_t* tid) : tid_(tid) {
