...
bool IsKitKat() {
  ...
  int length = __system_property_get("ro.build.version.release", prop_buf);
  if (length <= 0) {
    fprintf(stderr, "Couldn't get version");
    return true;
  }
  constexpr char kitkat_prefix[] = "4.4";
  if (strncmp(prop_buf, kitkat_prefix, strlen(kitkat_prefix)) == 0) {
    return true;
  }
  return false;
}
