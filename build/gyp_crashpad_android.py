...
def main(args):
  parser = argparse.ArgumentParser(description='Set up an Android cross build')
  parser.add_argument('--ndk', required=True, help='Standalone NDK toolchain')
  parser.add_argument('--build', required=True, help='Build output directory')
  ...
  return gyp_crashpad.main(
      ['-D', 'OS=android',
       ...
       '--generator-output=%s' % parsed.build,
