...
def _CheckForTun():
  ...
  """
    ...
    if returncode != 0:
        print(
            'To use QEMU with networking on Linux, configure TUN/TAP. See:',
            file=sys.stderr)
        print(
...
def _Start(pid_file):
    ...
    popen = subprocess.Popen([
        ...
        '-enable-kvm',
        '-netdev',
        'type=tap,ifname=qemu,script=no,downscript=no,id=net0',
        '-device',
        'e1000,netdev=net0,mac=52:54:00:' + mac_tail,
        '-append',
        'TERM=dumb zircon.nodename=' + instance_name,
    ],
                             stdin=DEVNULL,
                             stdout=DEVNULL,
                             stderr=DEVNULL)
