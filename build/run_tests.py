...
def _RunOnFuchsiaTarget(binary_dir, test, device_name, extra_command_line):
  ...
  try:
    ...
    namespace_command.extend(extra_command_line)
...
def main(args):
  ...
  parser.add_argument('single_test', nargs='?', help='Single test to run.')
  ...
  if args.single_test:
    if args.single_test not in tests:
      print('Unrecognized test:', args.single_test, file=sys.stderr)
      return 3
    tests = [args.single_test]
