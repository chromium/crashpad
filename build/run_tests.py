...
def _FindGNFromBinaryDir(binary_dir):
  ...
  if os.path.isfile(build_ninja):
    with open(build_ninja, 'rb') as f:
      build_ninja_contents = f.read().splitlines()
    ...
    found_rule_gn = False
    for line in build_ninja_contents:
      if line == 'rule gn':
        found_rule_gn = True
        continue
      if found_rule_gn and line.startswith('  command = '):
...
def _BinaryDirTargetOS(binary_dir):
  ...
  gn = _FindGNFromBinaryDir(binary_dir)
  ...
  if gn:
    ...
    popen = subprocess.Popen(
        ...
        cwd=os.path.join(binary_dir, os.pardir, os.pardir))
