...
def main(args):
    ...
    parser.add_argument(
        '--gtest_filter', help='GTest filter applied to GTest binary runs.')
    ...
    if is_android:
        ...
        if not android_device:
            ...
            if len(devices) != 1:
                print(
                    "Please set ANDROID_DEVICE to your device's id",
                    file=sys.stderr)
                return 2
        ...
        if not zircon_nodename:
            ...
            if popen.returncode != 0 or len(devices) != 1:
                print(
                    "Please set ZIRCON_NODENAME to your device's hostname",
                    file=sys.stderr)
                return 2
    ...
    for test in tests:
        ...
        if test.endswith('.py'):
            ...
            if is_android:
                ...
                _RunOnIOSTarget(
                    args.binary_dir, test, is_xcuitest=test.startswith('ios'))
            else:
