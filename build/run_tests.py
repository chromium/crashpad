...
def _RunOnAndroidTarget(binary_dir, test, android_device):
  ...
  def _adb_shell(command, env={}):
    ...
    # With older adb servers and clients, adb will “exit 0” even if the command
    # failed on the device. This makes subprocess.check_call() semantics
    # difficult to implement directly. As a workaround, have the device send the
    # command’s exit status over stdout and pick it back up in this function.
    #
    # Both workarounds are implemented by giving the device a simple script,
    # which adbd will run as an “sh -c” argument.
    adb_command = ['adb', '-s', android_device, 'shell', '-n']
    subcommands = []
    for k,v in env.items():
      subcommands.append('export %s=%s' % (pipes.quote(k), pipes.quote(v)))
    subcommands.extend([
        ' '.join(pipes.quote(x) for x in command),
        'status=${?}',
        'echo "status=${status}"',
        'exit ${status}'])
    subcommand_string = '; '.join(subcommands)
    adb_command.append(subcommand_string)
    child = subprocess.Popen(
        adb_command, shell=IS_WINDOWS_HOST, stdout=subprocess.PIPE)
  ...
  try:
    ...
    # this script’s own standard output connection. But don’t do this for
    # Windows, whose console doesn’t understand ANSI-style color commands.
    env = {'CRASHPAD_TEST_DATA_ROOT': device_temp_dir}
    if not IS_WINDOWS_HOST:
      gtest_color = os.environ.get('GTEST_COLOR')
      if gtest_color in (None, 'auto') and sys.stdout.isatty() and (
