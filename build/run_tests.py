...
def _RunOnAndroidTarget(binary_dir, test, android_device, extra_command_line):
    ...
    try:
        ...
        for artifact in test_build_artifacts:
            local_test_build_artifacts.append(
                os.path.join(binary_dir, artifact))
       ...
       XCTest-based test app."""
        ...
        module_data = {
            ...
            'TestingEnvironmentVariables': {
                ...
                'DYLD_LIBRARY_PATH':
                '__TESTROOT__/Debug-iphonesimulator',
                'IDEiPhoneInternalTestBundleName':
                test + '.app',
                'XCInjectBundleInto':
                '__TESTHOST__/' + test,
            }
       ...
       XCUITest-based test app."""
        ...
        module_data = {
            ...
            'TestingEnvironmentVariables': {
                ...
                '__TESTROOT__/Debug-iphonesimulator',
                'XCInjectBundleInto':
                '__TESTHOST__/' + test + '_module-Runner',
            },
        }
        return {test: module_data}
...
def main(args):
    ...
    # dynamically.
    if (sys.platform == 'win32' and args.binary_dir.endswith('_x64')
            and 'CRASHPAD_TEST_32_BIT_OUTPUT' not in os.environ):
        ...
        if not android_device:
            ...
            for line in adb_devices.splitlines():
                ...
                if (line == 'List of devices attached'
                        or re.match('^\* daemon .+ \*$', line) or line == ''):
                    continue
    ...
    for test in tests:
        ...
        if test.endswith('.py'):
            ...
            if args.gtest_filter:
                extra_command_line.append('--gtest_filter=' +
                                          args.gtest_filter)
            if is_android:
