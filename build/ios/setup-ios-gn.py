...
class ConfigParserWithStringInterpolation(ConfigParser.SafeConfigParser):
    ...
    def getstring(self, section, option):
        return self._UnquoteString(
            self._ExpandEnvVar(self.get(section, option)))
...
class GnGenerator(object):
        ...
        if build_arch == 'fat':
            ...
            args.append(
                ('additional_target_cpus',
                 [cpu for cpu in cpu_values.itervalues()
                  if cpu != target_cpu]))
        else:
    ...
    def GetGnCommand(self, gn_path, src_path, out_path,
...
def Main(args):
    ...
    if settings.getboolean('goma', 'enabled'):
        if settings.getint('xcode', 'jobs') < 0:
            ...
            sys.exit(1)
        goma_install = os.path.expanduser(settings.getstring(
            'goma', 'install'))
        if not os.path.isdir(goma_install):
            sys.stderr.write(
                'WARNING: goma.install directory not found: %s\n' %
