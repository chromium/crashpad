...
class StringAnnotation : public testing::Test {
 public:
  ...
  // them, they are declared as part of the test. Public so they are accessible
  // from global helpers.
...
TEST_F(StringAnnotation, SetAndClear) {
  ...
  EXPECT_EQ(6u, one_.size());
  EXPECT_EQ(256u, two_.size());

  EXPECT_TRUE(ContainsNameValue(annotations, "First", "short"));
  EXPECT_TRUE(ContainsNameValue(annotations, "Second", std::string(255, 'A')));
}
