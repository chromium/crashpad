...
class CrashHandler : public Thread, public UniversalMachExcServer::Interface {
 public:
  ...
  void DumpWithoutCrash() {
    // TODO(justincohen): This should take a CPUContext once CaptureContext is
    // implemented for ARM64.  For now use HandleException with
    // mach_thread_self.
    mach_exception_data_type_t code[2] = {};
    HandleException(
        0, mach_thread_self(), Signals::kSimulatedSigno, code, 0, 0, 0, 0);
  }
  ...
  bool Install() {
    ...
    if (!InstallMachExceptionHandler()) {
      if (!sigaction(SIGABRT, &old_action_, nullptr))
        LOG(ERROR) << "Could not uninstall handler";
      return false;
    }
  ...
  kern_return_t CatchMachException(exception_behavior_t behavior,
    ...
    *destroy_complex_request = true;
    ...
    if (task != mach_task_self()) {
      LOG(ERROR) << "task 0x" << std::hex << task << " != 0x"
                 << mach_task_self();
      return KERN_FAILURE;
    }
