...
class CrashHandler : public Thread,
 ...
 public:
  ...
  void DumpWithoutCrash(NativeCPUContext* context, bool process = false) {
    ...
    base::FilePath file;
    if (in_process_handler_.UseAlternateMachExceptionPath(&file)) {
      DumpWithContext(context);
      if (process) {
        in_process_handler_.ProcessIntermediateDump(file);
      }
    }
  }
  ...
  void DumpWithoutCrashAtPath(NativeCPUContext* context,
    ...
    if (in_process_handler_.UseAlternateMachExceptionPathAtPath(path))
      DumpWithContext(context);
  }
