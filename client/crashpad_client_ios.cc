...
class CrashHandler : public Thread, public UniversalMachExcServer::Interface {
 public:
  ...
  void test_only_dump(SerializedProcessSnapshotIOS& process_snapshot) {
  ...
  void ProcessPendingDumps() {
    ...
    for (auto& file : files) {
      SerializedProcessSnapshotIOS process_snapshot;
      if (process_snapshot.Initialize(file)) {
//        test_only_dump(process_snapshot);
      ...
      const PackedMap in_process_dump = PackedMap::Parse(mapping.addr_as<uint8_t*>(), mapping.len());
      in_process_dump.DumpJson();
//      const PackedData foo =
//          in_process_dump["ProcessInfo"]["kern_proc_info"].AsData();
...
//      const auto& parentList = in_process_dump["Modules"].AsList();
//      for(auto& value : parentList) {
//        const PackedData data = (*value)["name"].AsData();
//        DLOG(WARNING) << std::string((char*)data.data(), data.length());
//      }
//      
//      const PackedData data = in_process_dump["SystemInfo"]["machine_description"].AsData();
//      DLOG(WARNING) << std::string((char*)data.data(), data.length());
      close(fd);
