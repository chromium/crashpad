...
class ScopedEnvironmentVariable {
 public:
  ...
  void SetValue(const wchar_t* new_value);
...
ScopedEnvironmentVariable::~ScopedEnvironmentVariable() {
  SetValue(original_value_.size() ? original_value_.data() : L"");
}

std::wstring ScopedEnvironmentVariable::GetValue() const {
  // The length returned is inclusive the terminating zero, except
  // if the variable doesn't exists, in which case the return value is zero.
  ...
  std::wstring ret;
  ret.resize(len);
  // The length returned on succes is exlusive the terminating zero.
...
void ScopedEnvironmentVariable::SetValue(const wchar_t* new_value) {
...
void WaitForAllChildProcessesOf(HANDLE parent) {
  ...
  if (!parent_info.Initialize(parent)) {
    LOG(ERROR) << "Initialize(parent)";
    return;
  }

  std::vector<pid_t> child_pids =
      GetPotentialChildProcessesOf(parent_info.ProcessID());
  ...
  for (pid_t child_pid : child_pids) {
    ...
    if (!child_process.is_valid()) {
      PLOG(WARNING) << "OpenProcess pid " << child_pid;
      continue;
    }
    ...
    if (!child_info.Initialize(child_process.get())) {
      LOG(ERROR) << "Initialize(child_process)";
      continue;
    }

    if (parent_info.ProcessID() != child_info.ParentProcessID()) {
    ...
    if (err != WAIT_OBJECT_0)
      LOG(ERROR) << "WaitForSingleObject returned " << err;
  }
}
