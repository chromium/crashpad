...
CrashReportDatabase::NewReport::NewReport()
    : writer_(), uuid_(), file_remover_() {}

CrashReportDatabase::NewReport::~NewReport() {}

CrashReportDatabase::ReadReport::ReadReport()
    : Report(), reader_(), lock_file_() {}

CrashReportDatabase::ReadReport::~ReadReport() {}

CrashReportDatabase::UploadReport::UploadReport()
    : Report(), reader_(), lock_file_(), database_() {}
...
OperationStatus CrashReportDatabase::PrepareNewCrashReport(
  ...
  const base::FilePath path = ReportPath(base_dir_, kNew, new_report->uuid_);
...
OperationStatus CrashReportDatabase::FinishedWritingCrashReport(
  ...
  base::FilePath path = ReportPath(base_dir_, kPending, report->uuid_);
  ...
  base::FilePath unused(report->file_remover_.release());
...
OperationStatus CrashReportDatabase::SkipReportUpload(
  ...
  base::FilePath completed_path(ReportPath(base_dir_, kCompleted, uuid));
...
OperationStatus CrashReportDatabase::RequestUpload(const UUID& uuid) {
  ...
  base::FilePath pending_path = ReportPath(base_dir_, kPending, uuid);
...
OperationStatus CrashReportDatabase::RecordUploadAttempt(
  ...
  if (successful) {
    ...
    base::FilePath completed_report_path =
