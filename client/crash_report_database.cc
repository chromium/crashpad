...
OperationStatus CrashReportDatabase::FinishedWritingCrashReport(
  ...
  (*report)->writer.Close();
  if (!MoveFileOrDirectory((*report)->file_.get(), path)) {
    return kFileSystemError;
  }
...
OperationStatus CrashReportDatabase::SkipReportUpload(
  ...
  if (!MoveFileOrDirectory(path, new_path)) {
    return kFileSystemError;
  }
...
OperationStatus CrashReportDatabase::RecordUploadAttempt(
  ...
  if (successful) {
    ...
    lock_file = ObtainReportLock(new_report_path);
    if (!lock_file.is_valid()) {
      return kBusyError;
    }

    report->reader.Close();
    if (!MoveFileOrDirectory(report_path, new_report_path)) {
      return kFileSystemError;
    }
