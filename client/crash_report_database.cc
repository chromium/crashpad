...
bool MoveFile(const base::FilePath& source, const base::FilePath& dest) {
  DCHECK(!source.empty());
  DCHECK(!dest.empty());
...
CrashReportDatabase::UploadReport::UploadReport() : Report(), handle() {}
...
OperationStatus CrashReportDatabase::FinishedWritingCrashReport(
  ...
  LOG(INFO) << "Finishing report " << (*report)->uuid.ToString();
  ...
  if (!MoveFile((*report)->file_.get(), path)) {
    return kFileSystemError;
  }
  LOG(INFO) << "Moved report to path " << path.value();
...
OperationStatus CrashReportDatabase::ErrorWritingCrashReport(
    std::unique_ptr<NewReport>* report) {
  report->reset();
  return kNoError;
}
...
OperationStatus CrashReportDatabase::LookUpCrashReport(const UUID& uuid,
  ...
  LOG(INFO) << "Looked up report at " << path.value();
  return kNoError;
}
...
OperationStatus CrashReportDatabase::GetReportForUploading(
    const UUID& uuid,
    std::unique_ptr<UploadReport>* report) {
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
...
OperationStatus CrashReportDatabase::RecordUploadAttempt(
  ...
  if (successful) {
    ...
    if (!MoveFile(report_path, new_report_path)) {
      return kFileSystemError;
    }
  ...
  if (!WriteMetadata(report_path, report->get())) {
    return kDatabaseError;
  }
...
OperationStatus CrashReportDatabase::RequestUpload(const UUID& uuid) {
  ...
  LOG(INFO) << "Requesting upload for report " << uuid.ToString();
  ScopedLockFile lock_file;
  base::FilePath path;
  OperationStatus os = LocateAndLockReport(uuid, &path, &lock_file);
  if (os != kNoError) {
  ...
  LOG(INFO) << "Successfully requested upload for " << uuid.ToString();
...
CrashReportDatabase::OperationStatus CrashReportDatabase::ReportsInDirectory(
  ...
  LOG(INFO) << "Looking for reports in directory " << dir_path.value();
#if defined(OS_POSIX)
  ...
  while ((dir_entry = readdir(scoped_dir.get()))) {
    LOG(INFO) << "Looking at file:" << dir_entry->d_name;
