...
OperationStatus CrashReportDatabase::LocateAndLockReport(
  ...
  if (state == kSearchable) {
    searchable_states.push_back(kPending);
    searchable_states.push_back(kCompleted);
  } else {
    DCHECK(state == kPending || state == kCompleted);
    searchable_states.push_back(state);
  }
...
OperationStatus CrashReportDatabase::CheckoutReport(const UUID& uuid,
  ...
  if (!ReadMetadata(local_path, report)) {
    // Clean up reports without metadata files
    if (!IsRegularFile(ReplaceFinalExtension(local_path, kMetadataExtension))) {
      LoggingRemoveFile(local_path);
    }
    return kDatabaseError;
  }
