...
// static
std::unique_ptr<CrashReportDatabase> CrashReportDatabase::BuildDatabase(
    const base::FilePath& path,
    bool may_create) {
#if defined(OS_MACOSX)
  using DatabaseType = CrashReportDatabaseMac;
#elif defined(OS_WIN)
  using DatabaseType = CrashReportDatabaseWin;
#else
  using DatabaseType = CrashReportDatabase;
#endif  // OS_MACOSX

  std::unique_ptr<CrashReportDatabase> db(new DatabaseType());
  return db->Initialize(path, may_create) ? std::move(db) : nullptr;
}

CrashReportDatabase::CrashReportDatabase() = default;
...
CrashReportDatabase::Report::~Report() = default;
...
CrashReportDatabase::ScopedLockFile::ScopedLockFile() = default;
...
CrashReportDatabase::ReadReport::~ReadReport() = default;
...
int CrashReportDatabase::CleanReportsInState(ReportState state,
                                             time_t lockfile_ttl) {
  const base::FilePath dir_path(base_dir_.Append(kReportDirectories[state]));
  DirectoryReader reader;
  if (!reader.Open(dir_path)) {
