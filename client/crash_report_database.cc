...
bool CrashReportDatabase::ScopedLockFile::ResetAcquire(
  ...
  if (!LoggingWriteFile(lock_fd.get(), &timestamp_, sizeof(timestamp_))) {
    return false;
  }
...
bool CrashReportDatabase::ScopedLockFile::IsExpired(
  ...
  //timespec mtime;
  //if (FileHandleModificationTime(lock_fd.get(), &mtime) &&
  //    now < mtime.tv_sec + seconds_per_day) {
  //  return false;
  //}
  ...
  if (now < timestamp + seconds_per_day) {
    return false;
  }
...
OperationStatus CrashReportDatabase::ReportsInState(
  ...
  while ((result = reader.NextFile(&filename)) ==
    ...
    if (extension.compare(kLockExtension) != 0 &&
        ScopedLockFile::IsExpired(filepath)) {
      const base::FilePath no_ext(filepath.RemoveFinalExtension());
      const base::FilePath report_path(no_ext.Append(kCrashReportExtension));
      const base::FilePath metadata_path(no_ext.Append(kMetadataExtension));
