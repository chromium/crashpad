...
OperationStatus CrashReportDatabase::GetReportForUploading(
  ...
  LOG(INFO) << "Getting lock for report " << base::UTF16ToUTF8(path.value().c_str());
...
OperationStatus CrashReportDatabase::GetReportForReading(const UUID& uuid,
  ...
  read_report->handle.reset(LoggingOpenFileForRead(path));
  if (read_report->handle.get() == kInvalidFileHandle) {
    return kFileSystemError;
  }
...
CrashReportDatabase::OperationStatus CrashReportDatabase::ReportsInDirectory(
  ...
  while (reader.NextFile(&filename)) {
    if (filename == base::FilePath(FILE_PATH_LITERAL(".")) ||
        filename == base::FilePath(FILE_PATH_LITERAL(".."))) {
      continue;
    }
    ...
    LOG(INFO) << "Found report " << base::UTF16ToUTF8(path.value());
  }
  return kNoError;
}
