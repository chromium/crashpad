...
TEST(CrashpadClient, SimulateCrash) {
  ...
  LOG(INFO) << "testing simulate crash";
  ...
  LOG(INFO) << "done simulating crash";
...
class StartHandlerAtCrashTest : public MultiprocessExec {
 public:
  StartHandlerAtCrashTest() : MultiprocessExec() {
    ...
    SetExpectedChildTermination(kTerminationSignal, SIGSEGV);
  }
...
class StartHandlerForChildTest : public Multiprocess {
 ...
 private:
  void MultiprocessParent() {
    char c;
    ASSERT_TRUE(
        LoggingReadFileExactly(test_state_.ServerSock(), &c, sizeof(c)));
