...
//!
//! Annotation objects should never be destructed. Once they are Set(), they
...
class Annotation {
 public:
  ...
  constexpr static Type UserDefinedType(uint16_t value) {
    ...
    UnderlyingType start = static_cast<UnderlyingType>(Type::kUserDefinedStart);
    value += start;
    DCHECK(value > start) << "User-defined Type is 0 or overflows";
  ...
  //!     Annotations with the same name. Names should be static, constexpr
  //!     data.
...
class StringAnnotation : public Annotation {
 public:
  ...
  void Set(const char* value) {
    ...
    SetSize(std::min(MaxSize, static_cast<ValueSizeType>(n) + 1));
  }
