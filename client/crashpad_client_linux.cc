...
class RequestCrashDumpHandler : public SignalHandler {
 ...
 private:
  ...
  // don't anything with it except pass it along) and we don't have functions
  // to deal with time_t's well, while we do have functions to deal with
  // uint64_t's.
...
void CrashpadClient::SetCrashLoopBefore(uint64_t crash_loop_before_time) {
  ...
  static_cast<RequestCrashDumpHandler*>(signal_handler)
      ->SetCrashLoopBefore(crash_loop_before_time);
}
#endif
