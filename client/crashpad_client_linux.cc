...
class SignalHandler {
 public:
  virtual void HandleCrashFatal(int signo, siginfo_t* siginfo, void* context) = 0;
  virtual void HandleCrashNonFatal(int signo, siginfo_t* siginfo, void* context) = 0;
 protected:
...
class LaunchAtCrashHandler : public SignalHandler {
 public:
  ...
  void HandleCrashNonFatal(int signo, siginfo_t* siginfo, void* context) override {
  ...
  void HandleCrashFatal(int signo, siginfo_t* siginfo, void* context) override {
    ...
    _exit(EXIT_FAILURE);  // TODO some other exit code?
  }

 private:
  LaunchAtCrashHandler() : SignalHandler() {}
...
void CrashpadClient::DumpWithoutCrash(NativeCPUContext* context) {
  ...
  memset(context->uc_mcontext.__reserved, 0, sizeof(context->uc_mcontext.__reserved));
#else
#error Port.
#endif

  siginfo_t siginfo;
  siginfo.si_signo = 0xffffffff; // TODO
  siginfo.si_errno = 0;
  siginfo.si_code = 0;
  g_crash_handler->HandleCrashNonFatal(siginfo.si_signo, &siginfo, reinterpret_cast<void*>(context));
}
