...
class SignalHandler {
 ...
 private:
  static void HandleCrash(int signo, siginfo_t* siginfo, void* context) {
    g_crash_handler->HandleCrashFatal(signo, siginfo, context);
  }
...
class RequestCrashDumpHandler : public SignalHandler {
 public:
  ...
  bool Initialize(ScopedFileHandle sock, bool set_ptracer) {
    ExceptionHandlerClient client(sock.get(), true);
    if (set_ptracer && !client.InitializePtracer()) {
  ...
  bool HandleCrashNonFatal(int signo,
    ...
    ExceptionHandlerClient client(sock_to_handler_.get(),
...
bool CrashpadClient::StartHandler(
  ...
  ScopedFileHandle handler_sock(socks[1]);

  int optval = 1;
  socklen_t optlen = sizeof(optval);
  if (setsockopt(client_sock.get(), SOL_SOCKET, SO_PASSCRED, &optval, optlen) !=
      0) {
  ...
  argv.push_back(FormatArgumentInt("initial-client-fd", socks[1]));
  argv.push_back("--shared-client-connection");
  if (!DoubleForkAndExec(argv, nullptr, handler_sock.get(), false, nullptr)) {
    LOG(ERROR) << "start handler failedd";
    return false;
  }

  auto signal_handler = RequestCrashDumpHandler::Get();
  if (signal_handler->Initialize(std::move(client_sock), true)) {
