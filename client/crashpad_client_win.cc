...
bool CrashpadClient::DumpAndCrashTargetProcess(HANDLE process,
  ...
  fprintf(stderr, "inject_memory: %llx\n", inject_memory);
  ...
  // its address here.
  WinVMAddress raise_exception_address = reinterpret_cast<WinVMAddress>(
      GetProcAddress(GetModuleHandle(L"kernel32.dll"), "RaiseException"));
  fprintf(stderr, "RaiseException: %llx\n", raise_exception_address);
  ...
  if (process_info.Is64Bit()) {
    ...
    data_to_write.push_back(0x48);
    data_to_write.push_back(0x83);
    data_to_write.push_back(0xec);
    data_to_write.push_back(0x28);
    ...
    // Call to RaiseException() via rax.
    data_to_write.push_back(0x48);  // mov rax, imm.
    data_to_write.push_back(0xb8);
    AddUint64(&data_to_write, raise_exception_address);
    data_to_write.push_back(0xff);  // call rax.
    data_to_write.push_back(0xd0);
  } else {
    // Data written is first, the data for the 4th argument (lpArguments) to
    // RaiseException(). A two element array:
    //
  ...
  if (!WriteProcessMemory(process,
                          ...
                          data_to_write.size(),
                          &bytes_written) ||
      bytes_written != data_to_write.size()) {
