...
class CrashReportDatabaseTest : public testing::Test {
 ...
 protected:
  ...
  void SetUp() override {
#if defined(OS_MACOSX)
    db_ = std::make_unique<CrashReportDatabaseMac>();
#elif defined(OS_WIN)
    db_ = std::make_unique<CrashReportDatabaseWin>();
#else
...
#if defined(OS_MACOSX)
  using CrashReportDatabaseType = CrashReportDatabaseMac;
#elif defined(OS_WIN)
  using CrashReportDatabaseType = CrashReportDatabaseWin;
#else
  using CrashReportDatabaseType = CrashReportDatabase;
#endif  // OS_MACOSX

  {
    CrashReportDatabaseType db;
