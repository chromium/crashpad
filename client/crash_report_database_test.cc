...
class CrashReportDatabaseTest : public testing::Test {
 ...
 protected:
  // testing::Test:
  void SetUp() override {
    db_ = std::make_unique<CrashReportDatabase>();
    ASSERT_TRUE(db_->Initialize(path(), /* may_create= */ true));
  }
...
TEST_F(CrashReportDatabaseTest, Initialize) {
...

  {
    ...
    ASSERT_TRUE(db.Initialize(path(), /* may_create= */ false));
  ...
  {
    CrashReportDatabase db;
    ASSERT_TRUE(db.Initialize(path(), /* may_create= */ true));
  ...
  // InitializeWithoutCreating() shouldn’t create a nonexistent database.
  {
    ...
    EXPECT_FALSE(db.Initialize(non_database_path, /* may_create= */ false));
  }
}
...
void ExpectCleanDatabase(CrashReportDatabase* db) {
  std::vector<CrashReportDatabase::Report> reports;
  EXPECT_EQ(db->GetPendingReports(&reports), CrashReportDatabase::kNoError);
...
TEST_F(CrashReportDatabaseTest, CleanBrokenDatabase) {
  ...
  const base::FilePath metadata(report.file_path.RemoveFinalExtension().Append(
  ...
  ASSERT_NO_FATAL_FAILURE(ExpectCleanDatabase(db()));
  ...
  ASSERT_NO_FATAL_FAILURE(CreateCrashReport(&report));
  const base::FilePath metadata2(report.file_path.RemoveFinalExtension().Append(
      FILE_PATH_LITERAL(".meta")));
  ASSERT_TRUE(PathExists(report.file_path));
  ...
  ASSERT_NO_FATAL_FAILURE(CreateCrashReport(&report));
  const base::FilePath metadata3(report.file_path.RemoveFinalExtension().Append(
      FILE_PATH_LITERAL(".meta")));
  ASSERT_TRUE(PathExists(report.file_path));
  ASSERT_TRUE(PathExists(metadata3));

  const base::FilePath lockpath(report.file_path.RemoveFinalExtension().Append(
  ...
  timespec expired_timestamp;
  expired_timestamp.tv_sec = time(nullptr) - 60 * 60 * 24 * 3;
  expired_timestamp.tv_nsec = 0;

  ASSERT_TRUE(LoggingWriteFile(handle.get(),
                               &expired_timestamp.tv_sec,
                               sizeof(expired_timestamp.tv_sec)));
  //ASSERT_TRUE(SetFileTimes(handle.get(), expired_timestamp));
...
#if defined(OS_WIN)
TEST(CrashReportDatabaseTest, CleanOldWindowsDatabase) {
  ScopedTempDir temp_dir;

  const base::FilePath db_path(temp_dir.path().Append(L"test_database"));
  ASSERT_TRUE(LoggingCreateDirectory(db_path, false));

  const base::FilePath reports_dir(db_path.Append(L"reports"));
  ASSERT_TRUE(LoggingCreateDrectory(reports_dir, false));
  ...
  EXPECT_FALSE(PathExists(test_report));
