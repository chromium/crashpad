...
class CrashReportDatabaseTest : public testing::Test {
 ...
 protected:
  ...
  void CreateCrashReport(CrashReportDatabase::Report* report) {
    ...
    ASSERT_EQ(
              db_->PrepareNewCrashReport(&new_report),CrashReportDatabase::kNoError);
    const char kTest[] = "test";
    ASSERT_TRUE(LoggingWriteFile(new_report->handle, kTest, sizeof(kTest)));

    UUID uuid;
    EXPECT_EQ(
              db_->FinishedWritingCrashReport(new_report, &uuid),CrashReportDatabase::kNoError);

    EXPECT_EQ(
              db_->LookUpCrashReport(uuid, report),CrashReportDatabase::kNoError);
  ...
  void UploadReport(const UUID& uuid, bool successful, const std::string& id) {
    ...
    ASSERT_EQ(
              db_->GetReportForUploading(uuid, &report),CrashReportDatabase::kNoError);
    ...
    EXPECT_EQ(
              db_->RecordUploadAttempt(report, successful, id),CrashReportDatabase::kNoError);
...
TEST_F(CrashReportDatabaseTest, NewCrashReport) {
  ...
  EXPECT_EQ(
            db()->PrepareNewCrashReport(&new_report),CrashReportDatabase::kNoError);
  UUID expect_uuid = new_report->uuid;
  EXPECT_TRUE(FileExists(new_report->path)) << new_report->path.value();
  UUID uuid;
  EXPECT_EQ(
            db()->FinishedWritingCrashReport(new_report, &uuid),CrashReportDatabase::kNoError);
  EXPECT_EQ( uuid,expect_uuid);

  CrashReportDatabase::Report report;
  EXPECT_EQ(
            db()->LookUpCrashReport(uuid, &report),CrashReportDatabase::kNoError);
  ExpectPreparedCrashReport(report);
...
TEST_F(CrashReportDatabaseTest, ErrorWritingCrashReport) {
  ...
  EXPECT_EQ(
            db()->ErrorWritingCrashReport(new_report),CrashReportDatabase::kNoError);
...
TEST_F(CrashReportDatabaseTest, RecordUploadAttempt) {
  ...
  EXPECT_EQ(
            db()->LookUpCrashReport(reports[0].uuid, &query[0]),CrashReportDatabase::kNoError);
  EXPECT_EQ(
            db()->LookUpCrashReport(reports[1].uuid, &query[1]),CrashReportDatabase::kNoError);
  EXPECT_EQ(
            db()->LookUpCrashReport(reports[2].uuid, &query[2]),CrashReportDatabase::kNoError);
...
TEST_F(CrashReportDatabaseTest, GetCompletedAndNotUploadedReports) {
  ...
  EXPECT_EQ(
            db()->GetCompletedReports(&completed),CrashReportDatabase::kNoError);
  ...
  EXPECT_EQ(
            db()->SkipReportUpload(
                report_3_uuid, Metrics::CrashSkippedReason::kUploadsDisabled),CrashReportDatabase::kNoError);
...
TEST_F(CrashReportDatabaseTest, DuelingUploads) {
  ...
  EXPECT_EQ(
            db()->GetReportForUploading(report.uuid, &upload_report),CrashReportDatabase::kNoError);

  const CrashReportDatabase::Report* upload_report_2 = nullptr;
  EXPECT_EQ(
            db()->GetReportForUploading(report.uuid, &upload_report_2),CrashReportDatabase::kBusyError);
  EXPECT_FALSE(upload_report_2);

  EXPECT_EQ(
            db()->RecordUploadAttempt(upload_report, true, std::string()),CrashReportDatabase::kNoError);
}
...
TEST_F(CrashReportDatabaseTest, ReportRemoved) {
  ...
  EXPECT_EQ(
            db()->LookUpCrashReport(uuid, &report),CrashReportDatabase::kReportNotFound);
}
...
TEST_F(CrashReportDatabaseTest, DeleteReport) {
  ...
  EXPECT_EQ(
            db()->LookUpCrashReport(keep_completed.uuid, &keep_completed),CrashReportDatabase::kNoError);
  EXPECT_EQ(
            db()->LookUpCrashReport(delete_completed.uuid, &delete_completed),CrashReportDatabase::kNoError);

  EXPECT_TRUE(FileExists(keep_completed.file_path));
  EXPECT_TRUE(FileExists(delete_completed.file_path));

  EXPECT_EQ(
            db()->DeleteReport(delete_pending.uuid),CrashReportDatabase::kNoError);
  EXPECT_FALSE(FileExists(delete_pending.file_path));
  EXPECT_EQ(
            db()->LookUpCrashReport(delete_pending.uuid, &delete_pending),CrashReportDatabase::kReportNotFound);
  EXPECT_EQ(
            db()->DeleteReport(delete_pending.uuid),CrashReportDatabase::kReportNotFound);

  EXPECT_EQ(
            db()->DeleteReport(delete_completed.uuid),CrashReportDatabase::kNoError);
  EXPECT_FALSE(FileExists(delete_completed.file_path));
  EXPECT_EQ(
            db()->LookUpCrashReport(delete_completed.uuid, &delete_completed),CrashReportDatabase::kReportNotFound);
  EXPECT_EQ(
            db()->DeleteReport(delete_completed.uuid),CrashReportDatabase::kReportNotFound);

  EXPECT_EQ(
            db()->LookUpCrashReport(keep_pending.uuid, &keep_pending),CrashReportDatabase::kNoError);
  EXPECT_EQ(
            db()->LookUpCrashReport(keep_completed.uuid, &keep_completed),CrashReportDatabase::kNoError);
}
...
TEST_F(CrashReportDatabaseTest, DeleteReportEmptyingDatabase) {
  ...
  EXPECT_EQ(
            db()->LookUpCrashReport(report.uuid, &report),CrashReportDatabase::kNoError);
...
TEST_F(CrashReportDatabaseTest, RequestUpload) {
  ...
  EXPECT_EQ(
            db()->SkipReportUpload(
                report_1_uuid, Metrics::CrashSkippedReason::kUploadsDisabled),CrashReportDatabase::kNoError);
  ...
  CrashReportDatabase::Report report;
  EXPECT_EQ(
            db()->LookUpCrashReport(report_1_uuid, &report),CrashReportDatabase::kNoError);
  EXPECT_FALSE(report.upload_explicitly_requested);
  ...
  EXPECT_EQ(
            RequestUpload(report_0_uuid),CrashReportDatabase::kCannotRequestUpload);
}
