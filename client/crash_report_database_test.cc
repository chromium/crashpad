...
class CrashReportDatabaseTest : public testing::Test {
 ...
 protected:
  ...
  void SetUp() override {
    ...
    ASSERT_TRUE(db_->Initialize(path(), true));
  }
...
TEST_F(CrashReportDatabaseTest, Initialize) {
...

  {
    ...
    ASSERT_TRUE(db.Initialize(path(), false));
  ...
  {
    CrashReportDatabase db;
    ASSERT_TRUE(db.Initialize(path(), true));
  ...
  // InitializeWithoutCreating() shouldn’t create a nonexistent database.
  {
    ...
    EXPECT_FALSE(db.Initialize(non_database_path, false));
  }
}
