...
class CrashReportDatabase {
 public:
  ...
  //! and destroyed with RecordUploadComplete().
  ...
  //!
  //! Callers can then write the crash report using the FileWriter provided.
  ...
  //!     NewReport object and FileWriter within will be invalidated as part of
  //!     this call.
  ...
  //!     server.
  //!
  //! The FileReader in \a report should be used by the caller to upload the
  //! report. If the upload is successful, the caller should then call
  //! RecordUploadComplete(). If RecordUploadComplete() is not called, the
  //! upload attempt will be recorded as unsuccessful when \a report is
  //! destroyed. The lock on the report will be released when
  //! RecordUploadComplete() is called or \a report is destroyed.
  //!
  ...
  //!     upload attempt time as returned by Settings::GetLastUploadAttemptTime().
  //!
