...
class CrashReportDatabase {
 public:
  //! \brief Constructs and initializes the database using an appropriate
  //!     implementation for the platform.
  //!
  //! \see Initialize()
  //!
  //! \return an initialized database or `nullptr` on failure with a message
  //!     logged.
  static std::unique_ptr<CrashReportDatabase> BuildDatabase(
  ...
  struct Report {
    ...
    //! A file path for the report. This path may be invalid by the time this
    //! report is returned, so the UUID should be used as the canonical
    //! identifier.
  ...
  class ReadReport : public Report {
   public:
    ReadReport();
    virtual ~ReadReport();
   ...
   private:
    ...
    DISALLOW_COPY_AND_ASSIGN(ReadReport);
  };
  ...
  //!     the database. Any internal directory structure will be created if it
  //!     does not exist, as long as the outermost directory exists or \a
  //!     may_create is `true`.
  //! \param[in] may_create If `true`, the database directory, \a path, will be
  //!     created if it does not already exist.
  //!
  //! \return `true` if the database was successfully initialized. Otherwise
  //!     `false` with a message logged.
  virtual bool Initialize(const base::FilePath& path, bool may_create);
 ...
 private:
  ...
  // A helper for RecordUploadComplete(), recording an attempt as a success or
  // failure as indicated by successful.
  virtual OperationStatus RecordUploadAttempt(UploadReport* report,
                                              bool successful,
                                              const std::string& id);

#if defined(OS_WIN)
  // Removes reports, metadata, and directory structure leftover from the old
  // Windows database.
  void CleanOldWindowsData();
#endif
