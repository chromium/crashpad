...
class CrashReportDatabase {
 public:
  ...
  //!
  //! \param[in] lockfile_ttl The time-to-live for lockfiles or new reports.
  //! \return The number of reports cleaned.
  int CleanDatabase(time_t lockfile_ttl);
 ...
 private:
  ...
  // lock for the report in lock_file.
  OperationStatus LocateAndLockReport(const UUID& uuid,
                                      ReportState state,
                                      base::FilePath* path,
                                      ScopedLockFile* lock_file);

  // Locates the report with uuid in the specified state.
  // state may be kPending, kCompleted, or kSearchable.
