...
#include "util/file/scoped_delete_file.h"
...
class CrashReportDatabase {
 public:
  explicit CrashReportDatabase(const base::FilePath& path);
  ~CrashReportDatabase();
  ...
  struct NewReport {
   ...
   private:
    friend class CrashReportDatabase;
    ScopedDeleteFile file_;
  };

  using ScopedLockFile = ScopedDeleteFile;
  ...
  //!
  //! \return The operation status code.
  virtual OperationStatus ErrorWritingCrashReport(
      std::unique_ptr<NewReport>* report);
  ...
  //!
  //! \return The operation status code.
  virtual OperationStatus RecordUploadAttempt(
      std::unique_ptr<UploadReport>* report,
      bool successful,
      const std::string& id);
