...
bool CrashpadClient::StartHandler(
  ...
  exception_port_.reset(exception_port_raw);

  // This constant is from zircon/system/core/devmgr/devmgr.c and crashlogger.c.
  constexpr uint64_t kSysExceptionKey = 1166444u;
  status = zx_task_bind_exception_port(
      zx_process_self(), exception_port_.get(), kSysExceptionKey, 0);
  if (status != ZX_OK) {
  ...
  // exception port is passed on, so released here, but the process handle needs
  // to be duplicated.
  zx_handle_t handles[] = {ZX_HANDLE_INVALID, ZX_HANDLE_INVALID};
  status =
      zx_handle_duplicate(zx_process_self(), ZX_RIGHT_SAME_RIGHTS, &handles[0]);
  if (status != ZX_OK) {
    ZX_LOG(ERROR, status) << "zx_task_bind_exception_port";
    return false;
  }
  handles[1] = exception_port_.release();
  ...
  // bringup, but should be minimal for real usage.
  launchpad_clone(lp,
