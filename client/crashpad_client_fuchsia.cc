...
bool CrashpadClient::StartHandler(
  ...
  status =
      zx_handle_duplicate(zx_job_default(), ZX_RIGHT_SAME_RIGHTS, &actions[0].h.handle);
  if (status != ZX_OK) {
  ...
  // is useful during bringup, but should probably be made minimal for real usage.
  status = fdio_spawn_etc(ZX_HANDLE_INVALID, FDIO_SPAWN_CLONE_ALL, argv[0], argv.data(),
                          nullptr, kActionCount, actions, &child_raw, error_message);
