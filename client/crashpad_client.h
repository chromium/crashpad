...
class CrashpadClient {
 public:
  ...
  //! \brief Add intermediate dump \a file to pending reports, to be later
  //! processed by ProcessIntermediateDumps(). This intermediate dump can be
  //! generated by DumpWithoutCrashAndDeferProcessingAtPath().
  //!
  //! A handler must have already been installed before calling this method.
  //!
  //! \param[in] file The intermediate dump to move.
  static void AddIntermediateDump(const base::FilePath& file);
  ...
  //!     will not be converted to a mindump until it is added with
  //!     AddIntermediateDump() and ProcessIntermediateDumps() is called.
  //!
  //! For internal use only. Clients should use
