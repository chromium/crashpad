...
class CrashpadClient {
 public:
  ...
  //!
  //! \param[in] timeout The number of milliseconds to wait to join the thread,
  //!     or `0xffffffff` to block indefinitely.
  //!
  //! \return `true` if the hander startup succeeded, `false` otherwise, and an
  //!     error message will have been logged.
  bool WaitForHandlerStart(unsigned int timeout);
