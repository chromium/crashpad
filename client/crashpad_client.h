...
class CrashpadClient {
 public:
  ...
  //!     that should be used in fabricating and exception record.
  //! \param[in] exception_code The exception code to use in the fabricated
  //!     exception record.
  //!
  //! \return `true` if the request was sent successfully to the crashpad
  //!     handler, or `false` if not, with a message logged. This does not
  //!     guarantee that the target process was crashed, just that the request
  //!     was sent.
  bool DumpAndTerminateTargetProcess(HANDLE process,
                                     DWORD thread_id,
