...
bool WriteAndRename(const base::FilePath& file_path, const T& data) {
...

  {
    ...
    if (!temp.is_valid()) {
      LOG(ERROR) << "open " << temp_name.value();
      return false;
    }
...
bool WriteAndRenameAndReinitializeOnFailure(
  ...
  if (!settings->Initialize(settings_root)) {
    return false;
  }
...
bool Settings::Initialize(const base::FilePath& file_path) {
  ...
  if (CreateDirectory(file_path_, FilePermissions::kOwnerOnly, false)) {
    initialized_.set_valid();

    // Initialize all data. It is important that the client id be initialized
    // last, as that's what the race-loser path below waits on. These initial
    // sets do not use the public accessors because those attempt to
    // reinitialize on failure, which is not desirable from inside Initialize().
    bool uploads_enabled = false;
    time_t last_upload_time = 0;
    if (!WriteAndRename(file_path_.Append(kUploadsEnabled), uploads_enabled) ||
        !WriteAndRename(file_path_.Append(kLastUploadAttemptTime),
                        last_upload_time) ||
    ...
    if (!IsRegularFile(client_id)) {
      // TODO(scottmg): Non-Fuchsia-specific way of doing this, and consider
      // using this implementation on other platforms.
      ScopedDIR dir(opendir(file_path_.value().c_str()));
      const auto kWaitTimeForOtherProcess = zx_deadline_after(ZX_SEC(1));
      zx_status_t status = fdio_watch_directory(dirfd(dir.get()),
                                                WaitUntilFileCreatedCallback,
                                                kWaitTimeForOtherProcess,
                                                &client_id);
      if (status == ZX_ERR_TIMED_OUT) {
        // The other process did not successfully create the client id file, so
        // the settings are in an indeterminate state. Try starting over.
        LOG(ERROR) << "client id not created, attempting recovery";
        RemoveOldData(file_path);
