...
void AnnotationList::Add(Annotation* annotation) {
  ...
  do {
    ...
    cxs = head_.link_node().compare_exchange_weak(head_next, annotation);
    if (!cxs) {
      // Another thread has updated the head-next pointer, so reload it and
      // try again.
      head_next = head_.link_node().load(std::memory_order_relaxed);
