...
std::unique_ptr<Metadata> AcquireMetadata(const base::FilePath& base_dir) {
  base::FilePath metadata_file = base_dir.Append(kMetadataFileName);
  return Metadata::Create(metadata_file, base_dir.Append(kReportsDirectory));
}
...
OperationStatus CrashReportDatabaseWin::PrepareNewCrashReport(
  ...
  const base::FilePath path = base_dir_.Append(kReportsDirectory)
                                  .Append(new_report->uuid_.ToString16() +
                                          L"." + kCrashReportFileExtension);
...
OperationStatus CrashReportDatabaseWin::FinishedWritingCrashReport(
  ...
  std::unique_ptr<Metadata> metadata(AcquireMetadata(base_dir_));
  if (!metadata)
    return kDatabaseError;
  metadata->AddNewRecord(ReportDisk(report->uuid_,
                                    report->file_remover_.get(),
                                    time(nullptr),
                                    ReportStateWin::kPending));
...
int CrashReportDatabaseWin::CleanDatabase(time_t ttl) {
  return 0;
}
