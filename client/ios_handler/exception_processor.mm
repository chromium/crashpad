...
static void ObjcUncaughtExceptionHandler(NSException* exception) {
  if (g_ignore_uncaught) {
    // TODO(justincohen): This will crash again, do something else, maybe
    // unregister?
    return;
  }
  ...
  g_exception_delegate->HandleUncaughtNSException(&addresses[0],
                                                  addresses.size());
  // TODO(justincohen): This will crash again, maybe unregister?
  return;
}

id TerminatingFromNSException(id exception, const char* sinkhole) {
  std::string name, reason;
  SetNSExceptionAnnotations(exception, name, reason);
  LOG(WARNING) << "Terminating from Objective-C exception name: " << name
               << " reason: " << reason << " with sinkhole: " << sinkhole;
//  g_exception_delegate->HandleCaughtNSException();
...
id ObjcExceptionPreprocessor(id exception) {
  ...
  while (LoggingUnwStep(&cursor) > 0) {
    ...
    for (const char* sinkhole : kExceptionSymbolNameSinkholes) {
      if (strcmp(sinkhole, proc_name) == 0) {
        return TerminatingFromNSException(exception, sinkhole);
      }
    }
    ...
    if (ModulePathMatchesSinkhole(dl_info.dli_fname, kUIKitCorePath)) {
      ...
      if (LoggingUnwStep(&cursor) > 0 &&
        ...
        if (gesture_environment_min_imp && gesture_environment_max_imp &&
          ...
          return TerminatingFromNSException(exception,
  ...
  // See:
  // https://github.com/llvm/llvm-project/blob/c5d2746fbea7/libcxxabi/src/cxa_exception.cpp
  // __cxa_throw. Instead, terminate via TerminatingFromNSException so
  // the exception name and reason are properly recorded.
  if (!handler_found) {
    return TerminatingFromNSException(exception, "__cxa_throw");
  }
