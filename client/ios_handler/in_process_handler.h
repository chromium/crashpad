...
class InProcessHandler {
 public:
  ...
  //! \brief Instructs the handler to use an alternate file handle for the next
  //!     call to DumpExceptionFromMachException(). This is useful so calls to
  //!     DumpWithoutCrash do not interfere with the main exception file handle.
  //!
  //! \param[out] out_file The directory to use for the next call to
  //!     DumpExceptionFromMachException.
  //! \return `true` on success, `false` on failure with a message logged.
  bool UseAlternateMachExceptionPath(base::FilePath* out_file);

  //! \brief Instructs the handler to use an alternate file handle for the next
  //!     call to DumpExceptionFromMachException(). This is useful so calls to
  //!     DumpWithoutCrash do not interfere with the main exception file handle.
  //!
  //! \param[in] path The directory to use for the next call to
  //!     DumpExceptionFromMachException.
  //! \return `true` on success, `false` on failure with a message logged.
  bool UseAlternateMachExceptionPathAtPath(const base::FilePath& path);
