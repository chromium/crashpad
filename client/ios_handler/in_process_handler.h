...
class InProcessHandler {
 ...
 private:
  ...
  void WriteSystemInfo(const IOSSystemDataCollector& system_data);
  void WriteThreadInfo();
  void WriteModuleInfo();
  void WriteModuleInfoAtAddress(uint64_t address);
  void WriteMachExceptionInfo(exception_behavior_t behavior,
                              thread_t thread,
                              exception_type_t exception,
                              const mach_exception_data_type_t* code,
                              mach_msg_type_number_t code_count,
