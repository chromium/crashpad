...
void WriteHeader(int fd) {
  uint8_t version = 1;
  IOSIntermediatedumpWriterUtil::Property(
      fd, IntermediateDumpKey::kVersion, &version, sizeof(version));
}

void WriteProcessInfo(int fd) {
  IOSIntermediatedumpWriterUtil::MapStart(fd,
  ...
  if (sysctl(mib, base::size(mib), &kern_proc_info, &len, nullptr, 0) == 0) {
    IOSIntermediatedumpWriterUtil::Property(fd,
                                            IntermediateDumpKey::kPID,
  ...
  if (kr == KERN_SUCCESS) {
    IOSIntermediatedumpWriterUtil::MapStart(
        fd, IntermediateDumpKey::kTaskBasicInfo);
    IOSIntermediatedumpWriterUtil::Property(fd,
    ...
    IOSIntermediatedumpWriterUtil::Property(fd,
                                            ...
                                            sizeof(time_value_t));
    IOSIntermediatedumpWriterUtil::MapEnd(fd);
  } else {
  ...
  if (kr == KERN_SUCCESS) {
    IOSIntermediatedumpWriterUtil::MapStart(
        fd, IntermediateDumpKey::kTaskThreadTimes);
    IOSIntermediatedumpWriterUtil::Property(fd,
                                            IntermediateDumpKey::kUserTime,
  ...
  if (gettimeofday(&snapshot_time, nullptr) == 0) {
    IOSIntermediatedumpWriterUtil::Property(fd,
                                            IntermediateDumpKey::kSnapshotTime,
                                            &snapshot_time,
...
void WriteSystemInfo(int fd, const IOSSystemDataCollector& system_data) {
  IOSIntermediatedumpWriterUtil::MapStart(fd, IntermediateDumpKey::kSystemInfo);
  ...
  system_data.OSVersion(
      &os_version_major, &os_version_minor, &os_version_bugfix, nil);
  std::string os_version_build = system_data.Build();
  IOSIntermediatedumpWriterUtil::Property(fd,
                                          IntermediateDumpKey::kOSVersionMajor,
                                          &os_version_major,
                                          sizeof(os_version_major));
  IOSIntermediatedumpWriterUtil::Property(fd,
                                          IntermediateDumpKey::kOSVersionMinor,
                                          &os_version_minor,
                                          sizeof(os_version_minor));
  IOSIntermediatedumpWriterUtil::Property(fd,
                                          IntermediateDumpKey::kOSVersionBugfix,
                                          &os_version_bugfix,
  ...
  IOSIntermediatedumpWriterUtil::Property(
      fd, IntermediateDumpKey::kCpuCount, &cpu_count, sizeof(cpu_count));
  ...
  IOSIntermediatedumpWriterUtil::Property(
      fd, IntermediateDumpKey::kPageSize, &page_size, sizeof(page_size));
  ...
  kern_return_t kr = host_statistics(mach_host_self(),
                                     ...
                                     &host_size);
  if (kr == KERN_SUCCESS) {
    IOSIntermediatedumpWriterUtil::MapStart(fd, IntermediateDumpKey::kVMStat);
    IOSIntermediatedumpWriterUtil::Property(fd,
                                            IntermediateDumpKey::kActive,
                                            &vm_stat.active_count,
                                            sizeof(uint64_t));
    IOSIntermediatedumpWriterUtil::Property(fd,
                                            IntermediateDumpKey::kInactive,
                                            &vm_stat.inactive_count,
                                            sizeof(uint64_t));
    IOSIntermediatedumpWriterUtil::Property(
        fd, IntermediateDumpKey::kWired, &vm_stat.wire_count, sizeof(uint64_t));
    IOSIntermediatedumpWriterUtil::Property(
        fd, IntermediateDumpKey::kFree, &vm_stat.free_count, sizeof(uint64_t));
    IOSIntermediatedumpWriterUtil::MapEnd(fd);
  } else {
...
void WriteThreadInfo(int fd) {
  IOSIntermediatedumpWriterUtil::ArrayStart(fd, IntermediateDumpKey::kThreads);
  ...
  for (uint32_t thread_index = 0; thread_index < thread_count; ++thread_index) {
    IOSIntermediatedumpWriterUtil::ArrayMapStart(fd);
    ...
    IOSIntermediatedumpWriterUtil::Property(fd,
                                            IntermediateDumpKey::kThreadState,
                                            &thread_state,
                                            sizeof(thread_state));
    ...
    IOSIntermediatedumpWriterUtil::Property(fd,
                                            IntermediateDumpKey::kFloatState,
                                            &float_state,
                                            sizeof(float_state));
    ...
    IOSIntermediatedumpWriterUtil::Property(fd,
                                            IntermediateDumpKey::kDebugState,
                                            &debug_state,
                                            sizeof(debug_state));
#endif
  ...
  IOSIntermediatedumpWriterUtil::ArrayEnd(fd);
}
...
#error Only 64-bit Mach-O is supported
  ...
  IOSIntermediatedumpWriterUtil::ArrayStart(fd, IntermediateDumpKey::kModules);
  ...
  for (uint32_t image_index = 0; image_index < image_count; ++image_index) {
    ...
    IOSIntermediatedumpWriterUtil::Property(
        fd, IntermediateDumpKey::kAddress, &address, sizeof(address));
    IOSIntermediatedumpWriterUtil::Property(fd,
                                            IntermediateDumpKey::kTimestamp,
...
void WriteModuleInfoAtAddress(int fd, uint64_t address) {
  ...
  for (uint32_t cmd_index = 0, cumulative_cmd_size = 0;
    ...
    if (command->cmd == LC_SEGMENT_64) {
      ...
      IOSIntermediatedumpWriterUtil::Property(
          fd, IntermediateDumpKey::kUUID, &uuid->uuid, sizeof(uuid->uuid));
    }
...
void WriteMachExceptionInfo(int fd,
  ...
  IOSIntermediatedumpWriterUtil::MapStart(fd,
                                          IntermediateDumpKey::kMachException);
  IOSIntermediatedumpWriterUtil::Property(
      fd, IntermediateDumpKey::kException, &exception, sizeof(exception));
  IOSIntermediatedumpWriterUtil::Property(
      fd,
      IntermediateDumpKey::kCode,
      &code,
      sizeof(mach_exception_data_type_t) * code_count);
  IOSIntermediatedumpWriterUtil::Property(fd,
                                          IntermediateDumpKey::kCodeCount,
                                          &code_count,
                                          sizeof(mach_msg_type_number_t));
  IOSIntermediatedumpWriterUtil::Property(
      fd, IntermediateDumpKey::kFlavor, &flavor, sizeof(flavor));
  size_t state_length =
      IOSIntermediatedumpWriterUtil::ThreadStateLengthForFlavor(flavor);
  IOSIntermediatedumpWriterUtil::Property(
      fd, IntermediateDumpKey::kState, state, state_length);
  IOSIntermediatedumpWriterUtil::Property(
      fd, IntermediateDumpKey::kStateCount, &state_count, sizeof(state_count));
...
void WriteExceptionFromSignal(int fd,
                              ...
                              ucontext_t* context) {
  IOSIntermediatedumpWriterUtil::MapStart(
      fd, IntermediateDumpKey::kSignalException);

  IOSIntermediatedumpWriterUtil::Property(fd,
                                          IntermediateDumpKey::kSignalNumber,
                                          &siginfo->si_signo,
                                          sizeof(siginfo->si_signo));
  IOSIntermediatedumpWriterUtil::Property(fd,
                                          IntermediateDumpKey::kSignalCode,
                                          &siginfo->si_code,
                                          sizeof(siginfo->si_code));
  IOSIntermediatedumpWriterUtil::Property(fd,
                                          IntermediateDumpKey::kSignalAddress,
                                          &siginfo->si_addr,
