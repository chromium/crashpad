...
void WriteProcessInfo(int fd) {
  IOSMinidumpWriterUtil::MapStart(fd, IntermediateDumpKey::kProcessInfo);
  ...
  if (kr == KERN_SUCCESS) {
    IOSMinidumpWriterUtil::MapStart(fd, IntermediateDumpKey::kTaskBasicInfo);
    IOSMinidumpWriterUtil::Property(fd,
  ...
  if (kr == KERN_SUCCESS) {
    IOSMinidumpWriterUtil::MapStart(fd, IntermediateDumpKey::kTaskThreadTimes);
    IOSMinidumpWriterUtil::Property(fd,
                                    IntermediateDumpKey::kUserTime,
...
void WriteSystemInfo(int fd, const IOSSystemDataCollector& system_data) {
  IOSMinidumpWriterUtil::MapStart(fd, IntermediateDumpKey::kSystemInfo);
  ...
  kern_return_t kr = host_statistics(mach_host_self(),
                                     ...
                                     &host_size);
  if (kr == KERN_SUCCESS) {
    IOSMinidumpWriterUtil::MapStart(fd, IntermediateDumpKey::kVMStat);
    IOSMinidumpWriterUtil::Property(fd,
...
void WriteThreadInfo(int fd) {
  IOSMinidumpWriterUtil::ArrayStart(fd, IntermediateDumpKey::kThreads);
...
#error Only 64-bit Mach-O is supported
  ...
  IOSMinidumpWriterUtil::ArrayStart(fd, IntermediateDumpKey::kModules);
...
void WriteMachExceptionInfo(int fd,
  ...
  IOSMinidumpWriterUtil::MapStart(fd, IntermediateDumpKey::kMachException);
  IOSMinidumpWriterUtil::Property(
  ...
  size_t state_length =
      IOSMinidumpWriterUtil::ThreadStateLengthForFlavor(flavor);
  IOSMinidumpWriterUtil::Property(
...
void WriteExceptionFromSignal(int fd,
                              ...
                              siginfo_t* siginfo,
                              ucontext_t* context) {
  IOSMinidumpWriterUtil::MapStart(fd, IntermediateDumpKey::kSignalException);
