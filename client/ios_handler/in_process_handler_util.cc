...
void ArrayStart(int fd, const char* name) {
  uint8_t t = crashpad::ARRAY_START;
  write(fd, &t, sizeof(uint8_t));
  size_t name_length = strlen(name);
  write(fd, &name_length, sizeof(size_t));
  write(fd, name, name_length);
...
void MapEnd(int fd) {
  uint8_t t = crashpad::MAP_END;
  write(fd, &t, sizeof(uint8_t));
}

void ArrayEnd(int fd) {
  uint8_t t = crashpad::ARRAY_END;
  write(fd, &t, sizeof(uint8_t));
}
...
void WriteProcessInfo(int fd) {
  ...
  if (sysctl(mib, base::size(mib), &kern_proc_info, &len, nullptr, 0) == 0) {
    MapStart(fd, "kern_proc_info");
    Property(fd, "p_pid", &kern_proc_info.kp_proc.p_pid, sizeof(pid_t));
    Property(fd, "e_ppid", &kern_proc_info.kp_eproc.e_ppid, sizeof(pid_t));
    Property(fd,
             "p_starttime",
  ...
  if (kr == KERN_SUCCESS) {
    MapStart(fd, "task_basic_info");
    Property(fd, "user_time", &task_basic_info.user_time, sizeof(time_value_t));
    Property(
  ...
  if (gettimeofday(&snapshot_time, nullptr) == 0) {
    Property(fd, "snapshot_time", &snapshot_time, sizeof(snapshot_time));
  } else {
...
void WriteSystemInfo(int fd, const IOSSystemDataCollector& system_data) {
  ...
  system_data.OSVersion(&os_version_major,
                        ...
                        &os_version_bugfix,
                        &os_version_build);
  Property(fd, "os_version_major", &os_version_major, sizeof(os_version_major));
  Property(fd, "os_version_minor", &os_version_minor, sizeof(os_version_minor));
  Property(
  ...
  Property(fd, "cpu_count", &cpu_count, sizeof(cpu_count));
  std::string cpu_vendor = system_data.CPUVendor();
  Property(fd, "cpu_vendor", cpu_vendor.c_str(), cpu_vendor.length());
  ...
  Property(fd, "standard_name", standard_name.c_str(), standard_name.length());
  std::string daylight_name = system_data.DaylightName();
  Property(fd, "daylight_name", daylight_name.c_str(), daylight_name.length());

  vm_size_t page_size;
  host_page_size(mach_host_self(), &page_size);
  Property(fd, "page_size", &page_size, sizeof(page_size));
  ...
  if (kr == KERN_SUCCESS) {
    MapStart(fd, "vm_stat");
    Property(fd, "active", &vm_stat.active_count, sizeof(uint64_t));
    Property(fd, "inactive", &vm_stat.inactive_count, sizeof(uint64_t));
    Property(fd, "wired", &vm_stat.wire_count, sizeof(uint64_t));
    Property(fd, "free", &vm_stat.free_count, sizeof(uint64_t));
    MapEnd(fd);
  } else {
...
void WriteThreadInfo(int fd) {
  ...
  for (uint32_t thread_index = 0; thread_index < thread_count; ++thread_index) {
    ...
    Property(fd, "thread_state", &thread_state, sizeof(thread_state));
    ...
    Property(fd, "float_state", &float_state, sizeof(float_state));
    ...
    Property(fd, "debug_state", &debug_state, sizeof(debug_state));
#endif
    ...
    kr = thread_info(thread,
                     ...
                     &count);
    if (kr == KERN_SUCCESS) {
      Property(fd, "suspend_count", &basic_info.suspend_count, sizeof(int));
    } else {
      // TODO(justincohen): What to do with errors and warnings?
    }
    ...
    kr = thread_info(thread,
                     ...
                     &count);
    if (kr == KERN_SUCCESS) {
      Property(fd, "thread_id", &identifier_info.thread_id, sizeof(uint64_t));
      Property(fd,
    ...
    kr = thread_policy_get(thread,
                           ...
                           &get_default);
    if (kr == KERN_SUCCESS) {
      Property(fd, "priority", &precedence.importance, sizeof(int));
    } else {
      // TODO(justincohen): What to do with errors and warnings?
    }
    ...
    if (kr == KERN_SUCCESS) {
      ...
      Property(fd, "stack_region_data", offset_data, stack_region_size);
...
#error Only 64-bit Mach-O is supported
  ...
  for (uint32_t image_index = 0; image_index < image_count; ++image_index) {
    ...
    Property(fd, "name", image->imageFilePath, strlen(image->imageFilePath));
    uint64_t address = FromPointerCast<uint64_t>(image->imageLoadAddress);
    Property(fd, "address", &address, sizeof(address));
    Property(fd,
             "timestamp",
  ...
  ArrayMapStart(fd);
  Property(fd, "name", image_infos->dyldPath, strlen(image_infos->dyldPath));
  uint64_t address =
...
void WriteModuleInfoAtAddress(int fd, uint64_t address) {
  ...
  for (uint32_t cmd_index = 0, cumulative_cmd_size = 0;
    ...
    if (command->cmd == LC_SEGMENT_64) {
      ...
      if (strcmp(segment->segname, SEG_TEXT) == 0) {
        Property(fd, "size", &segment->vmsize, sizeof(segment->vmsize));
      }
      ...
      Property(fd, "uuid", &uuid->uuid, sizeof(uuid->uuid));
    }
  ...
  Property(fd, "filetype", &header->filetype, sizeof(header->filetype));
}
...
void WriteMachExceptionInfo(int fd,
  ...
  MapStart(fd, "MachException");
  Property(fd, "exception", &exception, sizeof(exception));
  Property(fd, "code", &code, sizeof(mach_exception_data_type_t) * code_count);
  Property(fd, "code_count", &code_count, sizeof(mach_msg_type_number_t));
  Property(fd, "flavor", &flavor, sizeof(flavor));
  Property(fd, "state", &state, sizeof(state));
  Property(fd, "state_count", &state_count, sizeof(state_count));
...
void WriteExceptionFromSignal(int fd,
                              ...
                              siginfo_t* siginfo,
                              ucontext_t* context) {
  MapStart(fd, "SignalException");

  Property(fd, "siginfo", siginfo, sizeof(siginfo_t));
#if defined(ARCH_CPU_X86_64)
  Property(fd,
           "thread_state",
