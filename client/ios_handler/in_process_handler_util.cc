...
void WriteMachExceptionInfo(int fd,
  ...
  IOSMinidumpWriterUtil::Property(fd, "state", state, state_length);
  IOSMinidumpWriterUtil::Property(
...
void WriteExceptionFromSignal(int fd,
  ...
  IOSMinidumpWriterUtil::MapStart(fd, "siginfo");
  IOSMinidumpWriterUtil::Property(
      fd, "si_signo", &siginfo->si_signo, sizeof(siginfo->si_signo));
  IOSMinidumpWriterUtil::Property(
      fd, "si_code", &siginfo->si_code, sizeof(siginfo->si_code));
  IOSMinidumpWriterUtil::Property(
      fd, "si_addr", &siginfo->si_addr, sizeof(siginfo->si_addr));
  IOSMinidumpWriterUtil::MapEnd(fd);
#if defined(ARCH_CPU_X86_64)
  IOSMinidumpWriterUtil::Property(fd,
                                  "thread_state",
