...
void InProcessIntermediatedumpHandler::WriteModuleInfoAtAddress(
  ...
  if (header->filetype == MH_DYLINKER) {
    WriteDyldErrorStringAnnotation(
        writer, address, symtab_command, text_seg, linkedit_seg, slide);
  }
}
  ...
  for (uint32_t sect_index = 0; sect_index <= segment->nsects; ++sect_index) {
    if (strcmp(section->sectname, "crashpad_info") == 0) {
      ScopedVMRead<process_types::CrashpadInfo> crashpad_info(
          reinterpret_cast<process_types::CrashpadInfo*>(section->addr +
                                                         slide));
    ...
    if (strcmp(section->sectname, "__crash_info") == 0) {
      ScopedVMRead<process_types::crashreporter_annotations_t> crash_info(
          reinterpret_cast<process_types::crashreporter_annotations_t*>(
              section->addr + slide));
...
void InProcessIntermediatedumpHandler::WriteAnnotationList(
  ...
  ScopedVMRead<process_types::AnnotationList> annotation_list(
      reinterpret_cast<process_types::AnnotationList*>(
  ...
  for (size_t index = 0;
    ...
    if (node->size == 0 || node->size > Annotation::kValueMaxSize) {
      DLOG(WARNING) << "Incorrect annotation length";
      return;
    }
...
void InProcessIntermediatedumpHandler::WriteSimpleAnnotation(
  ...
  ScopedVMRead<SimpleStringDictionary> simple_annotations(
      reinterpret_cast<SimpleStringDictionary*>(
...
void InProcessIntermediatedumpHandler::WriteCrashInfoAnnotations(
  ...
  if (crash_info->message) {
    ...
    writer->AddProperty(IntermediateDumpKey::kAnnotationsCrashInfoMessage1,
  ...
  if (crash_info->message2) {
    ...
    writer->AddProperty(IntermediateDumpKey::kAnnotationsCrashInfoMessage2,
...
void InProcessIntermediatedumpHandler::WriteDyldErrorStringAnnotation(
  ...
  // TODO(justincohen): Optimize this by using a dysymtab is present to filter
  // the symtab.
