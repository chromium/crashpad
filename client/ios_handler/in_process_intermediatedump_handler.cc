...
void InProcessIntermediatedumpHandler::WriteProcessInfo(
  ...
  if (kr == KERN_SUCCESS) {
    ...
    RAW_LOG("task_info task_basic_info");
  }
...
void InProcessIntermediatedumpHandler::WriteThreadInfo(
  ...
  if (kr == KERN_SUCCESS) {
    ...
    // TODO(justincohen): What to do with errors and warnings?
    RAW_LOG_ERROR(kr, "thread_info::THREAD_IDENTIFIER_INFO");
  }
  ...
  for (uint32_t thread_index = 0; thread_index < thread_count; ++thread_index) {
    ...
    if (kr == KERN_SUCCESS) {
      ...
      // TODO(justincohen): What to do with errors and warnings?
      RAW_LOG_ERROR(kr, "thread_info::THREAD_BASIC_INFO");
    }
    ...
    if (kr == KERN_SUCCESS) {
      ...
      // TODO(justincohen): What to do with errors and warnings?
      RAW_LOG_ERROR(kr, "thread_policy_get");
    }
    ...
    if (kr != KERN_SUCCESS) {
      // TODO(justincohen): What to do with errors and warnings?
      RAW_LOG_ERROR(kr, "thread_get_state::kThreadStateFlavor");
    }
    ...
    if (kr != KERN_SUCCESS) {
      // TODO(justincohen): What to do with errors and warnings?
      RAW_LOG_ERROR(kr, "thread_get_state::kFloatStateFlavor");
    }
    ...
    if (kr != KERN_SUCCESS) {
      // TODO(justincohen): What to do with errors and warnings?
      RAW_LOG_ERROR(kr, "thread_get_state::kDebugStateFlavor");
    }
...
void InProcessIntermediatedumpHandler::WriteAnnotationList(
  ...
  if (!annotation_list.Read(crashpad_info->annotations_list)) {
    RAW_LOG("could not read annotations list object");
    return;
  }
  ...
  if (!current.Read(&annotation_list->head)) {
    RAW_LOG("could not read annotation");
    return;
  }
  ...
  for (size_t index = 0;
    ...
    if (node->size > Annotation::kValueMaxSize) {
      RAW_LOG("Incorrect annotation length");
      continue;
    }
...
void InProcessIntermediatedumpHandler::WriteSimpleAnnotation(
  ...
  if (!simple_annotations.Read(crashpad_info->simple_annotations)) {
    RAW_LOG("could not read simple annotations.");
    return;
  }
