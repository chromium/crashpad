  ...
  for (uint32_t cmd_index = 0, cumulative_cmd_size = 0;
    ...
    if (command->cmd == LC_SEGMENT_64) {
      ...
      if (strcmp(segment->segname, SEG_TEXT) == 0) {
        ...
        WriteAnnotations(writer, segment, slide);
      }
...
// static
void InProcessIntermediatedumpHandler::WriteAnnotations(
    IOSIntermediatedumpWriter* writer,
    const segment_command_64* segment,
    vm_size_t slide) {
  ...
  for (uint32_t sect_index = 0; sect_index <= segment->nsects; ++sect_index) {
    if (strcmp(section->sectname, "crashpad_info") == 0) {
      process_types::CrashpadInfo* crashpad_info = nullptr;
      IOSIntermediatedumpWriter::ScopedVMRead crashpad_info_vmread(
          reinterpret_cast<const void*>(section->addr + slide),
          sizeof(process_types::CrashpadInfo),
          &crashpad_info);
      if (!crashpad_info ||
          crashpad_info->signature != CrashpadInfo::kSignature ||
          crashpad_info->version != 1) {
        LOG(WARNING) << "Invalid crashpad_info";
        continue;
      }
      ...
      IOSIntermediatedumpWriter::ScopedVMRead annotation_list_object_read(
          reinterpret_cast<const void*>(crashpad_info->annotations_list),
          sizeof(process_types::AnnotationList),
          &annotation_list_object);

      if (!annotation_list_object) {
      ...
      for (size_t index = 0;
           ...
           ++index) {
        process_types::Annotation* node = nullptr;
        IOSIntermediatedumpWriter::ScopedVMRead annotation_node_vmread(
            reinterpret_cast<const void*>(current->link_node),
            sizeof(process_types::Annotation),
            &node);
        if (!node) {
          break;
        }

        current = (process_types::Annotation*)current->link_node;

        if (node->size == 0 || node->size > Annotation::kValueMaxSize) {
