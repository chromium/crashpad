...
#if 0
        // This isn't in process safe yet.
            ...
            if (crashpad_info && crashpad_info->is_valid()) {
              ...
              if (annotations_list) {
                ...
                for (AnnotationList::Iterator it = annotations_list->begin();
                  ...
                  const size_t name_len = strlen(annotation->name());
                  if (name_len <= Annotation::kNameMaxLength &&
                      annotation->size() <= Annotation::kValueMaxSize) {
                    writer->AddProperty(IntermediateDumpKey::kAnnotationName,
                                        annotation->name(),
                                        strlen(annotation->name()));
