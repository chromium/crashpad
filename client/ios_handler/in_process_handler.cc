...
void InProcessHandler::ProcessIntermediateDump(
  ...
  if (process_snapshot.Initialize(path, annotations)) {
    SaveSnapshot(process_snapshot);
  }
  LoggingRemoveFile(path);
}

void InProcessHandler::EnableUploading() {
  if (!enable_uploading_) {
    upload_thread_->Start();
    enable_uploading_ = true;
  }
}
...
void InProcessHandler::SaveSnapshot(
  ...
  // TODO: Is this necessary on iOS?
  crashpad::UUID client_id;
  database_->GetSettings()->GetClientID(&client_id);
