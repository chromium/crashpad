...
constexpr char kPendingIntermediateiOSDump[] = "pending-intermediate-ios-dump";
...
void InProcessHandler::Initialize(
  ...
  base_dir_ = database.Append(kPendingIntermediateiOSDump);
...
void InProcessHandler::DumpExceptionFromSignal(
  ...
  // TODO(justincohen): This should write an intermediate dump instead.
  // For now, load the snapshot just to get data.
...
void InProcessHandler::ProcessPendingDumps() {
  ...
  for (auto& file : files) {
    // TODO(justincohen): Load intermediate dump and SaveSnapshot();
...
bool InProcessHandler::OpenNewFile() {
  ...
  // TODO(justincohen): Open a new intermediate dump writer.
  return true;
}
