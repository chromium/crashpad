...
void InProcessHandler::DumpExceptionFromSignal(
  ...
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
  {
    ...
    InProcessIntermediateDumpHandler::WriteExceptionFromSignal(
        Writer(), system_data, siginfo, context);
  }
  PostReportCleanup();
}
...
void InProcessHandler::DumpExceptionFromNSExceptionFrames(
  ...
  {
    ScopedReport report(Writer(), system_data, frames, num_frames);
    InProcessIntermediateDumpHandler::WriteUncaughtNSException(Writer());
  }
  PostReportCleanup();
}
...
bool InProcessHandler::SetAlternateDumpWithoutCrashPath(
    const base::FilePath& path) {
  UUID uuid;
  uuid.InitializeWithNew();
  const std::string uuid_string = uuid.ToString();
  base::FilePath alternate_file = path.Append(uuid_string);
  alternate_writer_ = std::make_unique<IOSIntermediateDumpWriter>();
  if (!alternate_writer_->Open(alternate_file)) {
...
void InProcessHandler::PostReportCleanup() {
  if (alternate_writer_) {
    alternate_writer_->Close();
    alternate_writer_.reset();
  } else {
    OpenNewFile();
  }
}

IOSIntermediateDumpWriter* InProcessHandler::Writer() {
  if (alternate_writer_) {
    return alternate_writer_.get();
  } else {
    return writer_.get();
  }
}
