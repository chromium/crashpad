...
#error Port to your CPU architecture
  ...
  kern_return_t kr =
      thread_info(mach_thread_self(),
                  ...
                  &count);
  if (kr == KERN_SUCCESS) {
    Property(fd_, "thread_id", &identifier_info.thread_id, sizeof(uint64_t));
  } else {
    // TODO(justincohen): What to do with errors and warnings?
  }
...
void InProcessHandler::WriteProcessInfo() {
  ...
  if (sysctl(mib, base::size(mib), &kern_proc_info, &len, nullptr, 0) == 0) {
    MapStart(fd_, "kern_proc_info");
    Property(fd_, "p_pid", &kern_proc_info.kp_proc.p_pid, sizeof(pid_t));
    Property(fd_, "e_ppid", &kern_proc_info.kp_eproc.e_ppid, sizeof(pid_t));
    Property(fd_,
             "p_starttime",
  ...
  if (kr == KERN_SUCCESS) {
    MapStart(fd_, "task_basic_info");
    Property(
        fd_, "user_time", &task_basic_info.user_time, sizeof(time_value_t));
    Property(
        fd_, "system_time", &task_basic_info.system_time, sizeof(time_value_t));
    MapEnd(fd_);
  } else {
  ...
  if (kr == KERN_SUCCESS) {
    MapStart(fd_, "task_thread_times");
    Property(
        fd_, "user_time", &task_thread_times.user_time, sizeof(time_value_t));
    Property(fd_,
             "system_time",
...
void InProcessHandler::WriteSystemInfo(
  ...
  kern_return_t kr = host_statistics(mach_host_self(),
                                     ...
                                     &host_size);
  if (kr == KERN_SUCCESS) {
    MapStart(fd_, "vm_stat");
    Property(fd_, "active", &vm_stat.active_count, sizeof(uint64_t));
    Property(fd_, "inactive", &vm_stat.inactive_count, sizeof(uint64_t));
    Property(fd_, "wired", &vm_stat.wire_count, sizeof(uint64_t));
    Property(fd_, "free", &vm_stat.free_count, sizeof(uint64_t));
    MapEnd(fd_);
  } else {
...
void InProcessHandler::WriteThreadInfo() {
  ...
  for (uint32_t thread_index = 0; thread_index < thread_count; ++thread_index) {
    ...
    kr = thread_info(thread,
                     ...
                     &count);
    if (kr == KERN_SUCCESS) {
      Property(fd_, "suspend_count", &basic_info.suspend_count, sizeof(int));
    } else {
      // TODO(justincohen): What to do with errors and warnings?
    }
    ...
    kr = thread_policy_get(thread,
                           ...
                           &get_default);
    if (kr == KERN_SUCCESS) {
      Property(fd_, "priority", &precedence.importance, sizeof(int));
    } else {
      // TODO(justincohen): What to do with errors and warnings?
    }
    ...
    vm_offset_t* data = new vm_offset_t[stack_region_size];
    ...
    kr = vm_read(mach_task_self(),
                 ...
                 &dataCnt);
    if (kr == KERN_SUCCESS) {
      Property(fd_, "stack_region_data", data, dataCnt);
    }
    delete[] data;
