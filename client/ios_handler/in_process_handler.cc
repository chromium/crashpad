...
void InProcessHandler::DumpExceptionFromMachException(
  ...
  if (alternate_mach_writer_) {
    writer = alternate_mach_writer_.get();
  } else {
    writer = writer_.get();
  }

  {
    ScopedReport report(writer, system_data);
    InProcessIntermediateDumpHandler::WriteMachExceptionInfo(writer,
                                                             behavior,
                                                             thread,
                                                             exception,
                                                             code,
  ...
  if (alternate_mach_writer_) {
    alternate_mach_writer_->Close();
    alternate_mach_writer_.reset();
  } else {
    OpenNewFile();
  }
}
...
void InProcessHandler::DumpExceptionFromNSExceptionFrames(
    ...
    const size_t num_frames) {
  {
    ...
    InProcessIntermediateDumpHandler::WriteUncaughtNSException(writer_.get());
  }
  OpenNewFile();
}
...
bool InProcessHandler::UseAlternateMachExceptionPath(base::FilePath* out_file) {
  DCHECK(!alternate_mach_writer_);
  UUID uuid;
  uuid.InitializeWithNew();
  const std::string uuid_string = uuid.ToString();
  base::FilePath alternate_file = base_dir_.Append(uuid_string);
  alternate_mach_writer_ = std::make_unique<IOSIntermediateDumpWriter>();
  if (!alternate_mach_writer_->Open(alternate_file)) {
    DLOG(ERROR) << "Unable to open alternate intermediate dump file: "
                << alternate_file.value();
    return false;
  }
  *out_file = alternate_file;
  return true;
}

bool InProcessHandler::UseAlternateMachExceptionPathAtPath(
    const base::FilePath& path) {
  DCHECK(!alternate_mach_writer_);
  UUID uuid;
  uuid.InitializeWithNew();
  const std::string uuid_string = uuid.ToString();
  base::FilePath alternate_file = path.Append(uuid_string);
  alternate_mach_writer_ = std::make_unique<IOSIntermediateDumpWriter>();
  if (!alternate_mach_writer_->Open(alternate_file)) {
    DLOG(ERROR) << "Unable to open alternate intermediate dump file: "
                << alternate_file.value();
