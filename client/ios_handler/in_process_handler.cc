...
void InProcessHandler::DumpExceptionFromSignal(
  ...
  InProcessHandlerUtil::WriteExceptionFromSignal(
...
void InProcessHandler::DumpExceptionFromMachException(
  ...
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
  StartReport(system_data);
  InProcessHandlerUtil::WriteMachExceptionInfo(writer_.get(),
                                               behavior,
                                               thread,
                                               exception,
                                               code,
...
void InProcessHandler::StartReport(const IOSSystemDataCollector& system_data) {
  InProcessHandlerUtil::WriteHeader(writer_.get());
  InProcessHandlerUtil::WriteProcessInfo(writer_.get());
  InProcessHandlerUtil::WriteSystemInfo(writer_.get(), system_data);
  InProcessHandlerUtil::WriteThreadInfo(writer_.get());
  InProcessHandlerUtil::WriteModuleInfo(writer_.get());
}
