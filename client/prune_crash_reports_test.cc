...
TEST(PruneCrashReports, AgeCondition) {
  ...
  MockDatabase db;
  EXPECT_TRUE(condition.ShouldPruneReport(&db, report_80_days));
  EXPECT_FALSE(condition.ShouldPruneReport(&db, report_10_days));
  EXPECT_FALSE(condition.ShouldPruneReport(&db, report_30_days));
}
...
TEST(PruneCrashReports, BinaryCondition) {
  ...
  for (const auto& test : kTests) {
    ...
    EXPECT_EQ(condition.ShouldPruneReport(&db, report), test.cond_result);
