...
TEST(PruneCrashReports, SizeCondition) {
  ...
  auto db = CrashReportDatabase::BuildDatabase(temp_dir.path(),
                                               /* may_create= */ false);
  ASSERT_TRUE(db);
  ...
  ASSERT_NO_FATAL_FAILURE(AddReport(db.get(), &contents, 1024, &report_1k));
  CrashReportDatabase::Report report_3k;
  ASSERT_NO_FATAL_FAILURE(AddReport(db.get(), &contents, 3072, &report_3k));
