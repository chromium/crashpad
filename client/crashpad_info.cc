...
CrashpadInfo::CrashpadInfo()
    : signature_(kSignature),
      ...
      user_data_stream_head_(nullptr)
...
void CrashpadInfo::AddUserDataStream(uint32_t stream_type,
                                     const void* data,
                                     size_t size) {
  auto to_be_added = new internal::UserDataStreamListEntry();
  to_be_added->next = base::checked_cast<uint64_t>(
      reinterpret_cast<uintptr_t>(user_data_stream_head_));
  ...
  user_data_stream_head_ = to_be_added;
}
