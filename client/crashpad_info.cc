...
#if defined(OS_POSIX)
__attribute__((
// Put the structure in a well-known section name where it can be easily found
// without having to consult the symbol table.
#if defined(OS_MACOSX)
...
// The “used” attribute prevents the structure from being dead-stripped.
               used,
// There’s no need to expose this as a public symbol from the symbol table. All
// accesses from the outside can locate the well-known section name.
               visibility("hidden")
             ))
#elif defined(OS_WIN)
