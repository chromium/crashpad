...
//! when reading.
//!
//! Both the keys and values are `NUL`-terminated strings.
//!
...
class Annotations {
 ...
 private:
  ...
  //! \brief Adds a new entry to the head of the list.
  //!
  //! There must not be an existing entry for \a key in the list. The head is
  //! swapped using an atomic operation so that the list always represents a
  //! valid structure when viewed externally.
  void AddEntry(const char* key, const char* value);

  //! \brief Replaces the \a value store in \a entry. The value is swapped in
  //!     atomically so the entry is always valid when viewed externally.
