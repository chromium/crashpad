...
class CrashReportDatabaseMac : public CrashReportDatabase {
 ...
 private:
  ...
  //!
  //! Assums that report is locked.
  //!
  //! \param[in out] report_path The path of the file to mark completed. Will
  //!     carry the correct file path after the function call.
  //!
  //! \return The operation status code.
...
CrashReportDatabaseMac::RecordUploadAttempt(const Report* report,
  ...
  if (successful) {
    CrashReportDatabase::OperationStatus os =
        MarkReportCompletedLocked(report_path);
    if (os != kNoError)
      return os;
  }
...
CrashReportDatabase::OperationStatus CrashReportDatabaseMac::SkipReportUpload(
  ...
  return MarkReportCompletedLocked(report_path);
}
...
CrashReportDatabase::OperationStatus
CrashReportDatabaseMac::MarkReportCompletedLocked(base::FilePath& report_path) {
