...
FileWriter* CrashReportDatabase::NewReport::AddAttachment(
  ...
  base::FilePath attachments_dir =
      static_cast<CrashReportDatabaseGeneric*>(database_)->AttachmentsPath(
          uuid_);
  if (!LoggingCreateDirectory(attachments_dir, FilePermissions::kOwnerOnly, true)) {
    return nullptr;
  }
  ...
  removers_.emplace_back(ScopedRemoveFile(path));
...
OperationStatus CrashReportDatabaseGeneric::FinishedWritingCrashReport(
  ...
  report->Writer()->Close();
  ScopedRemoveFile& remover =
      report->removers_[CrashReportDatabase::NewReport::kIndexOfMainReport];
  if (!MoveFileOrDirectory(remover.get(), path)) {
    return kFileSystemError;
  }
  ...
  for (size_t i = NewReport::kIndexOfMainReport + 1;
       i < report->removers_.size();
       ++i) {
    report->writers_[i]->Close();
    ignore_result(report->removers_[i].release());
  }
