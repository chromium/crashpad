...
OperationStatus CrashReportDatabaseGeneric::RecordUploadComplete(
    std::unique_ptr<const UploadReport> report,
    const std::string& id) {
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
  OperationStatus os =
...
OperationStatus CrashReportDatabaseGeneric::CheckoutReport(const UUID& uuid,
                                                    ReportState state,
                                                    base::FilePath* path,
                                                    ScopedLockFile* lock_file,
                                                    Report* report) {
...
OperationStatus CrashReportDatabaseGeneric::ReportsInState(
  ...
  if (!reader.Open(dir_path)) {
    return CrashReportDatabaseGeneric::kDatabaseError;
  }
...
// static
bool CrashReportDatabaseGeneric::CleaningReadMetadata(const base::FilePath& path,
                                               Report* report) {
...
std::unique_ptr<CrashReportDatabase> InitializeInternal(
  ...
  return database->Initialize(path, may_create)
             ? std::move(database)
             : nullptr;
}
