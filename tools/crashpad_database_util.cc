...
// Converts |string| to |time_tt|, returning true if a conversion could be
// performed, and false without setting |boolean| if no conversion could be
...
// "never" is recognized as converted to a |time_tt| value of 0; "now" is
...
// ambiguous.
bool StringToTime(const char* string, time_t* time_tt, bool utc) {
  ...
  for (size_t index = 0; index < arraysize(kFormats); ++index) {
    ...
    if (strptime_result == end) {
      time_t test_time_tt;
      if (utc) {
        test_time_tt = timegm(&time_tm);
      } else {
        test_time_tt = mktime(&time_tm);
      }

      // mktime() is supposed to set errno in the event of an error, support for
      // this is spotty, so there’s no way to distinguish between a true time_t
      // of -1 (1969-12-31 23:59:59 UTC) and an error. Assume error.
      //
...
std::string TimeToString(time_t time_tt, bool utc) {
  ...
  tm time_tm;
  if (utc) {
    PCHECK(gmtime_r(&time_tt, &time_tm));
  } else {
    PCHECK(localtime_r(&time_tt, &time_tm));
  }
