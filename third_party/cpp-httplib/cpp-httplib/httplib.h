...
inline int select_read(socket_t sock, size_t sec, size_t usec)
{
    ...
    LOG(ERROR) << "going to select_read(), sock=" << sock;
    int rv = select(sock + 1, &fds, NULL, NULL, &tv);
    LOG(ERROR) << " select returned " << rv;
    return rv;
}
...
inline bool read_and_close_socket(socket_t sock, bool keep_alive, T callback)
{
    ...
    if (keep_alive) {
        ...
        LOG(ERROR) << "going to select_read in main loop";
        while (count > 0 &&
               detail::select_read(sock,
                   CPPHTTPLIB_KEEPALIVE_TIMEOUT_SECOND,
                   CPPHTTPLIB_KEEPALIVE_TIMEOUT_USECOND) > 0) {
            LOG(ERROR) << "  in here count " << count;
            SocketStream strm(sock);
            auto last_connection = count == 1;
            auto connection_close = false;

            LOG(ERROR) << "last_connection=" << last_connection;
            ret = callback(strm, last_connection, connection_close);
            LOG(ERROR) << "last_connection now =" << last_connection;
            LOG(ERROR) << "connection_close now =" << connection_close;
...
inline bool Server::listen_internal()
{
    ...
    for (;;) {
        LOG(ERROR) << "going to select read on svr_sock_ with 100000 timeout";
        ...
        if (sock == INVALID_SOCKET) {
            LOG(ERROR) << "sock==INVALID_SOCKET";
            if (svr_sock_ != INVALID_SOCKET) {
                LOG(ERROR) << "svr_sock_!=INVALID_SOCKET!!!!!!!!!!";
                detail::close_socket(svr_sock_);
                ret = false;
            } else {
                LOG(ERROR) << "svr_sock_==INVALID_SOCKET";
    ...
    // TODO: Use thread pool...
    for (;;) {
        LOG(ERROR) << "waiting for service threads...";
    ...
    T callback)
{
    ...
    if (keep_alive) {
        ...
        while (count > 0 &&
            ...
            count--;
            LOG(ERROR) << "going to re select_read at bottom of loop";
        }
    } else {
        SSLSocketStream strm(sock, ssl);
        auto dummy_connection_close = false;
