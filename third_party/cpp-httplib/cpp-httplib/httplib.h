...
bool read_content_without_length(Stream& strm, T& x)
{
    ...
    for (;;) {
        ...
        total += n;
        fprintf(stderr, "read so far %d\n", total);
        fprintf(stderr, "body now '''%s'''\n", x.body.c_str());
        if (n < 0) {
            return false;
        } else if (n == 0) {
            return true;
...
bool read_content_chunked(Stream& strm, T& x)
{
    ...
    fprintf(stderr, "read %d chunk_len\n", chunk_len);
    while (chunk_len > 0){
        ...
        fprintf(stderr, "next %d chunk_len\n", chunk_len);
    }

    fprintf(stderr, "done chunked loop\n");
    if (chunk_len == 0) {
...
bool read_content(Stream& strm, T& x, Progress progress = Progress())
{
    ...
    if (len) {
        ...
        fprintf(stderr, "zippy!\n");
        for (const auto& h: x.headers) {
...
{
    fprintf(stderr, "recv of %zu\n", size);
    int ret = recv(sock_, ptr, size, 0);
