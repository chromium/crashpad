...
bool GetChildTestFunctionName(std::string* child_func_name) {
  std::string look_for =
      std::string(crashpad::test::internal::kChildTestFunction) + "=";
  for (const auto& it : crashpad::test::GetMainArguments()) {
    if (it.substr(0, look_for.size()) == look_for) {
      *child_func_name = it.substr(look_for.size());
      return true;
    }
  }
  return false;
...
int main(int argc, char* argv[]) {
  ...
  if (GetChildTestFunctionName(&child_func_name)) {
    return crashpad::test::internal::InvokeMultiprocessChild(child_func_name);
  }
