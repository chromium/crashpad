...
//! \brief Manages a multiprocess test on Windows with a parent-created temp
//!     directory.
class WinMultiprocessWithTempDir : public WinMultiprocess {
 ...
 protected:
  ...
  //! \brief Waits for all child processes of \p parent.
  //!
  //! Waits for the processes directly created by \a parent - and specifically
  //! not their offspring. For this to work without race, \a parent has to be
  //! suspended or have exited.
  //!
  //! \param[in] parent The parent process of the processes to wait for.
  static void WaitForAllChildProcessesOf(HANDLE parent);
