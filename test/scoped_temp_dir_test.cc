...
void CreateFile(const base::FilePath& path) {
  ...
  ASSERT_EQ(0, IGNORE_EINTR(close(fd))) << ErrnoMessage("close") << " "
                                        << path.value();
#elif defined(OS_WIN)
...
#if defined(OS_POSIX)
  ASSERT_EQ(0, mkdir(path.value().c_str(), 0755)) << ErrnoMessage("mkdir")
                                                  << " " << path.value();
#elif defined(OS_WIN)
  ASSERT_EQ(0, _wmkdir(path.value().c_str())) << ErrnoMessage("_wmkdir") << " "
                                              << path.value();
#else
#error "Not implemented"
#endif
