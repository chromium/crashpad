...
zx_status_t AddPipe(fdio_spawn_action_t* action, int target_fd, int* fd_out) {
    zx_handle_t handle;
    uint32_t id;
...
void MultiprocessExec::MultiprocessChild() {
  ...
  zx_status_t status = fdio_spawn_etc(
      ZX_HANDLE_INVALID, flags, command_.value().c_str(), argv_.data(), nullptr,
      kActionCount, actions, &child, error_message);
