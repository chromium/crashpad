...
#if defined(OS_MACOSX) && MAC_OS_X_VERSION_MIN_REQUIRED < MAC_OS_X_VERSION_10_13
  struct stat st;
  if (lstat(path.value().c_str()) != 0) {
    PLOG(ERROR) << "lstat " << path.value();
    return false;
  }
  timeval times[2];
  TimespecToTimeval(st.st_atimespec, &times[0]);
  TimespecToTimeval(mtime, &times[1]);
