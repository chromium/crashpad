...
const char kChildTestFunction[] = "--child-test-function";
...
void MultiprocessExec::SetChildTestMainProc(const std::string& function_name) {
  std::string argv0 = GetMainArguments()[0];
  std::vector<std::string> rest(GetMainArguments().begin() + 1,
                                GetMainArguments().end());
  rest.push_back(internal::kChildTestFunction + std::string("=") +
                 function_name);
