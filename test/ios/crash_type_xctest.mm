...
#import "test/ios/host/shared_object.h"
...
- (void)handleCrashUnderSymbol:(id)arg1 {
  // Do Nothing.
  NSLog(@"Crash detected and swizzled away.");
}

+ (void)setUp {
  // Swizzle away crash reporting.
  ...
  // Override EDO default error handler.
...
- (void)testEDO {
  SharedObject* rootObject = [EDOClientService rootObjectWithPort:12345];
...
- (void)testSegv {
  ...
  XCTAssertTrue([_app waitForState:XCUIApplicationStateNotRunning timeout:5]);
