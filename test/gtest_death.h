...
//! crashes that occur in \a statement. `CHECK()` failures normally show up as
//! crashes to the system’s crash reporter, but it is undesirable for
//! intentional `ASSERT_DEATH_CRASH()` crashes to be handled by any crash
//! reporter.
//!
...
//! crashes that occur in \a statement. `CHECK()` failures normally show up as
//! crashes to the system’s crash reporter, but it is undesirable for
//! intentional `EXPECT_DEATH_CRASH()` crashes to be handled by any crash
//! reporter.
//!
