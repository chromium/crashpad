...
//! Reports generated for code that crashes intentionally has no value, and many
//! Crashpad tests do crash intentionally.
//!
...
class ExceptionSwallower {
 public:
  ...
  //!
  //! \sa Parent_PrepareForGtestDeathTest
  ...
  //! starting its own exception swallower server process.
 ...
 private:
  ...
  // swallower server process writes to it when it’s set up, allowing the parent
  // test process to synchronize with it. It remains open in the parent test
  // process so that the exception swallower server process can observe, based
  // on reading end-of-file, when the parent test process has died. The
  // exception swallower server process uses this as a signal that it’s safe to
  // exit.
  base::ScopedFD fd_;
