...
void ExceptionSwallower::SwallowExceptions() {
  ...
  // exception may be transformed into an EXC_CORPSE_NOTIFY, which would be sent
  // to the task’s or host’s EXC_CORPSE_NOTIFY handler, the system’s crash
  // reporter. Swallowing EXC_CORPSE_NOTIFY at the task level prevents this from
  // happening.
