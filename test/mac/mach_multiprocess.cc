...
void MachMultiprocess::MultiprocessParent() {
  ...
  // value.
  EXPECT_EQ(
            message.header.msgh_bits,MACH_MSGH_BITS(MACH_MSG_TYPE_MOVE_SEND, MACH_MSG_TYPE_MOVE_SEND) |
  ...
  EXPECT_EQ(
            message.port_descriptor.disposition,implicit_cast<mach_msg_type_name_t>(MACH_MSG_TYPE_MOVE_SEND));
  ASSERT_EQ(
            message.port_descriptor.type,implicit_cast<mach_msg_descriptor_type_t>(MACH_MSG_PORT_DESCRIPTOR));
  ASSERT_EQ(
            message.audit_trailer.msgh_trailer_type,implicit_cast<mach_msg_trailer_type_t>(MACH_MSG_TRAILER_FORMAT_0));
  ASSERT_EQ(
            message.audit_trailer.msgh_trailer_size,sizeof(message.audit_trailer));
