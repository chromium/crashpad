...
base::FilePath Paths::Executable() {
  ...
  // string. Coincidentally, PAGE_SIZE and PATH_MAX are normally the same
  // value, although neither is strictly a limit on the length of a pathname.
  //
  // On Android, the smaller of PAGE_SIZE and PATH_MAX actually does serve as
  // an effective limit on the length of an executable’s pathname. See Android
  // 7.0.0 bionic/linker/linker.cpp get_executable_path(), which aborts via
  // __libc_fatal() if the “exe” link can’t be read into a PATH_MAX-sized
  // buffer.
  std::string exe_path(std::min(implicit_cast<size_t>(PAGE_SIZE),
