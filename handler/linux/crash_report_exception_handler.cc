...
bool WriteMinidumpLogFromFile(base::FilePath& file_path) {
  FileReader file_reader;
  if (!file_reader.Open(file_path))
    return false;
...
bool CrashReportExceptionHandler::WriteMinidumpToDatabase(
  ...
  bool write_minidump_to_log_succeed = true;
  if (write_minidump_to_log) {
    CrashReportDatabase::Report report;
    database_status = database_->LookUpCrashReport(uuid, &report);
    if (database_status != CrashReportDatabase::kNoError) {
      LOG(ERROR) << "LookUpCrashReport failed";
      write_minidump_to_log_succeed = false;
    }
    if (!WriteMinidumpLogFromFile(report.file_path)) {
  ...
  return write_minidump_to_log_succeed;
}
