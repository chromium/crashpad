...
class ExceptionHandlerServer {
 ...
 private:
  ...
  bool InstallControlSocket(ScopedFileHandle socket);
  bool InstallRegistrationSocket(const RegistrationRequest& registration,
                                 ScopedFileHandle socket);
  void HandleEvent(Event* event, uint32_t event_type);
  void ReceiveControlMessage(Event* event);
  void ReceiveCrashMessage(Event* event);
