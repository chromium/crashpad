 ...
 public:
  class Delegate {
   public:
    ...
    //!
    //! \param[in] registration Registration infor the crashing client.
    //! \param[in] broker_sock A socket connected to a broker for the crashing
    //!     client, only valid if registration.use_broker is `true`.
    virtual void HandleException(const Registration& registration,
                                 int broker_sock) = 0;
  };
