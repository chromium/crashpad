 ...
 public:
  class Delegate {
   public:
    ...
    //! \param[in] requesting_thread_stack_address A stack address for the
    //!     thread that sent the crash dump request.
    //! \param[out] requesting_thread_id The thread ID of the thread which
    //!     requested the crash dump if not `nullptr` and
    //!     \a requesting_thread_stack_address isn't 0. Set to -1 if the thread
 ...
 private:
  struct Event {
    enum class Type {
      ...
      // A message from a client on a dedicated socket connection.
      kClientMessage,
