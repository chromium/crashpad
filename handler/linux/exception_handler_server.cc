...
PtraceScope GetPtraceScope() {
  ...
  if (contents.back() == '\n') {
    ...
    LOG(WARNING) << "missing newline";
  }

  int ptrace_scope;
  if (!base::StringToInt(contents, &ptrace_scope)) {
    LOG(ERROR) << "couldn't read ptrace scope";
    return PtraceScope::kUnknown;
  }
...
bool HaveCapSysPtrace() {
  // TODO(jperaza): libcap is incomplete on Android a compat implementation of
  // cap_get_flag is needed.
  return false;
}
#else
...
bool ExceptionHandlerServer::ReceiveClientMessage(Event* event) {
  ...
  switch (client_msg->type) {
    case ClientToServerMessage::kCrashDumpRequest: {
      ...
      switch (strategy_decider_->ChooseStrategy(event->fd.get(),
                                                *client_credentials)) {
        ...
        case PtraceStrategyDecider::Strategy::kNoPtrace:
          return SendMessageToClient(
              event->fd.get(), ServerToClientMessage::kTypeCrashDumpFailed);
