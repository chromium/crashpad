...
bool ExceptionHandlerServer::InitializeWithClient(
  ...
  if(!InstallClientRegistration(registration, std::move(sock))) {
    return false;
  }
...
bool ExceptionHandlerServer::ReceiveClientMessage(Event* event) {
  ...
  if (client_msg->type == ClientToServerMessage::kRegistration) {
    ...
    return LoggingWriteFile(event->fd.get(), &success, sizeof(success));
  }

  if (client_msg->type == ClientToServerMessage::kCrashDumpRequest) {
    bool success = event->registration.use_broker
                       ? delegate_->HandleExceptionWithBroker(
                             event->registration.client_process_id,
