...
class ExceptionHandlerServerTest : public testing::Test {
 ...
 protected:
  void SetUp() override {
    ...
    ASSERT_TRUE(server_.Initialize(socks[1]));
...
TEST_F(ExceptionHandlerServerTest, Registration) {
  ...
  char c;
  LoggingWriteFile(reg_client_sock.get(), &c, sizeof(c));
...
TEST_F(ExceptionHandlerServerTest, Shutdown) {
  ASSERT_TRUE(ShutdownHandler(SockToHandler()));
}
