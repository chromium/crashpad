...
TEST_F(ExceptionHandlerServerTest, Registration) {
  ...
  ASSERT_TRUE(RegisterWithHandler(
      SockToHandler(), registration, std::move(reg_server_sock)));
  ...
  ASSERT_TRUE(Delegate()->WaitForException(
      5.0, &last_client, &last_address, &last_broker));
  ...
  EXPECT_TRUE(WaitForCrashDump(reg_client_sock.get()));
}
