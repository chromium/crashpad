...
class TestDelegate : public ExceptionHandlerServer::Delegate {
 public:
  ...
  bool HandleException(pid_t client_process_id,
    ...
    *requesting_thread_id = client_process_id;
  ...
  class CrashDumpTest : public Multiprocess {
   public:
    ...
    void MultiprocessChild() override {
      ...
      pid_t tracer_pid =
          server_test_->use_multi_client_socket_ ? getppid() : getpid();

      ScopedPrSetPtracer set_ptracer(tracer_pid, /* may_log= */ true);
      ...
      ASSERT_EQ(
          ...
          server_test_->use_multi_client_socket_ && !succeeds_ ? EAGAIN : 0);
    }
