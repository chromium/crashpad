...
int HandlerMain(int argc,
  ...
  while ((opt = getopt_long(argc, argv, "", long_options, nullptr)) != -1) {
    switch (opt) {
      ...
      case kOptionSanitizationInformation: {
        if (!StringToNumber(optarg, &options.sanitization_information_address)) {
          ToolSupport::UsageHint(
              me, "failed to parse --sanitization-information");
          return ExitFailure();
        }
        break;
      }
  ...
  if (!options.exception_information_address &&
    ...
    ToolSupport::UsageHint(
        me,
        "--trace-parent-with-exception or --initial_client_fd is required");
    return ExitFailure();
  }
  ...
  if (options.exception_information_address) {
    ...
    info.sanitization_information_address = options.sanitization_information_address;
