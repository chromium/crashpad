...
struct Options {
  ...
  bool no_write_minidump_to_database;
#endif  // OS_ANDROID
#elif defined(OS_WIN)
...
int HandlerMain(int argc,
  ...
  while ((opt = getopt_long(argc, argv, "", long_options, nullptr)) != -1) {
    switch (opt) {
      ...
      case kOptionNoWriteMinidumpToDatabase: {
        options.no_write_minidump_to_database = true;
        break;
      }
#endif  // OS_ANDROID
#if defined(OS_WIN)
...
#if defined(OS_ANDROID)
  if (!options.write_minidump_to_log && options.no_write_minidump_to_database) {
    ToolSupport::UsageHint(me,
  ...
  exception_handler = std::make_unique<CrashReportExceptionHandler>(
      ...
      user_stream_sources,
#if defined(OS_ANDROID)
      !options.no_write_minidump_to_database,
