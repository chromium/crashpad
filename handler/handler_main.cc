...
// Handles a WM_CLOSE message sent to a message-only window.
class TerminateHandler final : public MessageWindowCloseWatcher {
 public:
  TerminateHandler() : MessageWindowCloseWatcher() {}
  ~TerminateHandler() override {}

 protected:
  // MessageWindowCloseWatcher:
  void MessageWindowCloseWatcherEvent(Event event) override {
    if (event == Event::kReceivedClose) {
      MetricsRecordExit(Metrics::LifetimeMilestone::kTerminated);
    }
  }
