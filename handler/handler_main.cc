...
// the conevenience of callers in main() which can simply write “return
// ExitFailure();”.
int ExitFailure() {
...
using SigactionFunction = void (*)(int, siginfo_t*, void*);

void InstallSignalHandler(const std::vector<int>& signals,
                          SigactionFunction handler) {
  struct sigaction sa = {};
