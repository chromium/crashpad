...
bool CrashReportExceptionHandler::HandleException(uint32_t type,
  ...
  // else when it does.
  zx_handle_t process_raw;
  zx_status_t status = zx_object_get_child(
      ZX_HANDLE_INVALID, pid, ZX_RIGHT_SAME_RIGHTS, &process_raw);
  if (status != ZX_OK) {
    Metrics::ExceptionCaptureResult(Metrics::CaptureResult::kSnapshotFailed);
    ZX_LOG(ERROR, status) << "zx_object_get_child process";
    return false;
  }
  base::ScopedZxHandle process(process_raw);
  ...
  if (client_options.crashpad_handler_behavior != TriState::kDisabled) {
    if (!process_snapshot.InitializeException(type, process.get(), tid)) {
