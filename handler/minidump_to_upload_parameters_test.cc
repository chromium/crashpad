...
TEST(MinidumpToUploadParameters, PrecedenceRules) {
  ...
  EXPECT_EQ(10u, upload_parameters.size());
  EXPECT_EQ("abcdefg", upload_parameters["process-1"]);
  EXPECT_EQ("process", upload_parameters["first"]);
  EXPECT_EQ("goat", upload_parameters["module-0-1"]);
  EXPECT_EQ("doge", upload_parameters["module-0-2"]);
  EXPECT_EQ("star", upload_parameters["module-0-3"]);
  EXPECT_EQ("module 0", upload_parameters["second"]);
  EXPECT_EQ("bear", upload_parameters["module-1-1"]);
  EXPECT_EQ("moon", upload_parameters["module-1-4"]);
  EXPECT_EQ(
      "list-module-0-1\nlist-module-0-2\n"
      "list-module-1-1\nlist-module-1-2",
      upload_parameters["list_annotations"]);
  EXPECT_EQ(guid, upload_parameters["guid"]);
}
