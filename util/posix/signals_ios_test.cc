...
//ninja -C out/Debug-iphoneos/ crashpad_util_test &&   ideviceinstaller -i out/Debug-iphoneos/crashpad_util_test.app/ && idevice-app-runner -s "com.google.gtest.crashpad-util-test" --args "--gtest_filter=*SignalsiOS*"
...
#pragma clang diagnostic pop
    ...
    case SIGTRAP: {
      ...
      g_advance = 4;
      fprintf(stderr, "FOOFOO: before brk 0\n");
...
class SignalsiOSTest {
 ...
 private:
  static void SignalHandler_FixUps(int sig, siginfo_t* siginfo, void* context) {
    fprintf(stderr, "FOOFOO: hello\n");
    g_didHandle = true;
    if (g_advance > 0)
#if TARGET_IPHONE_SIMULATOR
      ((ucontext_t*)context)->uc_mcontext->__ss.__rip += g_advance;
#else
      ((ucontext_t*)context)->uc_mcontext->__ss.__pc += g_advance;
#endif
  }
...
TEST(SignalsiOS, Cause_HandlerContinues) {
  for (int sig = 1; sig < NSIG; ++sig) {
    ...
    g_didHandle = false;
    fprintf(stderr, "FOOFOO: testing %d\n\n", sig);
