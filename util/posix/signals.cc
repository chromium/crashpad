...
bool Signals::WillSignalReraiseAutonomously(const siginfo_t* siginfo) {
  ...
  // handler.
  //
  // Unfortunately, on macOS, when SIGBUS (on all CPUs) and SIGILL ans SIGSEGV
  ...
  // asynchronously, these signals will not re-raise itself autonomously, but
  // this function (acting on information from the kernel) behaves as though it
  // will. This isn’t ideal, but these signals occurring asynchronously is an
  // unexpected condition. The alternative, to never treat these signals as
