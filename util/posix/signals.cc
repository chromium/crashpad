...
bool IsSignalInSet(int sig, const std::set<int>& signals) {
  return signals.find(sig) != signals.end();
}
...
bool Signals::IsCrashSignal(int sig) {
  return IsSignalInSet(
      sig,
      std::set<int>(kCrashSignals, kCrashSignals + arraysize(kCrashSignals)));
}
