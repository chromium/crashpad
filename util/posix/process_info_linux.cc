...
bool ProcessInfo::Initialize(pid_t pid) {
...

  {
    ...
    FileReader stat_file;
    if (!stat_file.Open(base::FilePath(path))) {
      return false;
    }
    ...
    while ((length = stat_file.Read(buffer, sizeof(buffer))) > 0) {
      stat_contents.append(buffer, length);
    }
    if (length < 0) {
      return false;
    }
    ...
    for (int index = 1;
         index < 22 && stat_pos < stat_contents.size();
         ++index) {
  ...
  if (pid == mypid) {
    ...
    is_64_bit_ = am_64_bit ? iov.iov_len == sizeof(regbuf.regs)
                           : iov.iov_len != sizeof(regbuf.regs);
...
bool ProcessInfo::Arguments(std::vector<std::string>* argv) const {
  ...
  FileReader cmd_file;
  if (!cmd_file.Open(base::FilePath(path))) {
    return false;
  }
  ...
  std::string command;
  FileOperationResult length;
  while ((length = cmd_file.Read(buffer, sizeof(buffer))) > 0) {
