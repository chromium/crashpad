...
bool ProcessInfo::Initialize(pid_t pid) {
...

  {
    ...
    FileLineReader status_file_line_reader(&status_file);
    ...
    std::string line;
    FileLineReader::Result result;
    while ((result = status_file_line_reader.GetLine(&line)) ==
           FileLineReader::Result::kSuccess) {
      ...
      // Make sure that there’s no unrecognized trailing junk on a line that was
      // processed.
      if (understood_line &&
          (!AdvancePastPrefix(&line_c, "\n") || line_c != &line.back() + 1)) {
        LOG(ERROR) << "format error";
    ...
    if (result != FileLineReader::Result::kEndOfFile) {
      return false;
    }
