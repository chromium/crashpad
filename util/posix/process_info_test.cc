...
void TestProcessSelfOrClone(const ProcessInfo& process_info) {
  ...
  // gtest argv processing consumes arguments from expect_argv. expect_argv[0]
  // will be left intact, so expect_argc will be at least 1. Those can always be
  // compared against argc and argv, which may reflect the state of those
  // variables when the program was invoked.
  ASSERT_GE(*expect_argc, 1);
  ASSERT_GE(argc, 1);
  EXPECT_GE(argc, *expect_argc);

  EXPECT_EQ(std::string((*expect_argv)[0]), argv[0]);

  // Everything else in expect_argv should have a match in argv too, but things
  // may have moved around.
  for (int expect_index = 1; expect_index < *expect_argc; ++expect_index) {
    const char* expect_arg = (*expect_argv)[expect_index];
    SCOPED_TRACE(base::StringPrintf(
        "expect_index %d, expect_arg %s", expect_index, expect_arg));
    EXPECT_NE(argv.end(), std::find(argv.begin(), argv.end(), expect_arg));
  }
}
