...
class FileOutputStream : public OutputStreamInterface {
 public:
  //! \param[in] file_name The file that this object writes to.
  explicit FileOutputStream(const base::FilePath& file_name);
  ~FileOutputStream();
 ...
 private:
  ...
  FileWriter writer_;
  InitializationState initialized_;  // protects writer_
  bool flush_needed_;
  bool flushed_;
