...
const size_t kShortDataLength = 10;
const size_t kLongDataLength = 4096 * 10;
...
class ZlibOutputStreamTest : public testing::Test {
 public:
  ...
  const uint8_t* BuildDeterministicInput(size_t size) {
    ...
    while (size-- > 0)
      deterministic_input_.get()[size] = static_cast<uint8_t>(size);
    return deterministic_input_.get();
  }

  const uint8_t* BuildRandomInput(size_t size) {
    input_ = std::make_unique<uint8_t[]>(size);
    while (size-- > 0)
      input_.get()[size] = base::RandInt(0, 256);
    return input_.get();
  }
...
TEST_F(ZlibOutputStreamTest, WriteDeterministicShortData) {
  ...
  EXPECT_EQ(kShortDataLength, test_output_stream().last_written_data().size());
  EXPECT_EQ(0,
            memcmp(test_output_stream().last_written_data().data(),
                   input,
                   kShortDataLength));
}
...
TEST_F(ZlibOutputStreamTest, WriteDeterministicLongDataMultipleTimes) {
  ...
  std::vector<size_t> write_len = {4, 96, 40, 4096 * 10 - 4 - 96 - 40};

  size_t index = 0;
  while (!write_len.empty()) {
    EXPECT_TRUE(zlib_output_stream()->Write(input + index, write_len.back()));
    index += write_len.back();
    write_len.pop_back();
  }
  EXPECT_TRUE(zlib_output_stream()->Flush());
  EXPECT_EQ(test_output_stream().all_data().size(), kLongDataLength);
...
TEST_F(ZlibOutputStreamTest, WriteLongDataMultipleTimes) {
  ...
  while (index < kLongDataLength) {
    size_t cur = base::RandInt(1, 4096 * 2);
    cur = cur < kLongDataLength - index ? cur : kLongDataLength - index;
    EXPECT_TRUE(zlib_output_stream()->Write(input + index, cur));
    index += cur;
  }
  EXPECT_TRUE(zlib_output_stream()->Flush());
  EXPECT_EQ(test_output_stream().all_data().size(), kLongDataLength);
...
TEST_F(ZlibOutputStreamTest, NoWriteOrFlush) {
  EXPECT_FALSE(test_output_stream().write_called());
  EXPECT_FALSE(test_output_stream().flush_called());
...
TEST_F(ZlibOutputStreamTest, FlushWithoutWrite) {
  EXPECT_TRUE(zlib_output_stream()->Flush());
  EXPECT_FALSE(test_output_stream().write_called());
  EXPECT_TRUE(test_output_stream().flush_called());
  EXPECT_TRUE(test_output_stream().all_data().empty());
}
