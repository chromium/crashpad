...
class ZlibOutputStreamTest : public testing::Test {
 public:
  ZlibOutputStreamTest() : input_() {
    auto test_helper = std::make_unique<OutputStreamTestHelper>();
 ...
 protected:
  ...
  ZlibOutputStream* zlib_output_stream() { return zlib_output_stream_.get(); }

 private:
  OutputStreamTestHelper* test_helper_;
...
TEST_F(ZlibOutputStreamTest, CallFlushWithoutWrite) {
  EXPECT_TRUE(zlib_output_stream()->Flush());
}
