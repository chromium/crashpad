...
std::string ConvertToString(const std::vector<uint8_t>& src) {
  return std::string(FromPointerCast<const char*>(src.data()), src.size());
}
...
class LogOutputStreamTest : public testing::Test {
 ...
 protected:
  void SetUp() override {
    ...
    log_stream_->set_output_stream_for_testing(std::move(output_stream));
  }
...
TEST_F(LogOutputStreamTest, WriteShotLog) {
...
TEST_F(LogOutputStreamTest, WriteAbort) {
  size_t input_length = kOutputCap + 1024 * 2;
  const uint8_t* input = BuildDeterministicInput(input_length);
  ...
  // TestOutputStream::Write() method has been called, invoking Flush()
  // to avoid DCHECK(!flush_needed_) failure in destructer().
  test_output_stream()->Flush();
}

TEST_F(LogOutputStreamTest, FlushAbort) {
  size_t input_length = kOutputCap + 1024;
  const uint8_t* input = BuildDeterministicInput(input_length);
  EXPECT_TRUE(log_stream()->Write(input, input_length));
