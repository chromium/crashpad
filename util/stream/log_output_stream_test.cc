...
class LogOutputStreamTestLogger : public LogOutputStream::Logger {
 public:
  LogOutputStreamTestLogger(std::string* logging_destination)
      : logging_destination_(logging_destination) {}
  ~LogOutputStreamTestLogger() = default;

  int Log(const char* buf) override {
    size_t len = strnlen(buf, kLineBufferSize+1);
    EXPECT_LE(len, kLineBufferSize);
    logging_destination_->append(buf);
    return len;
  }
 ...
 protected:
  void SetUp() override {
    log_stream_ = std::make_unique<LogOutputStream>(
        std::make_unique<LogOutputStreamTestLogger>(&test_log_output_));
  }
