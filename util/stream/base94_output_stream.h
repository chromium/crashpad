...
//! \brief This class implements Base94 encoding/decoding, it uses all printable
//! characters except space for encoding, and no padding is required.
//!
...
//! symbols if its low 13-bit less than 644 ( = 94^2 - 2^13), otherwise 13-bit
//! data is encoded.
class Base94OutputStream : public OutputStreamInterface {
 ...
 private:
  ...
  std::unique_ptr<uint8_t[]> table_;
  std::vector<uint8_t> buffer_;
  InitializationState initialized_;  // protects table_
  uint32_t bit_buf_;
