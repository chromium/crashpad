...
class LogFileProcessorTest : public testing::Test {
 public:
  ...
  LogFileProcessor* encode_processor() const {
    return encode_processor_.get();
  }

  LogFileProcessor* decode_processor() const {
    return decode_processor_.get();
  }
 ...
 protected:
  void SetUp() override {
    ...
    encoded_ = base::FilePath(temp_dir_->path().Append(FILE_PATH_LITERAL("encoded")));
    decoded_ = base::FilePath(temp_dir_->path().Append(FILE_PATH_LITERAL("decoded")));
    encode_processor_ = std::make_unique<LogFileProcessor>(LogFileProcessor::Mode::kEncode, org_, encoded_);
    decode_processor_ = std::make_unique<LogFileProcessor>(LogFileProcessor::Mode::kDecode, encoded_, decoded_);
  }
