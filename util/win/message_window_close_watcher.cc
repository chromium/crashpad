...
#include "util/win/message_window_close_watcher.h"
...
MessageWindowCloseWatcher::MessageWindowCloseWatcher()
...
// static
DWORD WINAPI MessageWindowCloseWatcher::ThreadMain(void* argument) {
  const DWORD kSuccess = 0;
  const DWORD kFailure = 1;

  MessageWindowCloseWatcher* self =
      reinterpret_cast<MessageWindowCloseWatcher*>(argument);

  WNDCLASS wndclass = {};
  wndclass.lpfnWndProc = WindowProc;
  wndclass.hInstance = reinterpret_cast<HMODULE>(&__ImageBase);
  wndclass.lpszClassName = L"crashpad_MessageWindowCloseWatcher";
  ...
  self->window_ = CreateWindow(
      MAKEINTATOM(atom), nullptr, 0, 0, 0, 0, 0, 0, 0, 0, self);
  if (!self->window_) {
    PLOG(ERROR) << "CreateWindow";
    return kFailure;
  }

  self->MessageWindowCloseWatcherEvent(Event::kStartedWatching);
...

  {
    ...
    // scope. That ensures that MessageWindowCloseWatcherEvent() doesn’t mess
    // with GetLastError() before one of the PLOG(ERROR)s that follow has a
    // chance to figure out the reason for a failure.
    auto lambda = [self]() {
      self->MessageWindowCloseWatcherEvent(Event::kStoppedWatching);
    };
...
// static
LRESULT CALLBACK MessageWindowCloseWatcher::WindowProc(HWND window,
                                                       UINT message,
                                                       WPARAM w_param,
                                                       LPARAM l_param) {
  ...
  if (!self && message == WM_CREATE) {
    ...
    self = reinterpret_cast<MessageWindowCloseWatcher*>(create->lpCreateParams);
  ...
  if (self) {
    if (message == WM_ENDSESSION) {
      // If the session is ending, post a close message and send the
      // notification when it’s handled.
      if (!PostMessage(self->window_, WM_CLOSE, 0, 0)) {
        PLOG(ERROR) << "PostMessage";
      }
    } else if (message == WM_CLOSE) {
      self->MessageWindowCloseWatcherEvent(Event::kReceivedClose);
