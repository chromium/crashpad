...
class MessageWindowCloseWatcherTest final : public MessageWindowCloseWatcher {
 public:
  MessageWindowCloseWatcherTest()
  ...
  ~MessageWindowCloseWatcherTest() override {}
  ...
  void Run() {
    ...
    ASSERT_TRUE(window);
    EXPECT_TRUE(PostMessage(window, WM_CLOSE, 0, 0));
...
TEST(MessageWindowCloseWatcher, MessageWindowCloseWatcher) {
  MessageWindowCloseWatcherTest test;
  test.Run();
}
