...
SessionEndWatcher::SessionEndWatcher() : Thread(), window_() {
  Start();
}
...
void SessionEndWatcher::ThreadMain() {
  ...
  SessionEndWatcherEvent(Event::kStartedWatching);
...

  {
    ...
    auto lambda = [this]() {SessionEndWatcherEvent(Event::kStoppedWatching);};
    ...
    if (window_) {
      ...
      // processed, DestroyWindow() will have already been called and window_
      // will be nullptr. But if the message loop terminated because it
      // processed a WM_QUIT message, the window still exists and needs to be
      // cleaned up.
