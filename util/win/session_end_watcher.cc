...
SessionEndWatcher::SessionEndWatcher()
    : Thread(),
      window_(),
      started_(CreateEvent(nullptr, true, false, nullptr)),
      stopped_(CreateEvent(nullptr, true, false, nullptr)) {
...
void SessionEndWatcher::ThreadMain() {
  auto signal_stop_lambda = [this]() {
    PLOG_IF(ERROR, !SetEvent(stopped_.get())) << "SetEvent";
  };
  ScopedCallLambda<decltype(signal_stop_lambda)> call_signal_stop(
      signal_stop_lambda);
  ...
  {
    auto signal_start_lambda = [this]() {
      PLOG_IF(ERROR, !SetEvent(started_.get())) << "SetEvent";
    };
    ScopedCallLambda<decltype(signal_start_lambda)> call_signal_start(
        signal_start_lambda);
    ...
    window_ =
        CreateWindow(MAKEINTATOM(atom), nullptr, 0, 0, 0, 0, 0, 0, 0, 0, this);
    if (!window_) {
  ...
  if (window_) {
    ...
    // processed, DestroyWindow() will have already been called and window_ will
    // be nullptr. But if the message loop terminated because it processed a
    // WM_QUIT message, the window still exists and needs to be cleaned up.
    rv = DestroyWindow(window_);
    if (!rv) {
