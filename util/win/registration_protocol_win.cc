  ...
  if (first_instance) {
    ...
    if (is_vista_or_later) {
      ...
      security_attributes.lpSecurityDescriptor =
          GetSecurityDescriptorForNamedPipeInstance(nullptr);
...
uint8_t* GetSecurityDescriptorForNamedPipeInstance(size_t* size) {
...
#pragma pack(push, 1)
  static struct SecurityDescriptorBlob {
    SECURITY_DESCRIPTOR_RELATIVE sd_rel;
    struct {
      ACL acl;
      // This is equivalent to SYSTEM_MANDATORY_LABEL_ACE, but there's no DWORD
      // offset to the SID, instead it's inline.
      ACE_HEADER header;
      ACCESS_MASK mask;
      SID sid;
    } sacl;
      ...
      // sd_rel.
      {
          0x01,  // Revision.
          0x00,  // Sbz1.
          0x8010,  // Control: (SR) Self-Relative, SP (SACL Present).
          0,  // OffsetOwner.
          0,  // OffsetGroup.
          0x14,  // OffsetSacl.
          0,  // OffsetDacl.
      ...
      // sacl.
      {
        ...
        // sid.
        {
          0x01,  // Revision.
          1,  // SubAuthorityCount.
          SECURITY_MANDATORY_LABEL_AUTHORITY,  // IdentifierAuthority.
          { 0 },  // SubAuthority.
        }
      },
    };
#pragma pack(pop)
  ...
  return reinterpret_cast<uint8_t*>(&kSecDescBlob);
}
