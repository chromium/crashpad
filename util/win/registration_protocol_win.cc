...
ScopedLocalAlloc GetSecurityDescriptorWithUser(const base::char16* sddl_string,
  ...
  ScopedLocalAlloc user_sec_desc_owner(user_sec_desc);
  *size = user_sec_desc_size;
  return std::move(user_sec_desc_owner);
}
...
const void* GetFallbackSecurityDescriptorForNamedPipeInstance(size_t* size) {
  ...
  // "S:(ML;;;;;S-1-16-0)". A static security descriptor is used as a fallback
  // if GetSecurityDescriptorWithUser fails, to avoid losing crashes from
  // non-AppContainer sandboxed applications.
...
const void* GetSecurityDescriptorForNamedPipeInstance(size_t* size) {
  ...
  // Note this memory allocation is intentionally leaked to avoid calling a
  // module destructor.
  static void* sec_desc =
      GetSecurityDescriptorWithUser(
