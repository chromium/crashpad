...
TEST(ProcessInfo, Self) {
  ...
  size_t environment_end = 0;
  while (environment_strings[environment_end] != L'\0' ||
         environment_strings[environment_end + 1] != L'\0') {
    environment_end++;
  }
  EXPECT_EQ(std::wstring(environment_strings, environment_end + 2),
            environment);
