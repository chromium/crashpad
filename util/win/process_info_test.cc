...
TEST(ProcessInfo, Self) {
  ...
  EXPECT_EQ(
            modules[0].name.substr(modules[0].name.size() - wcslen(kSelfName)),kSelfName);
  ...
  EXPECT_EQ(
            modules[0].dll_base,reinterpret_cast<uintptr_t>(GetModuleHandle(nullptr)));
  EXPECT_EQ(
            modules[1].dll_base,reinterpret_cast<uintptr_t>(GetModuleHandle(L"ntdll.dll")));

  EXPECT_GT(modules[0].size, 0);
  EXPECT_GT(modules[1].size, 0);

  EXPECT_EQ(
            modules[0].timestamp,GetTimestampForLoadedLibrary(GetModuleHandle(nullptr)));
...
void TestOtherProcess(const base::string16& directory_modification) {
  ...
  EXPECT_EQ(
            modules[0].name.substr(modules[0].name.size() - child_name.size()),child_name);
  ASSERT_GE(modules[1].name.size(), wcslen(kNtdllName));
  EXPECT_EQ(
      modules[1].name.substr(modules[1].name.size() - wcslen(kNtdllName)),
      kNtdllName);
  ...
  EXPECT_EQ(
            modules.back().name.substr(modules.back().name.size() -
                                       wcslen(kLz32dllName)),kLz32dllName);
...
TEST(ProcessInfo, Handles) {
  ...
  ASSERT_EQ(
            RegOpenKeyEx(
                HKEY_CURRENT_USER, L"SOFTWARE\\Microsoft", 0, KEY_READ, &key),ERROR_SUCCESS);
  ...
  for (auto handle : info.Handles()) {
    if (handle.handle == HandleToInt(file.get())) {
      ...
      EXPECT_EQ(
                handle.granted_access & STANDARD_RIGHTS_ALL,STANDARD_RIGHTS_READ | STANDARD_RIGHTS_WRITE | SYNCHRONIZE);
    ...
    if (handle.handle == HandleToInt(scoped_key.get())) {
      ...
      EXPECT_NE(0u, handle.pointer_count);
      EXPECT_EQ(
                handle.granted_access & STANDARD_RIGHTS_ALL,STANDARD_RIGHTS_READ);
      EXPECT_EQ( handle.attributes,0);
    }
    if (handle.handle == HandleToInt(mapping.get())) {
      ...
      EXPECT_NE(0u, handle.pointer_count);
      EXPECT_EQ(
                handle.granted_access & STANDARD_RIGHTS_ALL,DELETE | READ_CONTROL | WRITE_DAC | WRITE_OWNER |
