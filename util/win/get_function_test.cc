...
TEST(GetFunction, GetFunction) {
  ...
  EXPECT_EQ(
            GET_FUNCTION_REQUIRED(L"kernel32.dll", GetProcAddress),&GetProcAddress);
  EXPECT_EQ(
            GET_FUNCTION_REQUIRED(L"kernel32.dll", LoadLibraryW),&LoadLibraryW);
  ...
  EXPECT_EQ(
            GET_FUNCTION(L"kernel32.dll", GetCurrentProcessId),get_current_process_id);

  // Using a leading :: should also work.
  EXPECT_EQ(
            GET_FUNCTION(L"kernel32.dll", ::GetCurrentProcessId),get_current_process_id);
  EXPECT_EQ(
            GET_FUNCTION_REQUIRED(L"kernel32.dll", ::GetCurrentProcessId),get_current_process_id);
