...
TEST(Handle, HandleToInt) {
  ...
  EXPECT_EQ(
            HandleToInt(reinterpret_cast<HANDLE>(
                static_cast<intptr_t>(std::numeric_limits<int>::max()))),std::numeric_limits<int>::max());
  EXPECT_EQ(
            HandleToInt(reinterpret_cast<HANDLE>(
                static_cast<intptr_t>(std::numeric_limits<int>::min()))),std::numeric_limits<int>::min());
}
...
TEST(Handle, IntToHandle) {
  ...
  EXPECT_EQ(
            IntToHandle(std::numeric_limits<int>::max()),reinterpret_cast<HANDLE>(
                static_cast<intptr_t>(std::numeric_limits<int>::max())));
  EXPECT_EQ(
            IntToHandle(std::numeric_limits<int>::min()),reinterpret_cast<HANDLE>(
