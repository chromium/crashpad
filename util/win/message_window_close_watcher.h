...
//! \brief Creates a message-only window and waits for a `WM_CLOSE` message,
...
class MessageWindowCloseWatcher {
 ...
 protected:
  ...
  enum class Event {
    //! \brief A message window is being monitored for `WM_CLOSE` messages.
    //!
    //! Because the message loop runs on a different thread than the object was
    //! created on, this event allows for synchronization.
    kStartedWatching,

    //! \brief A `WM_CLOSE` message was received.
    kReceivedClose,

    //! \brief Monitoring for `WM_CLOSE` messages has ended.
    //!
    //! Because the message loop runs on a different thread than the object was
    //! created on, this event allows for synchronization.
    kStoppedWatching,
