...
void ExceptionHandlerServer::SetPrecreatedPipe(HANDLE first_pipe_instance) {
  DCHECK(pipe_name_.empty());
  DCHECK(!first_pipe_instance_.is_valid());

  first_pipe_instance_.reset(first_pipe_instance);
  wchar_t user_name[256];
  if (!GetNamedPipeHandleState(first_pipe_instance_.get(),
                          nullptr,
                          nullptr,
                          nullptr,
                          nullptr,
                          user_name,
                          sizeof(user_name))) {
    PCHECK(ERROR) << "GetNamedPipeHandleState ";
  }
  pipe_name_ = user_name;
}
