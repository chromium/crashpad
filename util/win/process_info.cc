...
bool UncheckedAllocate(size_t size, UniqueMallocPtr *allocated_ptr) {
  DCHECK(allocated_ptr);
  allocated_ptr->reset();
  void* raw_ptr = nullptr;
  if (!base::UncheckedMalloc(size, &raw_ptr))
    return false;

  allocated_ptr->reset(static_cast<uint8_t*>(raw_ptr));
  return true;
}
...
std::vector<ProcessInfo::Handle> ProcessInfo::BuildHandleVector(
  ...
  for (int tries = 0; tries < 5; ++tries) {
    if (!UncheckedAllocate(buffer_size, &buffer))
