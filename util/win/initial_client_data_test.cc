...
TEST(InitialClientData, RoundTrip) {
  ...
  EXPECT_EQ(
            second.non_crash_dump_completed(),first.non_crash_dump_completed());
  EXPECT_EQ( second.first_pipe_instance(),first.first_pipe_instance());
  EXPECT_EQ( second.client_process(),first.client_process());
  EXPECT_EQ(
            second.crash_exception_information(),first.crash_exception_information());
  EXPECT_EQ(
            second.non_crash_exception_information(),first.non_crash_exception_information());
  EXPECT_EQ(
            second.debug_critical_section_address(),first.debug_critical_section_address());
}
