...
TEST(SecurityDescriptor, MatchesAdvapi32) {
  ...
  size_t created_len;
  uint8_t* created = GetSecurityDescriptorForNamedPipeInstance(&created_len);
  ASSERT_EQ(sec_desc_len, created_len);
  for (ULONG i = 0; i < sec_desc_len; ++i) {
    SCOPED_TRACE(base::StringPrintf("byte offset %d", i));
    EXPECT_EQ(reinterpret_cast<uint8_t*>(sec_desc)[i], created[i]);
  }
}
