...
struct StringToLongTraits : public StringToSignedIntegerTraits<long, long long> {
  static LongType Convert(const char* str, char** end, int base) {
...
struct StringToUnsignedLongLongTraits
    : public StringToUnsignedIntegerTraits<unsigned long long, unsigned long long> {
  static LongType Convert(const char* str, char** end, int base) {
    if (str[0] == '-') {
      *end = const_cast<char*>(str);
      return 0;
...
bool StringToNumber(const std::string& string, unsigned long long* number) {
  return StringToIntegerInternal<StringToUnsignedLongLongTraits>(string, number);
}
