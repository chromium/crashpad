...
FileHandle LoggingOpenMemoryFileForReadAndWrite(const base::FilePath& name) {
  ...
  if (result >= 0 ||
      ...
      // doesn't have kernel support. O_TMPFILE was added in Linux 3.11. Local
      // testing confirms that at least Linux 2.6.29 and Linux 3.10 set errno to
      // EISDIR. The man pages also mention ENOENT as an error code to check,
      // but the language implies it would only occur when |tmp| is also an
      // invalid directory. EINVAL is mentioned as a possible error code for any
      // invalid values in flags, but O_TMPFILE isn't mentioned explicitly in
      // this context and hasn't been observed in practice.
