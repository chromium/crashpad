...
//! \brief Enumerates file and directory names in a directory.
//!
...
class DirectoryReader {
 public:
  ...
  //! \param[out] has_error Whether an error was encountered. If \a has_error is
  //!     not `nullptr` it is set to `true` if an error occurs when calling any
  //!     method in this class. Otherwise, it is set to `false`.
  DirectoryReader(const base::FilePath& path, bool* has_error);
  explicit DirectoryReader(const base::FilePath& path)
      : DirectoryReader(path, nullptr) {}
