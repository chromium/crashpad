...
class DirectoryReader {
 public:
  ...
  //!
  //! \return `true` if the reader is advanced to the next file. Otherwise
  //!     `false`. If an error occurs a message will be logged.
  bool NextFile();
...
#if defined(OS_POSIX)
  //! \brief Returns the file descriptor associated with this reader.
  int DirectoryFD();
#endif
 ...
 private:
  ...
  class Iterator {
   public:
    explicit Iterator(DirectoryReader* reader);
    ~Iterator();

    bool operator!=(const Iterator& other) const;
    bool operator==(const Iterator& other) const;
    Iterator& operator++();
    const base::FilePath& operator*() const;
 ...
 public:
  //! \brief Returns an iterator at the reader's current position.
  //!
  //! When any iterator for a reader is incremented, all other iterators for
  //! that reader are invalidated.
  Iterator begin();
  static Iterator end();
