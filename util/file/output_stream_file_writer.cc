...
bool OutputStreamFileWriter::Write(const void* data, size_t size) {
  DCHECK(!flushed_);
  flush_needed_ = true;
  if (!output_stream_->Write(static_cast<const uint8_t*>(data), size)) {
    flush_needed_ = false;
    return false;
  }
  return true;
...
bool OutputStreamFileWriter::WriteIoVec(std::vector<WritableIoVec>* iovecs) {
  ...
  flush_needed_ = true;
  StringFile buffer;
  if (!buffer.WriteIoVec(iovecs)) {
    flush_needed_ = false;
    return false;
  }
  const std::string& data = buffer.string();
  if (!output_stream_->Write(reinterpret_cast<const uint8_t*>(data.data()),
                             data.size())) {
    flush_needed_ = false;
    return false;
  }
