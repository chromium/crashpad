...
TEST(VMOFileWriter, Seek) {
  VMOFileWriter writer;
  writer.Write(kSomeData, sizeof(kSomeData));

  // SEEK_SET
  EXPECT_EQ(writer.Seek(0, SEEK_SET), 0);
  EXPECT_EQ(writer.Seek(-10, SEEK_SET), -1);
  EXPECT_EQ(writer.Seek(1, SEEK_SET), 1);
  EXPECT_EQ(writer.Seek(kSomeDataLength - 1, SEEK_SET), kSomeDataLength - 1);
  EXPECT_EQ(writer.Seek(kSomeDataLength, SEEK_SET), kSomeDataLength);
  EXPECT_EQ(writer.Seek(kSomeDataLength + 1, SEEK_SET), -1);

  // SEEK_CUR
  ASSERT_EQ(writer.Seek(0, SEEK_SET), 0);
  ASSERT_EQ(writer.Seek(0, SEEK_CUR), 0);

  ASSERT_EQ(writer.Seek(5, SEEK_CUR), 5);
  ASSERT_EQ(writer.Seek(-10, SEEK_CUR), -1);
  ASSERT_EQ(writer.Seek(-3, SEEK_CUR), 2);
  ASSERT_EQ(writer.Seek(-2, SEEK_CUR), 0);
  ASSERT_EQ(writer.Seek(-2, SEEK_CUR), -1);
  ASSERT_EQ(writer.Seek(kSomeDataLength - 1, SEEK_CUR), kSomeDataLength - 1);
  ASSERT_EQ(writer.Seek(-kSomeDataLength, SEEK_CUR), -1);
  ASSERT_EQ(writer.Seek(-(kSomeDataLength + 1), SEEK_CUR), -1);
  ASSERT_EQ(writer.Seek(kSomeDataLength, SEEK_CUR), -1);

  // SEEK_END
  EXPECT_EQ(writer.Seek(0, SEEK_END), kSomeDataLength);
  EXPECT_EQ(writer.Seek(1, SEEK_END), -1);
  EXPECT_EQ(writer.Seek(kSomeDataLength, SEEK_END), -1);
  EXPECT_EQ(writer.Seek(-1, SEEK_END), kSomeDataLength - 1);
  EXPECT_EQ(writer.Seek(-3, SEEK_END), kSomeDataLength - 3);
  EXPECT_EQ(writer.Seek(-(kSomeDataLength - 2), SEEK_END), 2);
  EXPECT_EQ(writer.Seek(-(kSomeDataLength - 1), SEEK_END), 1);
  EXPECT_EQ(writer.Seek(-(kSomeDataLength), SEEK_END), 0);
  EXPECT_EQ(writer.Seek(-(kSomeDataLength + 1), SEEK_END), -1);
}
