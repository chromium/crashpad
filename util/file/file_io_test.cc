...
TEST(FileIO, MultipleSharedLocks) {
  ...
  ASSERT_NE(handle1, kInvalidFileHandle);
  EXPECT_EQ(FileLockingResult::kSuccess,
            LoggingLockFile(handle1.get(), FileLocking::kShared, false));

  auto handle2 = ScopedFileHandle(LoggingOpenFileForRead(shared_file));
  ASSERT_NE(handle1, kInvalidFileHandle);
  EXPECT_EQ(FileLockingResult::kSuccess,
            LoggingLockFile(handle2.get(), FileLocking::kShared, false));
 ...
 private:
  void ThreadMain() override {
    for (int i = 0; i < iterations_; ++i) {
      EXPECT_EQ(FileLockingResult::kSuccess,
                LoggingLockFile(file_.get(), lock_type_, false));
...
void LockingTest(FileLocking main_lock, FileLocking other_locks) {
  ...
  EXPECT_EQ(FileLockingResult::kSuccess,
            LoggingLockFile(initial.get(), main_lock, false));
...
TEST(FileIO, ExclusiveVsExclusivesNonBlocking) {
  ...
  ASSERT_NE(handle1, kInvalidFileHandle);
  EXPECT_EQ(FileLockingResult::kSuccess,
            LoggingLockFile(handle1.get(), FileLocking::kExclusive, false));

  auto handle2 = ScopedFileHandle(LoggingOpenFileForRead(exclusive_file));
  ASSERT_NE(handle1, kInvalidFileHandle);
  EXPECT_EQ(FileLockingResult::kWouldBlock,
            LoggingLockFile(handle2.get(), FileLocking::kExclusive, true));
