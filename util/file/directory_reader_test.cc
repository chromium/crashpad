...
void ExpectFiles(const std::set<base::FilePath>& files,
  ...
  for (const auto& filename : expected) {
    ...
    SCOPED_TRACE(base::StringPrintf(
        "Filename: %s", base::UTF16ToUTF8(filename.value().c_str()).c_str()));
#else
    SCOPED_TRACE(base::StringPrintf("FIlename: %s", filename.value().c_str()));
#endif
...
TEST(DirectoryReader, FilesAndDirectories) {
...

  {
    ...
    ASSERT_TRUE(
        LoggingCreateDirectory(temp_dir.path().Append(directory), false));
    expected_files.insert(directory);

    LOG(INFO) << "Making nested file";
