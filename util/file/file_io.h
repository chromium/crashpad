...
//!
//! The logic is exposed so that it may be reused by FileReaderInterface. It is
//! not intended to be used more generally. Use ReadFileExactly(),
...
//! WriteFile() or something else that calls WriteFile().
//!
//! \param[in] file The file to write to.
//! \param[in] buffer A buffer containing data to be written.
//! \param[in] size The number of bytes from \a buffer to write.
//! \param[out] bytes_written The number of bytes actually written from \a
//!     buffer to \a file.
//!
//! \return `true` on success, with \a bytes_written set appropriately. `false`
//!     on error, with `errno` or `GetLastError()` set appropriately.
