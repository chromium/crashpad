...
bool FileModificationTime(const base::FilePath& path, timespec* mtime) {
  ...
  //  LOG(ERROR) << "path is a directory " << base::UTF16ToUTF8(path.value());
  //  return false;
  //}
  ...
  if (find_data.ftLastWriteTime.dwLowDateTime != 0) {
    *mtime = FiletimeToTimespecEpoch(find_data.ftLastWriteTime);
  } else if (find_data.ftCreationTime.dwLowDateTime != 0) {
    *mtime = FiletimeToTimespecEpoch(find_data.ftCreationTime);
  } else {
