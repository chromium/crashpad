...
bool IsSymbolicLink(const base::FilePath& path) {
  ...
  ScopedSearchHANDLE handle(FindFirstFileEx(path.c_str(),
                                            FindExInfoBasic,
                                            &find_data,
  ...
  return (find_data.dwFileAttributes & FILE_ATTRIBUTES_REPARSE_POINT) != 0 &&
...
bool LoggingRemoveFile(const base::FilePath& path) {
  ...
  if ((fileattr & FILE_ATTRIBUTE_DIRECTORY) != 0 &&
      (fileattr & FILE_ATTRIBUTE_REPARSE_POINT) != 0) {
    return LoggingRemoveDirectory(path);
  }
