  ...
  while (LoggingUnwStep(&cursor) > 0) {
    ...
    if (ModulePathMatchesSinkhole(dl_info.dli_fname, kUIKitCorePath)) {
      ...
      if (LoggingUnwStep(&cursor) > 0 &&
        ...
        auto uigestureimp_lambda = [](bool max) {
          IMP imp = bit_cast<IMP>(nullptr);
          unsigned int method_count = 0;
          Method* method_list = class_copyMethodList(
              NSClassFromString(@"UIGestureEnvironment"), &method_count);
          if (method_count > 0) {
            imp = method_getImplementation(method_list[0]);
            for (unsigned int method_index = 1; method_index < method_count;
                 method_index++) {
              IMP method_imp =
                  method_getImplementation(method_list[method_index]);
              if (max) {
                imp = std::max(method_imp, imp);
              } else {
                imp = std::min(method_imp, imp);
              }
            }
          }
          free(method_list);
        ...
        static IMP gesture_environment_min_imp = uigestureimp_lambda(false);
        static IMP gesture_environment_max_imp = uigestureimp_lambda(true);
