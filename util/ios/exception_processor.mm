...
// Returns true if |string| ends with |ending|.
bool StringEndsWith(const char* string, const char* ending) {
  ...
  if (ending_length > string_length)
    return false;
  return strncmp(string + string_length - ending_length,
                 ending,
                 ending_length) == 0;
}
  ...
  while (unw_step(&cursor) > 0) {
    ...
    if (dladdr(reinterpret_cast<const void*>(frame_info.start_ip), &dl_info) !=
      ...
      for (const char* sinkhole : kExceptionLibraryPathSinkholes) {
        ...
        // Simulator path's tend to include much of Xcode's internal structure,
        // but the end of the path is the same.
        if (StringEndsWith(dl_info.dli_fname, sinkhole)) {
#else
