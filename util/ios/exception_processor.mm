...
id ObjcExceptionPreprocessor(id exception) {
  ...
  while (unw_step(&cursor) > 0) {
    ...
    // Some sinkhole symbol names appear as "<redacted>" on devices, or when
    // the debugger is not attached.  Instead, use library paths that are known
    // to contain sinkholes.
