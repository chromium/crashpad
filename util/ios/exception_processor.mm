...
id ObjcExceptionPreprocessor(id exception) {
  ...
  while (LoggingUnwStep(&cursor) > 0) {
    ...
    // kExceptionLibraryPathSinkholes becaues it also has non-sinkhole handlers.
    // However, since _UIGestureEnvironmentUpdate is always called from
    // -[UIGestureEnvironment _deliverEvent:toGestureRecognizers:usingBlock:],
    // inspect the caller frame info to match the sinkhole.s
