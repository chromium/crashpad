...
static const char* const kExceptionProcedureSinkholes[] = {
    "CFRunLoopRunSpecific",  // Unredacted
    "_CFXNotificationPost",  // Unredacted
    "_dispatch_client_callout",  // Redacted, but useful for simulator testing.
    "__CFRunLoopRun",  // Redacted, but useful for simulator testing.
    "__CFRunLoopDoTimers",  // Redacted, but useful for simulator testing.
    "__NSFireDelayedPerform",  // Unredacted
};

// Use symbol paths when procedure names are redacted.
static const char* const kExceptionSymbolPathSinkholes[] = {
    // Everything in this lib siknholes, specifically _dispatch_client_callout.
...
static NOINLINE void TERMINATING_FROM_UNCAUGHT_NSEXCEPTION(
    id exception,
    const char* sinkhole) {
...
static id ObjcExceptionPreprocessor(id exception) {
  ...
  struct objc_exception* exc =
      (struct objc_exception*)__cxxabiv1::__cxa_allocate_exception(
  ...
  while (unw_step(&cursor) > 0) {
    ...
    for (const char* sinkhole : kExceptionProcedureSinkholes) {
    ...
    if (dladdr(reinterpret_cast<const void*>(frame_info.start_ip), &dl_info) !=
        0) {
      for (const char* sinkhole : kExceptionSymbolPathSinkholes) {
        if (strcmp(sinkhole, dl_info.dli_fname) == 0) {
          TERMINATING_FROM_UNCAUGHT_NSEXCEPTION(exception, sinkhole);
        }
      }
    }
  ...
  // See github.com/llvm/llvm-project/blob/master/libcxxabi/src/
  // cxa_exception.cpp::__cxa_throw
  if (!handler_found) {
    VLOG(1) << "No handler found, terminating before failed_throw";
