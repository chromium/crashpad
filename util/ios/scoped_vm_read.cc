...
ScopedVMReadInternal::~ScopedVMReadInternal() {
  if (data_) {
    ...
    MACH_LOG_IF(ERROR, kr != KERN_SUCCESS, kr) << "vm_deallocate";
  }
}

void ScopedVMReadInternal::reset(const void* data, const size_t data_length) {
  if (data_) {
    kern_return_t kr =
        vm_deallocate(mach_task_self(), vm_read_data_, vm_read_data_count_);
    MACH_LOG_IF(ERROR, kr != KERN_SUCCESS, kr) << "vm_deallocate";
  ...
  if (kr == KERN_SUCCESS) {
    ...
    MACH_LOG(ERROR, kr) << "vm_read";
  }
}

bool ScopedVMReadInternal::is_valid() const {
  return data_ != 0 && vm_read_data_count_ > 0;
}

vm_address_t ScopedVMReadInternal::data() const {
  return data_;
}
