...
TEST_F(IOSIntermediateDumpReaderTest, WriteBadPropertyDataLength) {
  ...
  const auto root_map = reader.RootMap();
  EXPECT_TRUE(root_map->empty());
  EXPECT_FALSE(root_map->HasKey(Key::kVersion));
