...
void RawLogInternal(const char* message) {
...
void RawLogInternal(uint64_t number) {
  char buffer[20];
  char* digit = buffer + sizeof(buffer);
  *digit = 0;
  do {
    *(--digit) = (number % 10) + '0';
    number /= 10;
  } while (number != 0);
  RawLogInternal(digit);
}
...
void RawLog(const char* file, int line, const char* message, int error) {
  ...
  RawLogInternal(line);
  HANDLE_EINTR(write(STDERR_FILENO, " ", 1));
  RawLogInternal(message);
  if (error) {
    HANDLE_EINTR(write(STDERR_FILENO, ":", 1));
    RawLogInternal(error);
  }
