...
// Similar to LoggingWriteFile but with RAW_LOG.
bool RawLoggingWriteFile(int fd, const void* buffer, size_t size) {
  ...
  while (size > 0) {
    ...
    if (bytes_written < 0 || bytes_written == 0) {
      RAW_LOG_ERROR(bytes_written, "RawLoggingWriteFile");
      return false;
    }
...
// Similar to LoggingCloseFile but with RAW_LOG.
bool RawLoggingCloseFile(int fd) {
  int rv = IGNORE_EINTR(close(fd));
  if (rv != 0) {
    RAW_LOG_ERROR(rv, "RawLoggingCloseFile");
  }
  return rv == 0;
}

// Similar to LoggingLockFile but with RAW_LOG.
bool RawLoggingLockFileExclusiveNonBlocking(int fd) {
  int rv = HANDLE_EINTR(flock(fd, LOCK_EX | LOCK_NB));
  if (rv != 0) {
    RAW_LOG_ERROR(rv, "RawLoggingLockFileExclusiveNonBlocking");
  }
  return rv == 0;
}
...
bool IOSIntermediateDumpWriter::Open(const base::FilePath& path) {
  ...
  fd_ = HANDLE_EINTR(open_dprotected_np(path.value().c_str(),
                                        ...
                                        0644 /* mode */));
  if (fd_ < 0) {
    RAW_LOG_ERROR(fd_, "open intermediate dump");
    RAW_LOG(path.value().c_str());
    return false;
  }
...
bool IOSIntermediateDumpWriter::Close() {
  ...
  return RawLoggingWriteFile(fd_, &t, sizeof(uint8_t)) &&
         RawLoggingCloseFile(fd_);
}
...
bool IOSIntermediateDumpWriter::AddPropertyInternal(IntermediateDumpKey key,
  ...
  if (!vmread.Read(value, value_length))
    return false;
  uint8_t t = PROPERTY;
  return RawLoggingWriteFile(fd_, &t, sizeof(t)) &&
         RawLoggingWriteFile(fd_, &key, sizeof(key)) &&
