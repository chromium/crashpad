...
class ScopedVMRead {
 public:
  explicit ScopedVMRead(const T* value, size_t count = 1) {
    reset(value, count);
  }
  explicit ScopedVMRead(uint64_t value, size_t count = 1) {
    reset(reinterpret_cast<T*>(value), count);
  }
  ...
  void reset(const void* value, size_t count = 1) {
    size_t value_length = count * sizeof(T);
    if (data_) {
      release();
    }
...
class IOSIntermediatedumpWriter final {
 public:
  ...
  template <typename T>
  void AddProperty(IntermediateDumpKey key, const T* value, size_t count = 1) {
    AddPropertyInternal(key, value, count * sizeof(T));
  }

  void AddPropertyBytes(IntermediateDumpKey key,
                        const void* value,
                        size_t value_length) {
    AddPropertyInternal(key, value, value_length);
  }

  static size_t ThreadStateLengthForFlavor(thread_state_flavor_t flavor);

 private:
  void AddPropertyInternal(IntermediateDumpKey key,
                           const void* value,
                           size_t value_length);
