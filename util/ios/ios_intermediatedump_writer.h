...
#define INTERMEDIATE_DUMP_KEYS(TD) \
  ...
  TD(kModules, 3000) \
    ...
    TD(kTimestamp, 3007) \
    TD(kUUID, 3008) \
    TD(kAnnotationsList, 3009) \
    TD(kAnnotationType, 3010) \
    TD(kAnnotationName, 3011) \
    TD(kAnnotationValue, 3012) \
...
class IOSIntermediatedumpWriter final {
 public:
  ...
  class ScopedVMRead {
   public:
    explicit ScopedVMRead(const void* value, size_t value_length, void* data) {
      ...
      if (kr == KERN_SUCCESS) {
        *(vm_address_t*)data =
            vm_read_data_ + ((vm_address_t)value - page_region_address);
      } else {
