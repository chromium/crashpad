...
class IOSIntermediatedumpWriter final {
 public:
  ...
  class ScopedArrayMap {
   public:
    explicit ScopedArrayMap(IOSIntermediatedumpWriter* writer) : writer_(writer) {
      writer->ArrayMapStart();
    }
 ...
 private:
  //! \return The `true` if able to write a PROPERTY command  with the \a key
  //!     \a value \a count tuple.
  bool AddPropertyInternal(IntermediateDumpKey key,
                           const char* value,
                           size_t value_length);

  //! \return The `true` if able to write a ARRAY_START command  with the
  //!     \a key.
  bool ArrayStart(IntermediateDumpKey key);

  //! \return The `true` if able to write a MAP_START command  with the
  //!     \a key.
  bool MapStart(IntermediateDumpKey key);

  //! \return The `true` if able to write a MAP_START command.
  bool ArrayMapStart();

  //! \return The `true` if able to write a ARRAY_END command.
  bool ArrayEnd();

  //! \return The `true` if able to write a MAP_END command.
  bool MapEnd();
