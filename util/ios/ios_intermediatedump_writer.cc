...
bool IOSIntermediatedumpWriter::Open(const base::FilePath& path) {
  fd_ = open_dprotected_np(path.value().c_str(),
                           ...
                           //-rw-r--r--
                           0644);
  return LoggingLockFile(fd_, FileLocking::kExclusive);
}
...
bool IOSIntermediatedumpWriter::AddProperty(IntermediateDumpKey key,
                                            const void* value,
                                            size_t value_length) {
  ScopedVMRead<void> vmread(value, value_length);
  if (!vmread.is_valid())
    return false;
  uint8_t t = PROPERTY;
