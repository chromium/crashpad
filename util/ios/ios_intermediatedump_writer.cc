...
bool IOSIntermediatedumpWriter::Open(const base::FilePath& path) {
  fd_ = open_dprotected_np(path.value().c_str(),
                           ...
                           //-rw-r--r--
                           0644);
  LoggingLockFile(fd_, FileLocking::kExclusive);
  return true;
}

bool IOSIntermediatedumpWriter::Close() {
  static uint8_t t = DOCUMENT_END;
  LoggingWriteFile(fd_, &t, sizeof(uint8_t));
...
void IOSIntermediatedumpWriter::AddPropertyInternal(IntermediateDumpKey key,
                                                    const void* value,
                                                    size_t value_length) {
  ScopedVMRead<char> vmread(reinterpret_cast<const char*>(value), value_length);
  if (!vmread.is_valid())
    return;
  uint8_t t = PROPERTY;
  LoggingWriteFile(fd_, &t, sizeof(t));
  LoggingWriteFile(fd_, &key, sizeof(key));
