...
// static
void IOSIntermediatedumpWriterUtil::ArrayMapStart(int fd) {
  static uint8_t t = MAP_START;
  write(fd, &t, sizeof(uint8_t));
}

// static
void IOSIntermediatedumpWriterUtil::MapStart(int fd, IntermediateDumpKey key) {
  static uint8_t t = MAP_START;
  write(fd, &t, sizeof(uint8_t));
...
// static
void IOSIntermediatedumpWriterUtil::ArrayStart(int fd,
...
// static
void IOSIntermediatedumpWriterUtil::MapEnd(int fd) {
  static uint8_t t = MAP_END;
  write(fd, &t, sizeof(uint8_t));
}

// static
void IOSIntermediatedumpWriterUtil::ArrayEnd(int fd) {
  static uint8_t t = ARRAY_END;
  write(fd, &t, sizeof(uint8_t));
}

// static
void IOSIntermediatedumpWriterUtil::Property(int fd,
                                             IntermediateDumpKey key,
                                             const void* value,
                                             size_t value_length) {
  ...
  if (kr == KERN_SUCCESS) {
    ...
    ssize_t actual_length = write(fd, vm_read_offset_data, value_length);
...
size_t IOSIntermediatedumpWriterUtil::ThreadStateLengthForFlavor(
