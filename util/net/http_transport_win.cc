...
bool HTTPTransportWin::ExecuteSynchronously(std::string* response_body) {
  ...
  do {
    struct HTTPRequestBodyBuffer {
      char size[8];
      char crlf0[2];
      uint8_t data[32 * 1024];
      char crlf1[2];
    };
    static_assert(offsetof(HTTPRequestBodyBuffer, crlf0) ==
                      sizeof(HTTPRequestBodyBuffer::size),
                  "padding between size and crlf0");
    static_assert(offsetof(HTTPRequestBodyBuffer, data) ==
                      sizeof(HTTPRequestBodyBuffer::size) +
                          sizeof(HTTPRequestBodyBuffer::crlf0),
                  "padding between crlf0 and data");
    static_assert(offsetof(HTTPRequestBodyBuffer, crlf1) ==
                      sizeof(HTTPRequestBodyBuffer::size) +
                          sizeof(HTTPRequestBodyBuffer::crlf0) +
                          sizeof(HTTPRequestBodyBuffer::data),
                  "padding between data and crlf1");

    HTTPRequestBodyBuffer buf;
