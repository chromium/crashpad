...
HTTPTransportWin::WinHttpStatusCallback(HINTERNET session,
  ...
  cert_chain_parameters.RequestedUsage.dwType = USAGE_MATCH_TYPE_OR;
  cert_chain_parameters.RequestedUsage.Usage.cUsageIdentifier = arraysize(usage);
  cert_chain_parameters.RequestedUsage.Usage.rgpszUsageIdentifier =
      const_cast<LPSTR*>(usage);
  const CERT_CHAIN_CONTEXT* cert_chain_context_temp;
  if (!CertGetCertificateChain(// Default chain engine (HCCE_CURRENT_USER).
                                nullptr,

                                // Certificate to retrieve chain for.
                                leaf_cert_context.get(),

                                // Use current system time to validate.
                                nullptr,

                                // No additional certificate store.
                                nullptr,
  ...
  ScopedHCRYPTPROV crypto_provider(crypto_provider_temp);

  const CERT_SIMPLE_CHAIN* chain = cert_chain_context.get()->rgpChain[0];
  for (size_t element_index = 0; element_index < chain->cElement;
    ...
    std::array<uint8_t, 32> hash_val;
    DWORD hash_len = 32;
    if (!CryptGetHashParam(crypto_hash, HP_HASHVAL, hash_val.data(), &hash_len, 0)) {
      PLOG(ERROR) << "CryptGetHashParam";
      continue;
    }
    DCHECK_EQ(hash_len, (DWORD)32);

    for(auto& pinned_hash : self->https_pins()) {
      if (pinned_hash == hash_val) {
  ...
  // We were unable to find a pinned certificate in the chain, but pinning was
  // requested so cancel the request. WinHttpSendRequest() will fail with
  // WININET_E_OPERATION_CANCELLED.
  LOG(ERROR) << "Unable to resolve certificate to pinned root.";
  self->request_->reset();
}
