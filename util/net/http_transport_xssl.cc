...
class HTTPTransportXSSL final : public HTTPTransport {
 public:
  HTTPTransportXSSL() = default;
  ~HTTPTransportXSSL() override = default;

  bool ExecuteSynchronously(std::string* response_body) override;

 private:
  DISALLOW_COPY_AND_ASSIGN(HTTPTransportXSSL);
};
...
bool CheckCertificate(BIO* bio, const char* hostname) {
  ...
  // TODO(scottmg): If I connect to google.ca instead, the common name is
  // google.com still, so this is presumably wrong.
...
bool HTTPTransportXSSL::ExecuteSynchronously(std::string* response_body) {
  ...
  // TODO(scottmg): Hack code to do a GET for testing.
...
std::unique_ptr<HTTPTransport> HTTPTransport::Create() {
  return std::unique_ptr<HTTPTransportXSSL>(new HTTPTransportXSSL);
}
