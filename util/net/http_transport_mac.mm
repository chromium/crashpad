...
NSString* Sysctl(int mib_0, int mib_1) {
  int mib[] = {mib_0, mib_1};
  char value[128];
  size_t size = sizeof(value);
  if (sysctl(mib, arraysize(mib), value, &size, nullptr, 0) != 0) {
    PLOG(WARNING) << base::StringPrintf("sysctl {%d, %d}", mib[0], mib[1]);
    return nullptr;
  }
...
NSString* UserAgentString() {
  ...
  NSString* ostype = Sysctl(CTL_KERN, KERN_OSTYPE);
  if (ostype) {
    user_agent = AppendEscapedFormat(user_agent, @" %@", ostype);

    NSString* osrelease = Sysctl(CTL_KERN, KERN_OSRELEASE);
    if (osrelease) {
      user_agent = AppendEscapedFormat(user_agent, @"/%@", osrelease);
    }

    // CFNetwork just uses the sysctl below to obtain the native (kernel)
    // architecture. Here, give the process’ architecture as well as the native
    // architecture. Use the same strings that the kernel would, so that they
...
#error Port
    ...
    NSString* machine = Sysctl(CTL_HW, HW_MACHINE);
    if (machine && ![machine isEqualToString:arch]) {
