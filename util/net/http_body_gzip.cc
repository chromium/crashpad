...
GzipHTTPBodyStream::GzipHTTPBodyStream(std::unique_ptr<HTTPBodyStream> source)
    : input_(),
      ...
      state_(GzipState::kUninitialized) {
}

GzipHTTPBodyStream::~GzipHTTPBodyStream() {
  DCHECK(state_ == GzipState::kFinished || state_ == GzipState::kError);
}

FileOperationResult GzipHTTPBodyStream::GetBytesBuffer(uint8_t* buffer,
                                                       size_t max_len) {
  if (state_ == GzipState::kError) {
    return -1;
  }

  if (state_ == GzipState::kFinished) {
    return 0;
  }

  if (state_ == GzipState::kUninitialized) {
    ...
    if (zr != Z_OK) {
      ...
      state_ = GzipState::kError;
      return -1;
    }

    state_ = GzipState::kOperating;
  }

  z_stream_.next_out = buffer;
  z_stream_.avail_out = max_len;

  while (state_ != GzipState::kFinished && z_stream_.avail_out > 0) {
    if (state_ != GzipState::kInputEOF && z_stream_.avail_in == 0) {
    ...
    int zr = deflate(&z_stream_,
                     state_ == GzipState::kInputEOF ? Z_FINISH : Z_NO_FLUSH);
    if (state_ == GzipState::kInputEOF && zr == Z_STREAM_END) {
      state_ = GzipState::kFinished;
