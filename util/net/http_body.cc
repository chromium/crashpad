...
FileHandleHTTPBodyStream::FileHandleHTTPBodyStream(FileHandle handle)
    : HTTPBodyStream(), reader_(handle), reached_eof_(false) {}

FileHandleHTTPBodyStream::~FileHandleHTTPBodyStream() {}

FileOperationResult FileHandleHTTPBodyStream::GetBytesBuffer(uint8_t* buffer,
                                                             size_t max_len) {
  if (reached_eof_) {
    return 0;
  }
