...
TEST(FileHandleHTTPBodyStream, ReadASCIIFile) {
  ...
  FileHandleHTTPBodyStream stream(handle.get());
...
TEST(FileHandleHTTPBodyStream, ReadBinaryFile) {
...
TEST_P(CompositeHTTPBodyStreamBufferSize, StringsAndFile) {
  ...
  ScopedFileHandle handle(LoggingOpenFileForRead(path));
  ASSERT_TRUE(handle.is_valid());
  parts.push_back(new FileHandleHTTPBodyStream(handle.get()));
  parts.push_back(new StringHTTPBodyStream(string2));
