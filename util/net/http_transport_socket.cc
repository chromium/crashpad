...
bool WaitUntilSocketIsReady(int sock) {
  ...
  timeval timeout = {};
  timeout.tv_sec = 1;
  if (select(sock + 1, &fds_read, &fds_write, &fds_error, &timeout) < 0) {
    PLOG(ERROR) << "select";
    return false;
...
base::ScopedFD CreateSocket(const std::string& hostname,
  ...
  for (const auto* ap = addrinfo.get(); ap; ap = ap->ai_next) {
...

    {
      ...
      int ret = connect(result.get(), ap->ai_addr, ap->ai_addrlen);
      if (ret < 0) {
...
bool WriteRequest(int sock,
  ...
  if (send(sock, request_line.data(), request_line.size(), 0) < 0) {
    PLOG(ERROR) << "send";
      return false;
  }
  ...
  for (const auto& header : headers) {
    ...
    if (send(sock, header_str.data(), header_str.size(), 0) < 0) {
      PLOG(ERROR) << "send";
      return false;
    }
  }
  ...
  if (chunked) {
    ...
    if (send(sock,
             ...
             sizeof(kTransferEncodingChunked) /
                 sizeof(kTransferEncodingChunked[0]) - 1,
             0) < 0) {
      PLOG(ERROR) << "send";
      return false;
    }
  ...
  do {
    ...
    fprintf(stderr,
            "bytes: %u, '%s'\n",
            write_size,
            std::string((char*)write_start, write_size).c_str());
    ...
    if (write_size != 0) {
      ssize_t written = send(sock, write_start, write_size, 0);
      if (written < 0) {
        PLOG(ERROR) << "send";
        return false;
      }
...
bool ReadContentWithLength(int sock, size_t len, std::string* body) {
  ...
  size_t received = 0;
  while (received < len) {
    ssize_t read = recv(sock, &body[received], len - received, 0);
    if (read <= 0) {
      return false;
    }
    received += read;
  }
  fprintf(stderr, "ReadContentWithLength got '''%s'''\n", body->c_str());
  return true;
}

bool ReadContentChunked(int sock, std::string* body) {
  LOG(ERROR) << "TODO!!!!! CHUNKED READ";
  return false;
}
...
bool ReadResponse(int sock, std::string* response_body) {
  ...
  if (len) {
    LOG(ERROR) << "reading with len=" << len;
    ...
    LOG(ERROR) << "reading with chunked";
...
bool HTTPTransportSocket::ExecuteSynchronously(std::string* response_body) {
  ...
  LOG(ERROR) << "HTTPTransportSocket read body of " << response_body->size();
