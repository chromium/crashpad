...
bool WriteRequest(int sock,
  ...
  if (!LoggingWriteFile(sock, kEndOfHeaders, strlen(kEndOfHeaders))) {
    return false;
  }
  ...
  do {
    constexpr size_t kTerminatingCRLFSize = 2;
    struct __attribute__((packed)) {
      char size[8];
      char crlf0[2];
      uint8_t data[32 * 1024 + kTerminatingCRLFSize];
    } buf;
    static_assert(sizeof(buf) == sizeof(buf.size) + sizeof(buf.crlf0) +
                                     sizeof(buf.data),
                  "buf should not have padding");

    // Read a block of data.
    data_bytes = body_stream->GetBytesBuffer(
        buf.data, sizeof(buf.data) - kTerminatingCRLFSize);
    if (data_bytes == -1) {
      return false;
    }
    DCHECK_GE(data_bytes, 0);
    DCHECK_LE(static_cast<size_t>(data_bytes),
              sizeof(buf.data) - kTerminatingCRLFSize);
    ...
    if (chunked) {
      ...
      write_size =
          size_len + sizeof(buf.crlf0) + data_bytes + kTerminatingCRLFSize;
    } else {
...
bool ReadContentWithLength(int sock, size_t len, std::string* body) {
  ...
  while (received < len) {
    FileOperationResult read =
        ReadFile(sock, &body->at(received), len - received);
    if (read <= 0) {
      return false;
    }
    received += read;
