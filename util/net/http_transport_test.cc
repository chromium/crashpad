...
enum class Scheme { HTTP = 0, HTTPS };

// This allows us to see meaningful names rather than /0, /1 and /2 in the
// parameterized test case names.
std::string PrettyPrint(const testing::TestParamInfo<Scheme>& info) {
  switch (info.param) {
    case Scheme::HTTP:
      return "HTTP";
    case Scheme::HTTPS:
...
class HTTPTransportTestFixture : public MultiprocessExec {
 public:
  ...
  HTTPTransportTestFixture(const Scheme& scheme,
...
class HTTPTransport : public testing::TestWithParam<Scheme> {};
...
void RunUpload33k(const Scheme& scheme, bool has_content_length) {
...
INSTANTIATE_TEST_SUITE_P(HTTPTransport,
                         HTTPTransport,
                         testing::Values(Scheme::HTTP, Scheme::HTTPS),
                         &PrettyPrint);
#else
INSTANTIATE_TEST_SUITE_P(HTTPTransport,
                         HTTPTransport,
                         testing::Values(Scheme::HTTP),
                         &PrettyPrint);
#endif
