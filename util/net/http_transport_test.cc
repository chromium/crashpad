...
#if defined(OS_WIN)
      auto to_utf8_if_windows =
          [](const std::string16& x) { return base::UTF16ToUTF8(x); };
#else
      auto to_utf8_if_windows = [](const std::string& x) { return x; };
#endif
      args.push_back(to_utf8_if_windows(cert_.value()));
      args.emplace_back(to_utf8_if_windows(
...
#else
INSTANTIATE_TEST_CASE_P(HTTPTransport, HTTPTransport, testing::Values("http"));
#endif
