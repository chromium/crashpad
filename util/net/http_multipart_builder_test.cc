...
TEST(HTTPMultipartBuilder, ThreeFileAttachments) {
  ...
  ScopedFileHandle handle1(LoggingOpenFileForRead(ascii_http_body_path));
  ASSERT_TRUE(handle1.is_valid());
  builder.SetFileAttachment("first", "minidump.dmp", handle1.get(), "");

  ScopedFileHandle handle2(LoggingOpenFileForRead(ascii_http_body_path));
  ASSERT_TRUE(handle2.is_valid());
  builder.SetFileAttachment(
      "second", "minidump.dmp", handle2.get(), "text/plain");

  ScopedFileHandle handle3(LoggingOpenFileForRead(ascii_http_body_path));
  ASSERT_TRUE(handle3.is_valid());
  builder.SetFileAttachment(
      "\"third 50% silly\"", "test%foo.txt", handle3.get(), "text/plain");
...
TEST(HTTPMultipartBuilder, OverwriteFileAttachment) {
  ...
  ScopedFileHandle handle1(LoggingOpenFileForRead(
      testdata_path.Append(FILE_PATH_LITERAL("binary_http_body.dat"))));
  ASSERT_TRUE(handle1.is_valid());
  builder.SetFileAttachment("minidump", "minidump.dmp", handle1.get(), "");

  ScopedFileHandle handle2(LoggingOpenFileForRead(
      testdata_path.Append(FILE_PATH_LITERAL("binary_http_body.dat"))));
  ASSERT_TRUE(handle2.is_valid());
  builder.SetFileAttachment("minidump2", "minidump.dmp", handle2.get(), "");

  ScopedFileHandle handle3(LoggingOpenFileForRead(
      testdata_path.Append(FILE_PATH_LITERAL("ascii_http_body.txt"))));
  ASSERT_TRUE(handle3.is_valid());
  builder.SetFileAttachment(
      "minidump", "minidump.dmp", handle3.get(), "text/plain");
...
TEST(HTTPMultipartBuilder, SharedFormDataAndAttachmentKeyNamespace) {
  ...
  ScopedFileHandle handle(LoggingOpenFileForRead(ascii_http_body_path));
  ASSERT_TRUE(handle.is_valid());
  builder.SetFileAttachment("minidump", "minidump.dmp", handle.get(), "");
...
TEST(HTTPMultipartBuilderDeathTest, AssertUnsafeMIMEType) {
  ...
  ASSERT_DEATH_CHECK(
      builder.SetFileAttachment("", "", kInvalidFileHandle, "\r\n"), "");
  ASSERT_DEATH_CHECK(
      builder.SetFileAttachment("", "", kInvalidFileHandle, "\""), "");
  ASSERT_DEATH_CHECK(
      builder.SetFileAttachment("", "", kInvalidFileHandle, "\x12"), "");
  ASSERT_DEATH_CHECK(
      builder.SetFileAttachment("", "", kInvalidFileHandle, "<>"), "");
  // Invalid but safe:
  builder.SetFileAttachment(
      "", "", kInvalidFileHandle, "0/totally/-invalid.pdf");
  // Valid and safe:
  builder.SetFileAttachment(
      "", "", kInvalidFileHandle, "application/xml+xhtml");
}
