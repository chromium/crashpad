...
TEST(HTTPMultipartBuilder, OverwriteFileAttachment) {
  ...
  builder.SetFileAttachment(
      "minidump", "minidump.dmp", &reader3, "text/plain");
...
TEST(HTTPMultipartBuilderDeathTest, AssertUnsafeMIMEType) {
  ...
  ASSERT_DEATH_CHECK(
      builder.SetFileAttachment("", "", &reader, "\r\n"), "");
  ASSERT_DEATH_CHECK(
      builder.SetFileAttachment("", "", &reader, "\""), "");
  ASSERT_DEATH_CHECK(
      builder.SetFileAttachment("", "", &reader, "\x12"), "");
  ASSERT_DEATH_CHECK(
      builder.SetFileAttachment("", "", &reader, "<>"), "");
  // Invalid but safe:
  builder.SetFileAttachment(
      "", "", &reader, "0/totally/-invalid.pdf");
  // Valid and safe:
  builder.SetFileAttachment(
      "", "", &reader, "application/xml+xhtml");
}
