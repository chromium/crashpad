...
void HTTPMultipartBuilder::SetGzip(bool enabled) {
  gzip_ = enabled;
}
...
std::unique_ptr<HTTPBodyStream> HTTPMultipartBuilder::GetBodyStream() {
  ...
  auto composite = std::unique_ptr<HTTPBodyStream>(new CompositeHTTPBodyStream(streams));
  if (gzip_) {
    return std::unique_ptr<HTTPBodyStream>(new GzipHTTPBodyStream(std::move(composite)));
  }
  return composite;
}

void HTTPMultipartBuilder::PopulateContentHeaders(HTTPHeaders* http_headers)
    const {
  std::string content_type =
      base::StringPrintf("multipart/form-data; boundary=%s", boundary_.c_str());
  http_headers->insert(std::make_pair(kContentType, content_type));

  if (gzip_) {
    http_headers->insert(std::make_pair(kContentEncoding, "gzip"));
  }
}
