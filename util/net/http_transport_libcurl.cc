...
#define DLSYM(symbol)                              \
  do {                                             \
    ...
    DLSYM(curl_easy_cleanup);
    DLSYM(curl_easy_init);
    DLSYM(curl_easy_perform);
    DLSYM(curl_easy_strerror);
    DLSYM(curl_easy_getinfo);
    DLSYM(curl_easy_setopt);
    DLSYM(curl_global_init);
    DLSYM(curl_slist_free_all);
    DLSYM(curl_slist_append);
    DLSYM(curl_version);

    return true;
  }

  crashpad::NoCfiIcall<decltype(curl_easy_cleanup)*> curl_easy_cleanup_;
  crashpad::NoCfiIcall<decltype(curl_easy_init)*> curl_easy_init_;
  crashpad::NoCfiIcall<decltype(curl_easy_perform)*> curl_easy_perform_;
  crashpad::NoCfiIcall<decltype(curl_easy_strerror)*> curl_easy_strerror_;
  crashpad::NoCfiIcall<decltype(curl_easy_getinfo)*> curl_easy_getinfo_;
  crashpad::NoCfiIcall<decltype(curl_easy_setopt)*> curl_easy_setopt_;
  crashpad::NoCfiIcall<decltype(curl_global_init)*> curl_global_init_;
  crashpad::NoCfiIcall<decltype(curl_slist_free_all)*> curl_slist_free_all_;
  crashpad::NoCfiIcall<decltype(curl_slist_append)*> curl_slist_append_;
  crashpad::NoCfiIcall<decltype(curl_version)*> curl_version_;
