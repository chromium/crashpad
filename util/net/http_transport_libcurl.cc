...
// Declares and initializes with dlsym() a static pointer to |function| and
// names it local_function.
#define STATIC_DLSYM(function)                                             \
  static const crashpad::NoCfiIcall<decltype(function)*> local_##function( \
      []() {                                                               \
        const auto local_##function = dlsym(Libcurl::Handle(), #function); \
        CHECK(local_##function) << "dlsym: " << dlerror();                 \
        return local_##function;                                           \
      }());

  static char* CurlVersion() {
    STATIC_DLSYM(curl_version)
    return local_curl_version();
  }

  static const char* CurlEasyStrError(CURLcode code) {
    STATIC_DLSYM(curl_easy_strerror);
    return local_curl_easy_strerror(code);
  }

  static void CurlEasyCleanup(CURL* curl) {
    STATIC_DLSYM(curl_easy_cleanup);
    return local_curl_easy_cleanup(curl);
  }

  template <typename Pointer>
  static CURLcode CurlEasyGetInfo(CURL* curl, CURLINFO info, Pointer out) {
    STATIC_DLSYM(curl_easy_getinfo);
    return local_curl_easy_getinfo(curl, info, out);
  }

  static CURL* CurlEasyInit() {
    STATIC_DLSYM(curl_easy_init);
    return local_curl_easy_init();
  }

  static CURLcode CurlEasyPerform(CURL* curl) {
    STATIC_DLSYM(curl_easy_perform);
    return local_curl_easy_perform(curl);
  }

  template <typename Pointer>
  static CURLcode CurlEasySetOpt(CURL* curl, CURLoption option, Pointer param) {
    STATIC_DLSYM(curl_easy_setopt);
    return local_curl_easy_setopt(curl, option, param);
  }

  static CURLcode CurlGlobalInit(long flags) {
    STATIC_DLSYM(curl_global_init);
    return local_curl_global_init(flags);
  }

  static void CurlSlistFreeAll(struct curl_slist* slist) {
    STATIC_DLSYM(curl_slist_free_all);
    return local_curl_slist_free_all(slist);
  }

  static struct curl_slist* CurlSlistAppend(struct curl_slist* slist,
                                            const char* data) {
    STATIC_DLSYM(curl_slist_append);
    return local_curl_slist_append(slist, data);
  }
