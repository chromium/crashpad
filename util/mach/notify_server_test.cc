...
mach_port_t SendOnceRightFromReceiveRight(mach_port_t receive_right) {
  ...
  EXPECT_EQ(
            acquired_type,implicit_cast<mach_msg_type_name_t>(MACH_MSG_TYPE_PORT_SEND_ONCE));
...
TEST_F(NotifyServerTest, Basic) {
  ...
  // trailer.
  EXPECT_EQ(
            server.MachMessageServerRequestSize(),offsetof(mach_port_destroyed_notification_t, trailer));
...
TEST_F(NotifyServerTest, MachNotifyDeadName) {
  ...
  EXPECT_EQ(
            RightRefCount(send_once_right.get(), MACH_PORT_RIGHT_SEND_ONCE),0u);
...
TEST_F(NotifyServerTest, MachNotifyDeadName_NoNotification) {
  ...
  EXPECT_EQ(
            RightRefCount(send_once_right.get(), MACH_PORT_RIGHT_SEND_ONCE),1u);
