...
bool ExcCrashCouldContainException(exception_type_t exception) {
  ...
  // EXC_GUARD. See 10.10 xnu-2782.1.97/bsd/kern/kern_exit.c proc_prepareexit().
  // Receiving these exception types wrapped in EXC_CRASH would lose information
  // because their code[0] uses all 64 bits (see below) and the code[0]
  // recovered from EXC_CRASH only contains 20 significant bits.
  //
  ...
  switch (exception) {
    ...
    default:
      ...
      if (exception == 0 && metrics_code_0 == 0 && signal != 0) {
        // This exception came from a signal that did not originate as a
        // hardware fault. Encode the signal number.
