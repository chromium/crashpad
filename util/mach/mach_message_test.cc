...
TEST(MachMessage, PrepareMIGReplyFromRequest_SetMIGReplyError) {
  ...
  EXPECT_EQ(
            reply.Head.msgh_bits,implicit_cast<mach_msg_bits_t>(
...
TEST(MachMessage, MachMessageTrailerFromHeader) {
  ...
  EXPECT_EQ(
            MachMessageTrailerFromHeader(&empty.rcv.header),&empty.rcv.trailer);
...
TEST(MachMessage, MachMessageDestroyReceivedPort) {
  ...
  ASSERT_EQ(
            right_type,implicit_cast<mach_msg_type_name_t>(MACH_MSG_TYPE_PORT_SEND));
  ...
  ASSERT_EQ(
            right_type,implicit_cast<mach_msg_type_name_t>(MACH_MSG_TYPE_PORT_SEND_ONCE));
  EXPECT_TRUE(
