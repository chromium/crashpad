...
TEST(ExceptionBehaviors, ExceptionBehaviors) {
  ...
  for (size_t index = 0; index < arraysize(kTestData); ++index) {
    ...
    EXPECT_EQ(
              ExceptionBehaviorHasIdentity(test_data.behavior),test_data.identity);
    EXPECT_EQ(
              ExceptionBehaviorHasMachExceptionCodes(test_data.behavior),test_data.mach_exception_codes);
    EXPECT_EQ(
              ExceptionBehaviorBasic(test_data.behavior),test_data.basic_behavior);
  }
}
