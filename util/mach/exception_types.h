...
//! `EXC_CRASH` exceptions that originate as POSIX signals, the mapping is
//! `(EXC_CRASH << 16) | code_0`.
//!
...
//! kernel-supplied bitmask having exactly one bit set with the index of that
//! bit.
//!
//! `EXC_CORPSE_NOTIFY` exceptions are reported as classic Mach exceptions with
//! the \a code_0 field set to `0`.
//!
//! Overflow conditions in any field are handled by saturation.
