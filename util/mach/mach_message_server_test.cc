...
class TestMachMessageServer : public MachMessageServer::Interface,
 ...
 public:
  ...
  virtual bool MachMessageServerFunction(
    ...
    *destroy_complex_request = options_.server_destroy_complex;
    ...
    EXPECT_EQ(
              request->header.msgh_size,options_.client_send_large ? sizeof(LargeRequestMessage)
    ...
    if (options_.client_send_complex) {
      ...
      EXPECT_EQ(
                request->port_descriptor.disposition,implicit_cast<mach_msg_type_name_t>(MACH_MSG_TYPE_MOVE_SEND));
      EXPECT_EQ(
    ...
    EXPECT_EQ(
              trailer->msgh_trailer_type,implicit_cast<mach_msg_trailer_type_t>(MACH_MSG_TRAILER_FORMAT_0));
 ...
 private:
  ...
  void MachMultiprocessParent() override {
    ...
    ASSERT_EQ(
              (kr = MachMessageServer::Run(this,
                                           ...
                                           options_.server_timeout_ms)),options_.expect_server_result)
  ...
  void ChildWaitForReply() {
    ...
    ASSERT_EQ( reply.Head.msgh_bits,implicit_cast<mach_msg_bits_t>(
    ...
    ASSERT_EQ(
              reply.trailer.msgh_trailer_type,implicit_cast<mach_msg_trailer_type_t>(MACH_MSG_TRAILER_FORMAT_0));
