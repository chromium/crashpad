...
struct __attribute__((packed, aligned(4))) ExceptionRaiseReply {
  ...
  void Verify(exception_behavior_t behavior) {
    EXPECT_EQ(
              Head.msgh_bits,implicit_cast<mach_msg_bits_t>(
...
TEST(ExcServerVariants, MachMessageServerRequestIDs) {
  ...
  EXPECT_EQ(
            universal_mach_exc_server.MachMessageServerRequestIDs(),expect_request_ids);
}
  ...
  virtual kern_return_t CatchMachException(
    ...
    *destroy_complex_request = true;
    ...
    if (has_state) {
      ...
      EXPECT_EQ(
                *new_state_count,implicit_cast<mach_msg_type_number_t>(THREAD_STATE_MAX));
    ...
    EXPECT_EQ(
              trailer->msgh_trailer_size,REQUESTED_TRAILER_SIZE(kMachMessageOptions));
  ...
  for (size_t index = 0; index < arraysize(kTestData); ++index) {
    ...
    EXPECT_EQ(
              ExcServerSuccessfulReturnValue(test_data.exception,
                                             test_data.behavior,
                                             test_data.set_thread_state),test_data.kr);
  }
}
