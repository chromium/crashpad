...
zx::handle GetChildHandleByKoid(const zx::object_base& parent,
...
zx::process FindProcess(const zx::job& job, zx_koid_t koid) {
  // TODO(wez): Use GetChildHandleByKoid() instead?
  for (auto child_koid : GetChildKoids(job, ZX_INFO_JOB_PROCESSES)) {
    if (child_koid == koid)
      return zx::process(GetChildHandleByKoid(job, koid));
  }
  ...
  for (auto child_koid : GetChildKoids(job, ZX_INFO_JOB_CHILDREN)) {
    zx::job child_job = zx::job(GetChildHandleByKoid(job, child_koid));
    zx::process process = FindProcess(child_job, koid);
    if (process.is_valid())
      return process;
  }
...
zx::thread GetThreadHandleByKoid(const zx::process& parent,
                                 zx_koid_t child_koid) {
  return zx::thread(GetChildHandleByKoid(parent, child_koid));
}
...
zx::process GetProcessFromKoid(zx_koid_t koid) {
  zx::process result = FindProcess(GetRootJob(), koid);
  if (!result.is_valid())
