...
zx::job GetRootJob() {
  ...
  zx::channel channel;
  zx_status_t status =
      fdio_get_service_handle(sysinfo.get(), channel.reset_and_get_address());
  if (status != ZX_OK) {
  ...
  zx::handle root_job;
  zx_status_t fidl_status = zircon_sysinfo_DeviceGetRootJob(
      channel.get(), &status, root_job.reset_and_get_address());
  if (fidl_status != ZX_OK) {
    ZX_LOG(ERROR, fidl_status) << "zircon_sysinfo_DeviceGetRootJob";
    return zx::job();
  } else if (status != ZX_OK) {
    ZX_LOG(ERROR, status) << "zircon_sysinfo_DeviceGetRootJob";
    return zx::job();
  }
