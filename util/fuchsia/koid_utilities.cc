...
// Returns |handle| if it contains a handle of the correct type for T, otherwise
// returns a null handle.
template <typename T>
T CheckHandleType(T handle) {
  ...
  if (actual.type != handle.TYPE) {
    LOG(ERROR) << "Wrong type: " << actual.type << ", expected " << handle.TYPE;
    return T();
  }
  return handle;
}
...
T GetChildHandleByKoid(const U& parent, zx_koid_t child_koid, bool* was_found) {
  ...
  return CheckHandleType(std::move(handle));
}
