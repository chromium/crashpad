...
//!
//! Currently, suspension and resume are not counted on Fuchsia, so while this
...
//! its name) not full tasks. As a result, it's possible some threads may not
//! be correctly suspended/resumed as their creation might race enumeration.
//!
//! As a result of these limitations, this class is limited to being a vague
//! best-effort, and full suspension cannot be relied upon.
//!
//! Callers should not attempt to suspend the current task `zx_process_self()`.
