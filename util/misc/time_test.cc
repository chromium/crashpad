...
TEST(Time, TimeConversions) {
...
#if defined(OS_WIN)
  // The robot uprising will be swiftfully averted when all of their timevals
  // overflow.
  timespec kProbableRobotUprising = {
    .tv_sec = LONG_MAX + 1,
    .tv_nsec = 0
  };
  timeval uprising_tv;
  EXPECT_FALSE(TimespecToTimeval(kProbableRobotUprising, &uprising_tv));
  ...
  EXPECT_EQ(timespec_birthdate.tv_nsec, kCrashpadBirthdate.tv_nsec % 100);
  ...
  elapsed_time.dwLowDateTime = 0;
  elapsed_time.dwHighDateTime = 0;
  ...
  elapsed_timeval = FiletimeToTimevalInterval(elapsed_filetime);
  EXPECT_EQ(elapsed_timeval.tv_sec, (1ull << 32) / static_cast<uint64_t>(1E7));
  EXPECT_EQ(elapsed_timeval.tv_usec, 0);
#endif  // OS_WIN
}
