...
MaybeRemoveVolatile(const T& value) {
  return const_cast<typename std::remove_volatile<typename std::remove_pointer<T>::type>::type*>(value);
}
#else
// This isn’t a problem in MSVS 2017 (MSC 19.1) or with other compilers.
template <typename T>
T MaybeRemoveVolatile(const T& value) { return value; }
#endif
...
TYPED_TEST(FromPointerCastTest, ToPointer) {
  ...
  EXPECT_EQ(MaybeRemoveVolatile(FromPointerCast<CVSomeType*>(reinterpret_cast<TypeParam>(1))),
  ...
  EXPECT_EQ(
      ...
      MaybeRemoveVolatile(reinterpret_cast<CVSomeType*>(std::numeric_limits<uintptr_t>::max())));
  EXPECT_EQ(
      MaybeRemoveVolatile(FromPointerCast<CVSomeType*>(
          reinterpret_cast<TypeParam>(std::numeric_limits<intptr_t>::min()))),
      MaybeRemoveVolatile(reinterpret_cast<CVSomeType*>(std::numeric_limits<intptr_t>::min())));
  EXPECT_EQ(
      MaybeRemoveVolatile(FromPointerCast<CVSomeType*>(
          reinterpret_cast<TypeParam>(std::numeric_limits<intptr_t>::max()))),
      MaybeRemoveVolatile(reinterpret_cast<CVSomeType*>(std::numeric_limits<intptr_t>::max())));
}
