...
TEST(NoCfiIcall, Args) {
#if defined(OS_WIN)
  HMODULE module = LoadLibrary(L"ucrtbase.dll");
  ASSERT_TRUE(module);

  EXPECT_TRUE(GetProcAddress(module, "snprintf"));
  EXPECT_TRUE(GetProcAddress(module, "_snprintf"));
  EXPECT_TRUE(GetProcAddress(module, "_snprintf_s"));
  EXPECT_TRUE(GetProcAddress(module, "_snprintf_s_l"));

  using SNPrintfType = int (*)(char*, size_t, size_t, const char*, ...);
  static const NoCfiIcall<SNPrintfType> call(
      GetProcAddress(module, "_snprintf_s"));
  ASSERT_TRUE(call);
  ...
  ASSERT_GT(call(buf, sizeof(buf), sizeof(buf), "Hello!"), 0);
  EXPECT_STREQ(buf, "Hello!");

  // Variadic args.
  memset(buf, 0, sizeof(buf));
  ASSERT_GT(call(buf, sizeof(buf), sizeof(buf), "%s, %s!", "Hello", "World"),
            0);
