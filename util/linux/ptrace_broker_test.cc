 ...
 protected:
  void PreFork() override {
    ...
    ASSERT_TRUE(mapping_.ResetMmap(nullptr,
                                   ...
                                   -1,
                                   0));
    ASSERT_TRUE(mapping_.ResetAddrLen(mapping_.addr(), getpagesize()));
 ...
 private:
  void MultiprocessParent() override {
...

    {
      ...
      for (size_t index = 0; index < mapping_.len(); ++index) {
        EXPECT_EQ(buffer[index], mapping_.addr_as<char*>()[index]);
      }

      char last;
      ASSERT_TRUE(client.Read(
          mapping_.addr_as<VMAddress>() + mapping_.len() - 1, 1, &last));
      EXPECT_EQ(last, mapping_.addr_as<char*>()[mapping_.len() - 1]);
    }
  }
