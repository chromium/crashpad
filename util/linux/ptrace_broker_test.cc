...
class ScopedRunBrokerThread : public Thread {
 public:
  ScopedRunBrokerThread(PtraceBroker* broker)
      : Thread(), broker_(broker), join_sem_(0) {}

  ~ScopedRunBrokerThread() { EXPECT_TRUE(JoinWithTimeout(5.0)); }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ScopedRunBrokerThread);
};

class ScopedBlockOnReadThread : public Thread {
 public:
  explicit ScopedBlockOnReadThread(int readfd, int writefd)
      : readfd_(readfd), writefd_(writefd), join_sem_(0) {}

  ~ScopedBlockOnReadThread() { EXPECT_TRUE(JoinWithTimeout(5.0)); }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ScopedBlockOnReadThread);
};
...
class SameBitnessTest : public Multiprocess {
 ...
 private:
  void MultiprocessParent() override {
    ...
    ScopedRunBrokerThread broker_thread(&broker);
    broker_thread.Start();
  ...
  void MultiprocessChild() override {
    ScopedBlockOnReadThread thread(ReadPipeHandle(), WritePipeHandle());
    thread.Start();
