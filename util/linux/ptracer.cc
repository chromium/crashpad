...
bool GetThreadArea32(pid_t tid, const ThreadContext& context, LinuxVMAddress* address) {
...
bool GetThreadArea64(pid_t tid, const ThreadContext& context, LinuxVMAddress* address) {
...
bool GetGeneralPurposeRegisters32(pid_t tid, ThreadContext* context) {
  if(GetGeneralPurposeRegistersAndLength(tid, context) != sizeof(context->t32)) {
    LOG(ERROR) << "Unexpected registers size";
    return false;
  }
  return true;
...
bool GetGeneralPurposeRegisters64(pid_t tid, ThreadContext* context) {
  if(GetGeneralPurposeRegistersAndLength(tid, context) != sizeof(context->t64)) {
    LOG(ERROR) << "Unexpected registers size";
    return false;
  }
  return true;
...
Ptracer::Ptracer() : is_64_bit_(false) {}
...
#if defined(ARCH_CPU_X86_FAMILY)
        GetThreadArea32(tid, info->thread_context, &info->thread_specific_data_address);
#else
...
#if defined(ARCH_CPU_X86_FAMILY)
        GetThreadArea64(tid, info->thread_context, &info->thread_specific_data_address);
#else
