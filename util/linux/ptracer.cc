...
// Reads at most size bytes at address, if address is within a word of a page
// boundary.
ssize_t Ptracer::ReadLastBytes(pid_t pid,
  ...
  size_t bytes_already_read = address - aligned;
  size_t last_bytes = std::min(sizeof(long) - bytes_already_read, size);
  memcpy(
      buffer, reinterpret_cast<char*>(&word) + bytes_already_read, last_bytes);
  return last_bytes;
}
