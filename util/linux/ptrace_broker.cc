...
int PtraceBroker::RunImpl() {
  while (true) {
    ...
    switch (request.type) {
      LOG(INFO) << "Broker received request " << static_cast<uint16_t>(request.type);
      case Request::kTypeAttach: {
        ...
        LOG(INFO) << "trying to attach thread " << request.tid;
      ...
      case Request::kTypeReadFile: {
        ...
        DCHECK_GE(sizeof(path_buffer), request.path_length);

        if (!ReadFileExactly(sock_, path_buffer, request.path_length)) {
          return errno;
        }
