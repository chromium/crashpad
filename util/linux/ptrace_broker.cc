...
size_t FormatPID(char* buffer, pid_t pid) {
  ...
  do {
    ...
    pid /= 10;
    ++length;
  } while (pid > 0 && length < sizeof(pid_buf));
  ...
  while (true) {
    ...
    switch (request.type) {
      ...
      case Request::kTypeOpenMemoryFile: {
        ...
        OpenResult open_rv = memory_file_.is_valid()
                                 ? kOpenResultSuccess
...
int PtraceBroker::SendMemory(pid_t pid, VMAddress address, VMSize size) {
  auto ReadMemory = [this, pid](VMAddress address, size_t size, char* buffer) {
  ...
  while (size > 0) {
    ...
    int32_t bytes_read = ReadMemory(address, to_read, buffer);
