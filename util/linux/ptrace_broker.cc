  ...
  while (true) {
    ...
    switch (request.type) {
      ...
      case Request::kTypeReadFile: {
        ...
        if (!ReadFileAllowed(path_buffer, request.path.path_length)) {
          int32_t result = -1;
          Errno err = EINVAL;
          if (!WriteFile(sock_, &result, sizeof(result)) ||
              !WriteFile(sock_, &err, sizeof(err))) {
            return errno;
          }
          continue;
        }
...
bool PtraceBroker::ReadFileAllowed(const char* path, size_t path_length) {
  return path_length > strlen(file_root_) && path[path_length - 1] == '\0' &&
         strncmp(path, file_root_, strlen(file_root_)) == 0;
}
