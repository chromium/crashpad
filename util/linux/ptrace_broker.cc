  ...
  while (true) {
    ...
    switch (request.type) {
      case Request::Type::kAttach: {
        ...
        if (attach_on_stack && status == Bool::kTrue) {
          return RunImpl();
        }
        continue;
      }
      ...
      case Request::Type::kGetThreadInfo: {
        ...
        if (response.success == Bool::kFalse) {
          Errno error = errno;
          if (!WriteFile(sock_, &error, sizeof(error))) {
            return errno;
          }
...
bool PtraceBroker::AllocateAttachments() {
  ...
  attach_capacity_ += page_size / sizeof(ScopedPtraceAttach);
  return true;
}
