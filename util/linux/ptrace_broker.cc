...
class PtraceBroker::AttachmentsArray {
 public:
  AttachmentsArray() : allocation_(), attach_count_(0) {}
  ...
  ScopedPtraceAttach* AllocateAttachment() {
    if (attach_count_ >= (allocation_.len() / sizeof(ScopedPtraceAttach))) {
      return nullptr;
    }
  ...
  ~AttachmentsArray() {
    for (size_t index = 0; index < attach_count_; ++index) {
      Attachments()[index].Reset();
    }
  }

  ScopedPtraceAttach* Attachments() {
    return allocation_.addr_as<ScopedPtraceAttach*>();
  }
...
int PtraceBroker::Run() {
  ...
  int result = RunImpl(&attachments);
  return result;
}
  ...
  while (true) {
    ...
    switch (request.type) {
      case Request::kTypeAttach: {
        AttachmentsArray more_attachments;

        bool recurse = false;
        ScopedPtraceAttach* attach = attachments->AllocateAttachment();
        if (!attach) {
          more_attachments.Initialize();
          attach = more_attachments.AllocateAttachment();
          recurse = true;
        }
        ...
        if (recurse && status == ExceptionHandlerProtocol::kBoolTrue) {
          return RunImpl(&more_attachments);
        }
