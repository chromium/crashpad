...
TEST(MemoryMap, SelfLargeMapFile) {
  ...
  for (size_t index = 0; index < kNumMappings - 1; ++index) {
    auto mapping_address =
        mappings.addr_as<LinuxVMAddress>() + index * page_size;
    const MemoryMap::Mapping* mapping = map.FindMapping(mapping_address);
    ASSERT_TRUE(mapping);
    EXPECT_EQ(mapping_address, mapping->range.Base()) << index;
    EXPECT_EQ(mapping_address + page_size, mapping->range.End()) << index;
    EXPECT_TRUE(mapping->readable);
    if (index % 2 == 0) {
      EXPECT_TRUE(mapping->writable);
      EXPECT_TRUE(mapping->readable);
