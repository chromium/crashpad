...
class MapParentTest : public Multiprocess {
 ...
 private:
  void MultiprocessParent() override {
    ...
    ScopedMmap mapping;
    ASSERT_TRUE(mapping.ResetMmap(nullptr,
                                  getpagesize(),
                                  PROT_NONE,
                                  MAP_SHARED | MAP_ANON,
                                  -1,
    ...
    base::FilePath path = temp_dir.path().Append(FILE_PATH_LITERAL("test_file"));
    ASSERT_FALSE(FileExists(path));
    std::string path_string = path.value();
    ScopedFileHandle handle(HANDLE_EINTR(open(path_string.c_str(), O_CREAT | O_RDWR | O_CLOEXEC | O_NOCTTY)));
  ...
  void MultiprocessChild() override {
    ...
    EXPECT_EQ(mapped_address + getpagesize(), mapping->range.End());
