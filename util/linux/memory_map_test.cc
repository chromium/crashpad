...
void InitializeMappings(ScopedMmap* mappings,
  ...
  for (size_t index = 0; index < num_mappings; ++index) {
    // If successive regions are mapped with the same protections, they get
    // merged into a single mapping. Will this test be flaky if some systems try
    // harder to merge regions?
    int prot = PROT_READ;
    if (index % 2 == 0) {
      prot |= PROT_WRITE;
    } else {
    ...
    ASSERT_TRUE(mappings[index].ResetMmap(
        nullptr, mapping_size, prot, MAP_PRIVATE | MAP_ANON, -1, 0));
  }
}

TEST(MemoryMap, SelfLargeMapFile) {
  const size_t kNumMappings = 4096;
  ScopedMmap mappings[kNumMappings];

  ASSERT_NO_FATAL_FAILURE(
      InitializeMappings(mappings, kNumMappings, getpagesize()));

  MemoryMap map;
  ASSERT_TRUE(map.Initialize(getpid()));

  for (size_t index = 0; index < kNumMappings; ++index) {
    auto mapping_address = mappings[index].addr_as<LinuxVMAddress>();
    const MemoryMap::Mapping* mapping = map.FindMapping(mapping_address);
    ASSERT_TRUE(mapping);
    EXPECT_EQ(mapping_address, mapping->range.Base());
    EXPECT_EQ(mapping_address + mappings[index].len(), mapping->range.End());
    EXPECT_TRUE(mapping->readable);
    if (index % 2 == 0) {
      EXPECT_TRUE(mapping->writable);
      EXPECT_FALSE(mapping->executable);
...
class MapRunningChildTest : public Multiprocess {
 public:
  MapRunningChildTest()
      : Multiprocess(), kNumMappings(4096), page_size(getpagesize()) {}
  ~MapRunningChildTest() {}
 ...
 private:
  ...
  void MultiprocessChild() override {
    ...
    ScopedMmap extra_mappings[kNumExtraMappings];
