...
class AuxiliaryVector {
 public:
  ...
  //! \brief Initializes this object with the auxiliary vector for the thread
  //!     with thread ID \a tid.
  //!
  //! This method must be called successfully prior to calling any other method
  //! in this class.
  //!
  //! \param[in] tid The thread ID of a target thread.
  //! \param[in] is_64_bit Whether the target thread is 64-bit.
  //!
  //! \return `true` on success, `false` on failure with a message logged.
  bool Initialize(pid_t tid, bool is_64_bit);
