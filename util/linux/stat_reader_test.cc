...
#include "util/linux/stat_reader.h"
...
class StatTimeThread : public Thread {
 ...
 private:
  void ThreadMain() override {
    GetStartTime(start_time_);
  }
  timeval* start_time_;
};

TEST(StatReader, Threads) {
  timeval main_time;
  ...
  EXPECT_PRED4([](time_t main_sec, suseconds_t main_usec, time_t thread_sec, suseconds_t thread_usec) {
    return (thread_sec > main_sec) || (thread_sec == main_sec && thread_usec > main_usec);
  },
  main_time.tv_sec, main_time.tv_usec, thread_time.tv_sec, thread_time.tv_usec);
}
