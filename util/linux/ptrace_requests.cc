...
// (0693bf68148c4). Older Linux kernels support PTRACE_GETREGS,
// PTRACE_GETFPREGS, and PTRACE_GETVFPREGS
//
// Fortunately, 64-bit ARM support only appeared in
...
GetRegistersResult GetGeneralPurposeRegisters(pid_t pid,
  ...
  } else if (iov.iov_len == sizeof(context->t64)) {
  ...
  if (ptrace(
    ...
    switch (errno) {
      ...
      case EINVAL:
        ...
        // It should have a VFP register set instead
        break;
      default:
        PLOG(ERROR) << "ptrace";
        return GetRegistersResult::kError;
