...
GetRegistersResult GetFloatingPointRegisters(pid_t pid, FloatContext* context) {
  ...
  if (ptrace(
    ...
    if (iov.iov_len == sizeof(context->f64)) {
      // Target process is 64-bit; there are no other register sets.
      return GetRegistersResult::k64Bit;
    }
...
bool GetThreadArea(pid_t pid, LinuxVMAddress* address) {
  ...
  if (ptrace(
    ...
    switch (errno) {
      case EIO:
      case EINVAL:
        // Assume the target process is 32-bit and use GET_THREAD_AREA instead.
        break;
      default:
        PLOG(ERROR) << "ptrace";
        return false;
...
#if defined(ARCH_CPU_ARMEL)
  void* result;
  if (ptrace(PTRACE_GET_THREAD_AREA, pid, nullptr, &result) != 0) {
    PLOG(ERROR) << "ptrace";
    return false;
  }
  ...
  *address = 0;
  ...
  return true;
#else
  // x86 tracing x86_64 should use the fs_base value from
  // GetGeneralPurposeRegisters instead.
  return false;
