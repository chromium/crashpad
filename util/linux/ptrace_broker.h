...
#pragma pack(push, 1)
  ...
  struct Request {
    ...
    enum Type : uint16_t {
      ...
      //!     Bool::kTrue on success, otherwise Bool::kFalse, followed by an
      //!     Errno.
      kAttach,
      ...
      //!     GetThreadInfoResponse::success is set to Bool::kFalse and is
      //!     followed by an Errno.
      kGetThreadInfo,

      //! \brief Reads memory from the attached process. The data is returned in
      //!     a series of messages, each starting with a VMSize of the number of
      //!     bytes returned, followed by the bytes read. If an error occurs
      //!     a VMSize equal to zero is returned, followed by an Errno.
      kReadMemory,
...
#pragma pack(pop)
  ...
  //!
  //! \return 0 if Run() exited due to an exit request. Otherwise returns an
  //!     error code.
  int Run();
