...
#pragma pack(push, 1)
  ...
  struct Request {
    ...
    enum Type : uint16_t {
      ...
      //! \brief Directs the broker to attempt to initialize a memory file. The
      //!     file path is determined by the broker. Responds with an
      //!     OpenResult.
      kTypeOpenMemoryFile,
...
#pragma pack(pop)
  ...
  //!     "/proc/[pid]/" and enables memory reading via /proc/[pid]/mem. If \a
  //!     pid is -1, the broker will read memory via `ptrace PEEKDATA`.
