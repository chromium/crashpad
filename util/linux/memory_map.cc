...
#define IF_EQ_SET_FIELD(c1, c2, dest, value) \
  if (c1 == c2) {                            \
    *dest = value;                           \
  }
#define SET_FIELD1(actual_c, outval, true_c, false_c)           \
  IF_EQ_SET_FIELD(actual_c, true_c, outval, true)               \
  else IF_EQ_SET_FIELD(actual_c, false_c, outval, false) else { \
    LOG(ERROR) << "format error";                               \
    return false;                                               \
  }
#define SET_FIELD2(actual_c, outval, true_c, true_c2, false_c) \
  IF_EQ_SET_FIELD(actual_c, true_c2, outval, true)             \
  else SET_FIELD1(actual_c, outval, true_c, false_c)

    SET_FIELD1(field[0], &mapping.readable, 'r', '-');
    SET_FIELD1(field[1], &mapping.writable, 'w', '-');
    SET_FIELD1(field[2], &mapping.executable, 'x', '-');
    SET_FIELD2(field[3], &mapping.shareable, 's', 'S', 'p');
#undef SET_FIELD2
#undef SET_FIELD1
#undef IF_EQ_SET_FIELD
