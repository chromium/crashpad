...
void MemoryMap::Print() {
  for (const auto& mapping : mappings_) {
    LOG(INFO) << "0x" << std::hex << mapping.range.Base() << ":0x" << mapping.range.Base() + mapping.range.Size() << " " << mapping.name << " " << mapping.offset << " " << mapping.readable << " " << mapping.writable << " " << mapping.executable;
  }
}
