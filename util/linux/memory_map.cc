...
#define CMP_SET_FIELD(actual_c, outval, cmp_c, eq_val) \
  if (actual_c == cmp_c) {                             \
    *outval = eq_val;                                  \
  }
#define SET_FIELD1(actual_c, outval, true_c, false_c)         \
  CMP_SET_FIELD(actual_c, outval, true_c, true)               \
  else CMP_SET_FIELD(actual_c, outval, false_c, false) else { \
    LOG(ERROR) << "format error";                             \
    return false;                                             \
  }
#define SET_FIELD2(actual_c, outval, true_c, true_c2, false_c) \
  CMP_SET_FIELD(actual_c, outval, true_c2, true)               \
