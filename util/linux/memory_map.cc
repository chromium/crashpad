...
bool ReadMapsFile(pid_t pid, std::string* contents) {
  char path[32];
  ...
  while ((length = maps_file.Read(buffer, sizeof(buffer))) > 0) {
...
#undef SET_FIELD
  ...
  if (path_start != std::string::npos) {
    mapping->name = field.substr(path_start);
  }
...
bool MemoryMap::Initialize(pid_t pid) {
  ...
  int attempts = 3;
  do {
    // Read the entire maps file as quickly as possible to reduce the
    // opportunity for errors caused by changing mappings.
    std::string contents;
    if (!ReadMapsFile(pid, &contents)) {
      return false;
    }
    ...
    Mapping mapping;
    ParseResult result;
    LinuxVMAddress last_address = 0;
    while (
        (result = ParseMapsLine(&maps_file_reader, last_address, &mapping)) ==
        ParseResult::kSuccess) {
      mappings_.push_back(mapping);
      last_address = mappings_.back().range.End();
    }
