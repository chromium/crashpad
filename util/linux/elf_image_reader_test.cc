...
TEST(ElfImageReader, MainExecutableSelf) {
  ...
  LOG(INFO) << "initializing elf at 0x" << std::hex << elf_address;
  ...
  LinuxVMAddress foo_addr;
  LinuxVMSize foo_size;
  ASSERT_TRUE(reader.GetSymbol("foo", &foo_addr, &foo_size));
  EXPECT_EQ(foo_addr, FromPointerCast<LinuxVMAddress>(foo));
}
