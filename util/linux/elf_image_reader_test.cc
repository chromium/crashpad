...
void ReadElfForTargetAndSymbol(pid_t pid, std::string name, void* addr) {
...
void ReadExecutableForTarget(pid_t pid) {
  ReadElfForTargetAndSymbol(pid, "foo", reinterpret_cast<void*>(foo));
}

void ReadLibcForTarget(pid_t pid) {
  ReadElfForTargetAndSymbol(pid, "getpid", reinterpret_cast<void*>(getpid));
}
