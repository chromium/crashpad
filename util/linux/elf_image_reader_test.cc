...
void ExpectElfImageWithSymbol(pid_t pid,
  ...
  ASSERT_TRUE(reader.Initialize(&memory, address));
  EXPECT_EQ(reader.Is64Bit(), is_64_bit);

  LinuxVMAddress symbol_address;
  LinuxVMSize symbol_size;
  ASSERT_TRUE(reader.GetSymbol(symbol_name, &symbol_address, &symbol_size));
  EXPECT_EQ(symbol_address, expected_symbol_address);

  EXPECT_FALSE(reader.GetSymbol("notasymbol", &symbol_address, &symbol_size));
}
...
// TODO(jperaza): Update test to locate libc using an out-of-process strategy.
