...
class BlockingThread : public Thread {
 public:
  BlockingThread() : tid_sem_(0), join_sem_(0), tid_(-1) {}
...
TEST(ProcTaskReader, Self) {
  ...
  BlockingThread thread1;
  thread1.Start();

  BlockingThread thread2;
  thread2.Start();
 ...
 private:
  ...
  void MultiprocessParent() override {
    ...
    for (size_t index = 0; index < ids_to_find.size(); ++index) {
      SCOPED_TRACE(base::StringPrintf("index %lu, tid %d", index, ids_to_find[index]));
