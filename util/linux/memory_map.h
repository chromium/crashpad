...
class MemoryMap {
 public:
  ...
  //!
  //! \a mapping must refer to a valid mapping in this MemoryMap. If \a mapping
  //! is found but does not map a file, \a mapping is returned in \a
  //! possible_starts.
  //!
  //! \param[in] mapping A Mapping whose series to find the start of.
  //! \param[out] possible_starts The mappings found.
  //! \return `true` on success. Otherwise, `false` with a message logged.
  bool FindFileMmapStart(const Mapping& mapping,
                         std::vector<const Mapping*>* possible_starts) const;
