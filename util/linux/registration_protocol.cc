...
bool RegisterWithHandler(int server_sock,
  ...
  *reinterpret_cast<int*>(CMSG_DATA(cmsg)) = registration_socket.get();
...
bool RequestCrashDump(int registration_socket) {
  ...
  if (sendmsg(registration_socket, &msg, MSG_NOSIGNAL) < 0) {
    PLOG(ERROR) << "sendmsg";
    return false;
  }
...
bool WaitForCrashDump(int registration_socket) {
  bool success;
  return LoggingReadFileExactly(
             registration_socket, &success, sizeof(success)) &&
         success;
}
