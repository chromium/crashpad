...
bool ThreadInfo::InitializeBitness() {
...
bool ThreadInfo::Is64Bit(bool* is_64_bit) {
  ...
  if (is_64_bit_initialized_.is_uninitialized() && !InitializeBitness()) {
    return false;
  }
...
// This method initializes is_64_bit_ and tls_address_ and should be called
// before any other methods that require them.
...
bool ThreadInfo::GetFloatingPointRegisters(FloatContext* context) {
  ...
  if (is_64_bit_) {
    return GetFloatingPointRegisters64(tid_, context);
  }
  return GetFloatingPointRegisters32(tid_, context);
#else
#error Port.
#endif  // ARCH_CPU_X86_FAMILY
}
