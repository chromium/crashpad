...
bool CheckedGetRegisterSet(pid_t tid, int set, size_t size, void* dest) {
  iovec iov;
  iov.iov_base = dest;
  iov.iov_len = size;
...
bool GetFloatingPointRegisters32(pid_t tid, FloatContext* context) {
  iovec iov;
  iov.iov_base = &context->f32.fxsave;
  iov.iov_len = sizeof(context->f32.fxsave);
  if (ptrace(
          PTRACE_GETREGSET, tid, reinterpret_cast<void*>(NT_PRXFPREG), &iov) !=
      0) {
    switch (errno) {
      case ENODEV:
        break;
  ...
  return CheckedGetRegisterSet(
      tid, NT_PRFPREG, sizeof(context->f32.fsave), &context->f32.fsave);
}

bool GetFloatingPointRegisters64(pid_t tid, FloatContext* context) {
  return CheckedGetRegisterSet(
      tid, NT_PRFPREG, sizeof(context->f64), &context->f64);
}
