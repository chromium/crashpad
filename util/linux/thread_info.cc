...
bool GetGeneralPurposeRegistersLegacy(pid_t pid, ThreadContext* context) {
...
bool GetFloatingPointRegistersLegacy(pid_t pid, FloatContext* context) {
...
bool GetFloatingPointRegisters64(pid_t pid, FloatContext* context) {
  iovec iov;
...
bool GetFloatingPointRegisters32(pid_t pid, FloatContext* context) {
  ...
  } else if (iov.iov_len != sizeof(context->f32.fpregs)) {
    LOG(ERROR) << "Unexpected registers size";
    return false;
  }
...
ThreadInfo::ThreadInfo()
    : attachment_(), pid_(-1), initialized_(), is_64_bit_(false) {}

ThreadInfo::~ThreadInfo() {}

bool ThreadInfo::Initialize(pid_t pid) {
  INITIALIZATION_STATE_SET_INITIALIZING(initialized_);

  if (!attachment_.ResetAttach(pid)) {
    return false;
  }
  pid_ = pid;

  ThreadContext context;
  if (!GetGeneralPurposeRegisters(&context)) {
    return false;
  }
...
bool ThreadInfo::Is64Bit(bool* is_64_bit) {
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
  *is_64_bit = is_64_bit_;
  return true;
}

bool ThreadInfo::GetGeneralPurposeRegisters(ThreadContext* context) {
  iovec iov;
  iov.iov_base = context;
  iov.iov_len = sizeof(*context);
  if (ptrace(
          PTRACE_GETREGSET, pid_, reinterpret_cast<void*>(NT_PRSTATUS), &iov) !=
      0) {
    switch (errno) {
#if defined(ARCH_CPU_ARMEL)
      case EIO:
        is_64_bit_ = false;
        return GetGeneralPurposeRegistersLegacy(pid_, context);
#endif  // ARCH_CPU_ARMEL
      default:
        PLOG(ERROR) << "ptrace";
        return false;
...
#if defined(ARCH_CPU_X86_FAMILY)
  if (is_64_bit_) {
    tls_address_ = context->t64.fs_base;
  }
...
bool ThreadInfo::GetFloatingPointRegisters(FloatContext* context) {
  ...
  iov.iov_len = sizeof(*context);
  if (ptrace(
          PTRACE_GETREGSET, pid_, reinterpret_cast<void*>(NT_PRFPREG), &iov) !=
      0) {
    PLOG(ERROR) << "ptrace";
    return false;
  }
  ...
  if (is_64_bit_) {
    return GetFloatingPointRegisters64(pid_, context);
  }
  return GetFloatingPointRegisters32(pid_, context);
#else
#error Port.
#endif  // ARCH_CPU_X86_FAMILY
}
...
bool ThreadInfo::GetThreadArea(LinuxVMAddress* address) {
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
#if defined(ARCH_CPU_ARM_FAMILY)
  // The TLS for a 64-bit target process can be retrieved with the NT_ARM_TLS
  // register set.
  if (is_64_bit_) {
    iovec iov;
...
#if defined(ARCH_CPU_ARMEL)
  void* result;
  if (ptrace(PTRACE_GET_THREAD_AREA, pid_, nullptr, &result) != 0) {
    PLOG(ERROR) << "ptrace";
    return false;
  }
  ...
  *address = 0;
  if (ptrace(
          PTRACE_GETREGSET, pid_, reinterpret_cast<void*>(NT_386_TLS), &iov) !=
      0) {
    PLOG(ERROR) << "ptrace";
    return false;
  }
