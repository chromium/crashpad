  ...
  do {
    ...
    switch (read_result) {
      case PtraceBroker::kReadResultAccessDenied:
        LOG(ERROR) << "PtraceBroker ReadFileContents access denied";
        return false;
      case PtraceBroker::kReadResultPathTooLong:
        LOG(ERROR) << "PtraceBroker ReadFileContents path too long";
        return false;
      case PtraceBroker::kReadResultErrno:
        ReceiveAndLogError(sock_, "PtraceBroker ReadFileContents");
        return false;
      default:
        if (read_result < 0) {
