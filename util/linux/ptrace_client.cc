...
ssize_t PtraceClient::ReadUpTo(VMAddress address, size_t size, void* buffer) const {
...
bool PtraceClient::ReadFileContents(const base::FilePath& path,
  ...
  if (!LoggingWriteFile(sock_, &request, sizeof(request)) ||
      !LoggingWriteFile(
          sock_, path.value().c_str(), request.path.path_length)) {
    return false;
  }

  std::string local_contents;
  PtraceBroker::ReadResult read_result;
  do {
    ...
    switch (read_result) {
      case PtraceBroker::kReadResultAccessDenied:
        LOG(ERROR) << "access denied";
        return false;
      case PtraceBroker::kReadResultPathTooLong:
        LOG(ERROR) << "path too long";
        return false;
      case PtraceBroker::kReadResultErrno:
        ReceiveAndLogError(sock_, "ReadFileContents");
        return false;
      default:
        if (read_result < 0) {
          LOG(ERROR) << "invalid result " << read_result;
          DCHECK(false);
          return false;
        }
    }
...
bool PtraceClient::BrokeredMemory::Initialize(int sock, pid_t pid) {
  ...
  request.path.path_length = strnlen(path, sizeof(path)) + 1;

  if (!LoggingWriteFile(sock, &request, sizeof(request)) ||
      !LoggingWriteFile(sock, path, request.path.path_length)) {
    return false;
  }
  ...
  return result == kBoolTrue ? true : ReceiveAndLogError(sock, "PtraceBroker SetMemoryFile");
}

ssize_t PtraceClient::BrokeredMemory::ReadUpTo(VMAddress address, size_t size, void* buffer) const {
