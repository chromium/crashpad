...
bool LoggingSignal(int signum, sighandler_t handler) {
  ...
  PLOG_IF(ERROR, previous == SIG_ERR) << "signal";
  return previous != SIG_ERR;
}
}  // namespace

bool InitializeSignalDispositions() {
  bool success = LoggingSignal(SIGPIPE, SIG_IGN);
  ...
  if (api_level < 0 || api_level == 23) {
    success = success && LoggingSignal(SIGABRT, SIG_DFL);
    success = success && LoggingSignal(SIGFPE, SIG_DFL);
#if defined(SIGSTKFLT)
    success = success && LoggingSignal(SIGSTKFLT, SIG_DFL);
#endif
    success = success && LoggingSignal(SIGTRAP, SIG_DFL);
  }
#endif  // OS_ANDROID
