...
class ScopedPrSetPtracer {
 public:
  explicit ScopedPrSetPtracer(pid_t pid) {
    // PR_SET_PTRACER is only supported if Yama LSM is enabled. Otherwise, this
    // prctl() call fails with EINVAL. See linux-4.9.20/security/yama/yama_lsm.c
    // yama_task_prctl() and linux-4.9.20/kernel/sys.c [sys_]prctl().
    //
    // If Yama LSM is not enabled, the default ptrace restrictions should be
    // sufficient for these tests.
    //
    // If Yama LSM is enabled, then /proc/sys/kernel/yama/ptrace_scope must be 0
