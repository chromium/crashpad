...
ProcessMemoryIOS::MappedMemory::~MappedMemory() {}

bool ProcessMemoryIOS::MappedMemory::ReadCString(size_t offset,
...
ProcessMemoryIOS::MappedMemory::MappedMemory(vm_address_t vm_address,
                                             size_t vm_size,
                                             size_t user_offset,
                                             size_t user_size)
...
ProcessMemoryIOS::ProcessMemoryIOS() : task_(TASK_NULL), initialized_() {}

bool ProcessMemoryIOS::Initialize(task_t task) {
...
std::unique_ptr<ProcessMemoryIOS::MappedMemory> ProcessMemoryIOS::ReadMapped(
  ...
  return std::unique_ptr<MappedMemory>(
      new MappedMemory(address, size, address, size));
}

ssize_t ProcessMemoryIOS::ReadUpTo(VMAddress address,
                                   size_t size,
                                   void* buffer) const {
  INITIALIZATION_STATE_DCHECK_VALID(initialized_);
