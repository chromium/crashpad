...
class ReadCStringTest : public TargetProcessTest {
 ...
 private:
  void DoTest(pid_t pid) override {
    ...
    if (limit_size_) {
      ASSERT_TRUE(ReadCStringSizeLimited(*memory, kConstCharEmpty,
                                         arraysize(kConstCharEmpty), &result));
      EXPECT_EQ(result, kConstCharEmpty);

      ASSERT_TRUE(ReadCStringSizeLimited(*memory, kConstCharShort,
                                         arraysize(kConstCharShort), &result));
      ...
      ASSERT_TRUE(ReadCStringSizeLimited(*memory, member_char_empty_,
                                         strlen(member_char_empty_) + 1,
                                         &result));
      EXPECT_EQ(result, member_char_empty_);

      ASSERT_TRUE(ReadCStringSizeLimited(*memory, member_char_short_,
                                         strlen(member_char_short_) + 1,
                                         &result));
      EXPECT_EQ(result, member_char_short_);
      EXPECT_FALSE(ReadCStringSizeLimited(*memory, member_char_short_,
                                          strlen(member_char_short_), &result));

      ASSERT_TRUE(ReadCStringSizeLimited(*memory, string_long_.c_str(),
                                         string_long_.size() + 1, &result));
      EXPECT_EQ(result, string_long_);
      EXPECT_FALSE(ReadCStringSizeLimited(*memory, string_long_.c_str(),
                                          string_long_.size(), &result));
    } else {
...
class ReadCStringUnmappedTest : public TargetProcessTest {
 ...
 private:
  void DoTest(pid_t pid) {
    ...
    if (limit_size_) {
      ASSERT_TRUE(ReadCStringSizeLimited(*memory, string1_,
                                         expected_length_ + 1, &result_));
      EXPECT_EQ(result_, string1_);
      ASSERT_TRUE(ReadCStringSizeLimited(*memory, string2_,
                                         expected_length_ + 1, &result_));
      EXPECT_EQ(result_, string2_);
      EXPECT_FALSE(ReadCStringSizeLimited(*memory, string3_,
                                          expected_length_ + 1, &result_));
      EXPECT_FALSE(ReadCStringSizeLimited(*memory, string4_,
                                          expected_length_ + 1, &result_));
    } else {
