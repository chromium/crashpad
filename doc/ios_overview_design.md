...
During an exception, the handler may not, for example:

 - Allocate memory.
 - Use libc, or most any library call.

During an exception, the handler may only:
...
Before an exception is triggered, the handler gathers basic system information
...
automatically handle exceptions and capture a crashed process state into an
intermediate dump. However, converting that intermediate dump file into a
minidump is likely unsafe from a crashed process, and uploading a minidump is
definitely unsafe to do at crash time.  Instead, applications have control over
when intermediate dumps are converted to minidumps and when pending minidumps
are uploaded.
...
uploading is enabled, this minidump will also be immediately uploaded. Any new
intermediate dumps generated after this call (such as by simulated
crash (dump-without-crashing) dumps or new exceptions) will not be processed
until the next call to `ProcessIntermediateDumps`.
