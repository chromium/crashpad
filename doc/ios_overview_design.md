...
While handling exception, the handler may not, for example:

 - Allocate memory.
 - Use libc, or most any library call.

While handling exception, the handler may only:
...
dumps. This is expected to happen during application startup, when possible.
After converting, a minidump will be written to the Crashpad database, similar
to how other platforms write a minidump on exception handling. If uploading is
enabled, this minidump will also be immediately uploaded. Any new intermediate
dumps generated after this call (such as by simulated crash
(dump-without-crashing) dumps or new exceptions) will not be processed
